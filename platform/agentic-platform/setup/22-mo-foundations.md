...
```
  ...
  Also add two fixtures in the same pull request, `register/fixtures/schema/pass-imp-mo.yaml` (the row of MO-1.3 with `manifest_sha` of 64 zeros) and `register/fixtures/schema/fail-imp-with-gateway-tier-r.yaml` (tier `R` with no `principal`), and re-run 16 RG-2.5's fixture loop. Why the row rules: `verifier: none` (Mo is recomputed by the validator, not verified by Eve), `privilege: none` and `risk_class: READ` (M-1), `publish_to_gemini: false`, `metric_pack: light` (a placeholder the schema requires; Mo is not measured by a pack). The `agent_id` pattern allows two letters because `mo` is two characters and 16's `{2,30}` requires three; 16's row schema pattern is widened in the same pull request (`^[a-z][a-z0-9-]{1,30}$`), which changes no existing row.
- **VERIFY:** Both `check-jsonschema --check-metaschema` lines print success; RG-2.5's loop prints `PASS-OK` for `pass-imp-mo.yaml` and `FAIL-OK` for the new failing fixture and unchanged results for every earlier fixture; the pull request is merged with the code owner's approval and an RG-3.6 parse file signed by the second human.
...
jq --arg id "$P_ID" --arg dom "$DOMAIN" --arg rc "$REG_COMMIT" '
  ...
  | .trigger_sink=null | .pab_bindings=[] | .project_org_policies=[] | .project_deny_policies=[]
...
- **ACTION:** Run 17's FM-2.2 to FM-2.22 in order, each with its checkpoint `FM-<n>@mo-prod`, inserting 17 §5's module steps: FM-5.1 (Mo's `deny-agents-platform` entry, written into the spec between FM-2.4 and FM-2.15 with the real `MO_PROJECT_NUMBER`, merged as a spec revision) and FM-5.2 (`pab_bindings: []`, the line for 40). FM-2.3 is the project create:
  ...
  FM-2.22 writes the deviation row with the id `BD-22-2`. No other command is typed for this step: every command is 17's, parameterised by the merged spec.
- **VERIFY:** `grep -E $'\tFM-2\.(2|3|4|5|6|7|8|10|11|12|13|15|17|18|19|20|21|22)@mo-prod\tDONE' "$BUILD_LOG_DIR/checkpoints.tsv" | wc -l` prints `18`, and FM-2.9, FM-2.14 and FM-2.16 are `N/A` with their reasons; FM-2.21 prints `ZERO-DIFF` (or exit 0 with `--accept-pending` and only the `_Trace` pending line); `BD-22-2` is in `DEVIATION_REGISTER`.
...
gcloud projects get-iam-policy "$MO_PROJECT" --flatten="bindings[].members" --filter="bindings.role=roles/owner OR bindings.members:user: OR bindings.members:group:" --format="table(bindings.role,bindings.members)"
...
gcloud projects get-iam-policy "$MO_PROJECT" --flatten="bindings[].members" --format="value(bindings.members)" | grep -vE "@${MO_PROJECT}\.iam\.gserviceaccount\.com$|gserviceaccount\.com$" || echo "no foreign non-service-agent member"
gcloud projects get-iam-policy "$MO_PROJECT" --flatten="bindings[].members" --format="value(bindings.members)" | grep -E "gcp-sa-discoveryengine|@[a-z0-9-]+\.iam\.gserviceaccount\.com$" | grep -v "@${MO_PROJECT}\.iam" || echo "no enforcement identity in MO_PROJECT"
```

- **VERIFY:** The describe prints the email and no `True`; the keys list is empty; the project roles print exactly `roles/bigquery.jobUser` (no `dataViewer`, `dataEditor`, `run.invoker`, `secretmanager.secretAccessor`, `aiplatform.user`); the organisation search lists only `//cloudresourcemanager.googleapis.com/projects/<MO_PROJECT>` with `roles/bigquery.jobUser` (dataset access entries appear after MO-6.4 and MO-10.1, and are read there); the two IAM greps print their "no ..." lines (topology rows 24 and 26 anti-grants: no Wall-E, Eve, Gemini or validator identity in `MO_PROJECT`; Google service agents of `MO_PROJECT` itself are expected). If the asset search is refused for lack of `cloudasset.assets.searchAllIamPolicies`, record the refusal and rely on the other reads (17 FM-2.20 records the same limit).
- **ROLLBACK:** `penv_set --force` for a typing error only. The account is removed only by FM-REVOKE.
- **EVIDENCE:** Output as `${R}-3.1-mo-metrics-v1.txt`, `evidence_add MO-3.1 mo-metrics E-08 4.1.1 ...`. TISAX 4.1.1, 4.2.1.
...
- **ACTION:** Nine arithmetic fixtures. The comparison is `ABS(actual - expected) < 0.00005`, half a unit of the fourth decimal: every true value lies within it (the largest gap, 18/20 upper, is 0.0000335), and the old 35/38 constant 0.7921 lies 0.0000937 away, so it fails.
...
- **VERIFY:** The count prints `fixtures 1`, `gates 1`, `image40 2`, `schema22 19`, `schema22own 1`, `schema29 10`, `schema36 1`, `sql29 4`, `sql36 15`, `udf 1`: 31 schema files and 19 SQL files, the counts the S043 verdict corrected. Merged with two human approvals, the second operator first.
...
need_now = {"schema22", "schema22own", "schema29", "schema36", "sql29", "sql36", "gates", "udf", "fixtures"}
missing = {}; bad = []
for r in rows:
    body = show(r["path"])
...
for g, ps in sorted(missing.items()): print(f"{'MISSING' if g in need_now else 'LATER  '} {g}: {len(ps)} file(s): {' '.join(ps)}")
...
mo_ds_access() {   # mo_ds_access DATASET: sets access to exactly [mo-metrics@ WRITER]
  ...
  jq --arg sa "$SA_MO_METRICS" '.access = ([{"role":"WRITER","userByEmail":$sa}] | unique)' "$W/before.json" > "$W/after.json" || { rm -rf "$W"; return 1; }
...
```
  ...
  Each line prints exactly `[{"role":"WRITER","who":"mo-metrics@<MO_PROJECT>.iam.gserviceaccount.com"}]`. For `MO_PRIVATE_DS` this is the permanent state: no later file adds any entry (MD-13 in 40 tests it).
...
need EXP_DAYS
case "$EXP_DAYS" in *[!0-9]*) echo "STOP: not an integer";; esac
...
bq --project_id="$MO_PROJECT" --location=EU query --use_legacy_sql=false --parameter="commit:STRING:${TOIL_COMMIT}" \
  "CREATE OR REPLACE TABLE \`${MO_PROJECT}.${MO_METRICS_DS}.toil_baseline\` OPTIONS(description='Toil baseline (02), keyed on agent_id; rebuilt from the commit-named archive copy on each merge; no expiry (S150)', labels=[('agent','mo')]) AS SELECT 'walle' AS agent_id, @commit AS source_commit, CURRENT_TIMESTAMP() AS loaded_at, * FROM \`${MO_PROJECT}.${MO_ARCHIVE_DS}.toil_baseline_${SHORT}\`"
rm -rf "$T"
```

  *Assumption:* the `labels=[('agent','mo')]` table option form is accepted by `CREATE TABLE`; if refused, drop the option and set the label with `bq update --set_label agent:mo`. `CREATE OR REPLACE` removes nothing but the previous keyed table, whose rows are all in an earlier archive copy.
- **VERIFY:**
...
```
  ...
  Both tables show the same `numRows`, equal to the CSV's data-line count, `no-expiry` and `unpartitioned`. Then the copy of Mo-0's verify, over BigQuery:
...
- **VERIFY:** `exit=0` with only the `_Trace` pending line (and none other); `tail -n 1 "$DEVIATION_REGISTER"` shows `BD-22-5`; the grants list with `state=ACTIVE` prints nothing; `gcloud projects get-iam-policy "$MO_PROJECT" --flatten="bindings[].members" --filter="bindings.members:user:" --format="value(bindings.role)"` prints nothing.
...
- [ ] MO-2.1, MO-2.2: run spec merged; FM-2.2 to FM-2.22 `DONE@mo-prod` (N/A where stated); checker `ZERO-DIFF`; `BD-22-2`.
...
- [ ] MO-3.1: `SA_MO_METRICS` keyless; only `roles/bigquery.jobUser`, only in `MO_PROJECT`; anti-grants 24 and 26 hold.
...
- [ ] MO-5.1, MO-5.2, MO-5.3: inputs list merged (31 schemas, 19 SQL); gate `INPUTS-COMPLETE` with `MO_INPUTS_COMMIT` and the second operator's record, or `INPUTS-BLOCKED` indexed as B-14.
- [ ] MO-6.1 to MO-6.4: four `EU` datasets with the signed names, no default expiry, each access array exactly `mo-metrics@ WRITER`.
...
- [ ] MO-8.1 to MO-8.3: toil baseline loaded from the merge commit into a commit-named archive table and the keyed table, both unpartitioned and without expiry; Mo-0's counts pass; `BD-22-4`.
...
| 40 | `MO_ARCHIVE_DS` (snapshots, rows 19 and 20 metadata grants); `MO_PRIVATE_DS` with exactly one entry (MD-13); the toil baseline table and archive copies; the FM-5.2 PAB line; `image40` inputs (B-15) | MO-6.4, MO-8.2, MO-2.1 |
...
| S043 | blocking | Closed | The inputs list with counts (31 schemas, 19 SQL, `gates.yaml`, UDFs, fixtures) and the gate at a named commit with the second operator's review; every step needing them BLOCKED on B-14 (MO-5.1 to MO-5.3, MO-6.5, MO-7.5, MO-7.6); the repository and path named (`mo/` in the platform repository) |
...
| S147 | major | Closed | No cross-owner command in an executable block; the "made elsewhere" table (§10) with PENDING lines (MO-10.2); `mo_ds_access` with `mktemp -d`, read failure returning non-zero, etag check, `unique` and read-back (MO-6.4) |
| S150 | major | Closed for the load | Explicit `bq load` from the merge commit into a commit-named archive table, then the keyed table, both without expiry; no transfer config (MO-8.1, MO-8.2); retention exception recorded (MO-8.3). 02 closed the CSV form |
...
| S155 | major | Closed for the grant and check half | WRITER on `MO_VIEWS_DS` kept for its purpose (36 creates views as `mo-metrics@`); zero-non-VIEW and private-only checks committed and run (MO-6.4, MO-6.6). The view creation and the check after it are 36 |
...
- The exact default access entries BigQuery adds when a dataset is created without an access list, in particular the creator's `OWNER` entry (MO-6.4); the step reads `before.json` and removes whatever it holds.
- Whether a budget read returns `projects/<number>` or `projects/<id>` in `budgetFilter.projects` (MO-2.3).
- Whether `CREATE OR REPLACE TABLE ... OPTIONS(labels=[...])` accepts the labels option in that form (MO-8.2).
