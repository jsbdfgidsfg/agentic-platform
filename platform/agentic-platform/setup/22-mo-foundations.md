...
grep -vE " serviceAccount:[^@]+@(${MO_PROJECT}\.iam|gcp-sa-[a-z0-9-]+\.iam|bigquery-encryption\.iam|system|cloudservices)\.gserviceaccount\.com$" "${R}-3.1-members-v1.txt" || echo "every project-level member is MO_PROJECT's own service account or a Google service agent"
...
gcloud identity groups describe "$MO_OWNER_GROUP" --format="value(groupKey.id)" >/dev/null 2>&1 && echo "mo-owners@ exists (MO-1.2)" || { echo "STOP: mo-owners@ does not exist; run MO-1.2 first — an access array without an OWNER is refused"; }
