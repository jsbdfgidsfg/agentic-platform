...
- **ACTION:** The twin proved two things about itself that only have value if they are also true of production on the day the role lands. [37](37-wall-e-sandbox-rehearsal.md) named both as re-run lines into this file.
  ...
  1. **No human can act as the credential holders, nor as the approval surfaces.** If a person can impersonate `walle-actions@` or `walle-actions-super@`, the two-credential split is decoration and the robot's Super Admin is one `gcloud` away from a human. The same question is asked of `walle-approvals@` and `walle-approvals-super@` (`SA_APPROVAL_A`, `SA_APPROVAL_SUPER`), because an impersonated surface is a forged approver: the action service takes the approver's identity as the surface forwards it ([33](33-wall-e-action-services-and-approval-surfaces.md) `WS-5.8`), so whoever can mint an ID token for a surface can post an approval naming `SA_2_ADMIN` and pass every in-app check — and [33](33-wall-e-action-services-and-approval-surfaces.md) `WS-5.2` granted exactly that ability to `SA_1_ADMIN` for a sitting, which `WS-9.0` alone withdrew.
...
```bash
need ORG_ID WALLE_PROJECT SA_ACTIONS SA_ACTIONS_SUPER SA_APPROVAL_A SA_APPROVAL_SUPER WALLE_CODE_COMMIT REGION
for sa in "$SA_ACTIONS" "$SA_ACTIONS_SUPER" "$SA_APPROVAL_A" "$SA_APPROVAL_SUPER"; do
...
python3 - "$R" "$SA_ACTIONS" "$SA_ACTIONS_SUPER" "$SA_APPROVAL_A" "$SA_APPROVAL_SUPER" <<'PY'
import json, sys
R = sys.argv[1]
