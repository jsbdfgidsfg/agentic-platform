...
- Step prefix: `PA`. Steps: 38. BLOCKED: PA-4.7 (the security reviewer is not named, README B-20), PA-8.1 (same person), PA-8.5 (the factory does not exist, README B-01). IRREVERSIBLE: PA-9.3 (the withdrawal leaves the platform owner no standing path back; only break-glass or an approved grant restores it).
...
The Privileged Access Manager (PAM) catalogue of [04 §5.2](../04-identity-and-privileged-access.md#52-the-catalogue), built by hand from committed files, each entitlement proven by one real grant, and then the removal of every standing human privilege the bootstrap needed. After this file, every later privileged step in files 13 to 42 names an entitlement, and no human or group holds any role from any Role column standing, except the break-glass group `gcp-organization-admins@` (04 §7.1, by design).
...
| The committed catalogue: one JSON file per entitlement, four templates, an index, and three small tools (catalogue writer, live compare, standing-role sweep) | `pam/` in the platform repository | `ENT_PROJECT_REPAIR_TEMPLATE`, `ENT_DEPLOY_CREDENTIAL_HOLDER_TEMPLATE` |
...
```mermaid
flowchart TD
  A["PA-0 Sitting, gates, exception rights checked"] --> B["PA-1 PAM API, service agent, PAM Admin to platform-owners@"]
  B --> C["PA-2 Committed catalogue files, reviewed by the second human"]
  C --> D["PA-3 Organisation entitlements and tests"]
  C --> E["PA-4 Folder and project entitlements and tests"]
  C --> F["PA-5 Gemini Enterprise entitlements and tests"]
...
- **WHERE:** Shell, `~/.platform-env` sourced.
- **ACTION:** Every PAM call passes `--billing-project="$CICD_PROJECT"`, so the API must be enabled there, and the requester group needs `serviceusage.services.use` there once the creator's Owner is gone (PA-9.2). `privilegedaccessmanager.googleapis.com` is not on 02 §4.2's `fld-platform-core` allow-list: this is recorded as a deviation and handed to 13, which adds it to the list by pull request before applying it (the same route as 10's `observability`).
...
pam_wait() { # GRANT STATE: polls every 20 s for up to 20 min
  i=0; while [ "$(pam_state "$1")" != "$2" ]; do i=$((i+1)); [ $i -gt 60 ] && { echo "TIMEOUT waiting for $2"; return 1; }; sleep 20; done; echo "STATE $2"
}
...
listed in pam/role-columns.txt, conditioned or not, except (a) GRP_GCP_ORG_ADMINS at the organisation (break-glass,
04 section 7.1) and (b) the conditioned binding of SWEEP_GRANT_MEMBER for SWEEP_GRANT_ROLE while that grant is active.
Also prints OWNER lines for roles/owner or roles/editor on human, group or domain members. Stdlib only (setup/12 PA-1.5)."""
import json, os, subprocess, sys, pathlib
...
for kind, ident in scopes:
    ...
    for b in json.loads(r.stdout).get("bindings", []):
        for m in b["members"]:
            ...
            if b["role"] in ("roles/owner", "roles/editor") and kind == "projects":
                print(f"OWNER {kind}/{ident} {b['role']} {m} {cond}"); bad = 1 if ident != E["GEMINI_PROJECT"] else bad
            ...
            if m == gm and b["role"] == gr and b.get("condition"):
                continue
            print(f"VIOLATION {kind}/{ident} {b['role']} {m} {cond}"); bad = 1
...
#!/usr/bin/env python3
"""Writes pam/entitlements/*.json, pam/templates/*.json, pam/index.tsv, pam/role-columns.txt and pam/no-approval.json
from ~/.platform-env values (setup/12 PA-2.1). Stdlib only. Re-running rewrites the same bytes."""
...
roles = sorted({b["role"] for *_, d in rows for b in d["privilegedAccess"]["gcpIamAccess"]["roleBindings"]} |
               {b["role"] for d in templates.values() for b in d["privilegedAccess"]["gcpIamAccess"]["roleBindings"]} | {"roles/iam.serviceAccountTokenCreator"})
...
print(len(rows), "entitlement files;", len(templates), "templates;", len(roles), "roles;", len(noappr), "no-approval")
EOF
...
- **VERIFY:** The script prints `21 entitlement files; 4 templates; <n> roles; 6 no-approval` (22 files once the security reviewer is named). Then:
...
```
  ...
  The second human reads, for every file: the scope, the roles against the table above, the requesters, the approver set, the no-approval list, and the deviation table. They approve only if no approver set contains `sa-1-admin@` or `platform-owners@` and the no-approval list has exactly six entries. Then merge and pull `main`.
...
- **VERIFY:** `gcloud pam entitlements describe "$ENT_PLATFORM_POLICY" --billing-project="$CICD_PROJECT" --format="yaml(state,maxRequestDuration,approvalWorkflow,eligibleUsers,privilegedAccess)"` shows `state: AVAILABLE`, `3600s`, one step with `approvalsNeeded: 1` and approver `user:sa-2-admin@…`, requester `group:platform-owners@…`, the three roles on `//cloudresourcemanager.googleapis.com/organizations/<ORG_ID>`. If the create refuses the JSON file, convert nothing by hand: stop, record the error, and write the file as YAML by the same generator (a one-line change reviewed as in PA-2.2).
...
tip() { curl -sS -X POST -H "Authorization: Bearer $(gcloud auth print-access-token)" -H "x-goog-user-project: $CICD_PROJECT" -H "Content-Type: application/json" -d "{\"permissions\":[$2]}" "https://cloudresourcemanager.googleapis.com/v3/$1:testIamPermissions"; }
...
If the shell was restarted after PA-3.2, re-run the `source` line and the `tip` definition of PA-3.2's ACTION before any later test.
...
- **VERIFY:** `describe` shows `7200s` and 19 roles. T3 prints a condition for each of the 19 roles; T4 prints `NO-BINDING` 19 times; T5 `approved`. If a role is refused at create ("doesn't support" or an unsupported-role error, for example a Beta role), stop, remove it from `catalogue.py` by a reviewed pull request, and record which role and why.
...
for g in $(gcloud pam grants search --entitlement="$ENT_PROJECT_REPAIR_CORE" --caller-relationship=had-created --billing-project="$CICD_PROJECT" --filter="state=ACTIVE" --format="value(name)"); do echo "ACTIVE GRANT $g"; done
...
for p in "$CICD_PROJECT" "$CORE_PROJECT" "$LOGGING_PROJECT" "$KMS_PROJECT" "$VALIDATOR_PROJECT"; do
  for sa in $(gcloud iam service-accounts list --project="$p" --format="value(email)"); do
    gcloud iam service-accounts get-iam-policy "$sa" --project="$p" --format=json | jq -r --arg sa "$sa" '.bindings[]? | select(.role=="roles/iam.serviceAccountUser" or .role=="roles/iam.serviceAccountTokenCreator") | .members[] | select(startswith("user:") or startswith("group:") or startswith("domain:")) | "ACTAS \($sa) \(.)"'
  done
done | tee "$BUILD_LOG_DIR/evidence/12/PA-9.1-actas.txt"
```

- **VERIFY:**
  1. `CATALOGUE ZERO DIFF`; no `ACTIVE GRANT` line (repeat the search for any entitlement tested today if in doubt).
  2. The sweep prints exactly these lines and no other `VIOLATION` or `OWNER`: `VIOLATION organizations/<ORG_ID> roles/resourcemanager.organizationAdmin user:sa-1-admin@… bootstrap-exception-sd-01`, the same for `roles/resourcemanager.projectCreator`, and one `OWNER projects/<id> roles/owner user:sa-1-admin@…` for each of the five core projects. `OWNER` lines on `GEMINI_PROJECT` are listed for 19 and do not fail the sweep. The last line is therefore `SWEEP NOT CLEAN`, expected here. Folder Creator and PAM Admin are not Role-column roles, so they are removed by PA-9.3 but not listed here.
  3. `PA-9.1-actas.txt` is empty (S143: no human, group or domain holds actAs or token creation on any core service account).
  Any other line is a stop: remove it by the path its entitlement gives, with the second human watching, and re-run this step.
- **ROLLBACK:** Read only.
- **EVIDENCE:** both files. `evidence_add PA-9.1 sweep-before-withdrawal E-05 4.2.1 "build-log:evidence/12" "$BUILD_LOG_DIR/evidence/12/PA-9.1-sweep.txt"`.
...
grep -P "\tPA-4.2\tDONE\t" "$BUILD_LOG_DIR/checkpoints.tsv" | cut -f2,3 || echo "STOP: PA-4.2 not DONE"
for p in "$CICD_PROJECT" "$CORE_PROJECT" "$LOGGING_PROJECT" "$KMS_PROJECT" "$VALIDATOR_PROJECT"; do
...
- **VERIFY:**
  1. The sweep's last line is `SWEEP CLEAN`: no role from any Role column is bound, conditioned or not, to any `user:`, `group:` or `domain:` member on the organisation, the 22 folders, the five core projects or `GEMINI_PROJECT`, except `group:gcp-organization-admins@…` at the organisation (break-glass by design) and this sweep's own PAM grant. `OWNER` lines appear only for `GEMINI_PROJECT` (19's removal).
...
- [ ] PA-0.2: the exception carried every permission PAM creation needs, tested, not assumed.
...
- [ ] No `user:`, `group:` or `domain:` member holds actAs or token creation on any core service account (PA-9.1).
...
| [16](16-register-and-shared-registry.md) | `pam/no-approval.json`, `pam/tools/compare.py`, `pam/tools/sweep.py`, `pam/role-columns.txt`; `ENT_PROJECT_REPAIR_CORE` for registry repair | The drift job runs both tools daily |
...
| S143 | major | The catalogue half: `ent-deploy-credential-holder` template and a proven instance replace standing actAs; PA-9.1 proves no human or group holds actAs or token creation on any core service account. | 25 and 26 run the same check on Eve's accounts and obtain actAs only through `ENT_DEPLOY_CREDENTIAL_HOLDER_EVE`. |
...
|---|---|---|
| gcloud's `--entitlement-file` accepts a JSON document | The reference says "YAML file"; JSON is valid YAML, but the parser was not run | PA-3.1's create and PA-6.1's compare |
