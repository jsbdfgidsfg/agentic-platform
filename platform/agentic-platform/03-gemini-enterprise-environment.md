...
- Maturity: detailed design, written 2026-09-13 against [01-hld.md](01-hld.md) §2 (the tenant app
  ...
  Model Armor row) and §7.1 (Data Access logs for `discoveryengine`). Nothing is built. Answers
  HLD brief item A6 of [00-objective-review.md](00-objective-review.md); closes the
...
- Decisions this page makes are P48–P59, final ids in [12-open-decisions.md](12-open-decisions.md).
...
Reading Google's pages again on 2026-09-13 changes seven things the HLD wrote in §2; each is
stated once here:
...
flowchart LR
    ...
    subgraph GP["GEMINI_PROJECT — fld-gemini-enterprise — location eu"]
        APP["Tenant app<br/>app-level IAM: ge-users@ agentspaceUser<br/>licence at first sign-in<br/>Assistant: retention 30 d (interim), Model Armor BLOCK on failure"]
...
|---|---|---|
| The app | One Gemini Enterprise app per tenant is the front door for every human-facing agent (no second production app, P59). Agents no human talks to (Eve's control path, Mo's jobs) are not published in it | One front door, one audit log naming the human |
| Project | `GEMINI_PROJECT` / `GEMINI_PROJECT_NUMBER`, the existing app project **imported in place** by the factory's `tenant-app` module and moved under `fld-gemini-enterprise` in a dated change window with the Gemini Enterprise administrators' agreement; a move changes inherited policy, so the window's exit is the drift job's zero diff. If the administrators refuse the move, the project stays outside the folder as topology decision 52 allowed — recorded as the one project outside the folder — and the floor, the deny policy, the `discoveryengine` Data Access config, the sink and every other folder control of this page are re-applied at project level by the same module, with a review date. Topology decision 52 therefore becomes "the app project is a platform project, imported in place"; the platform departs from 52's "only if they agree, otherwise outside" recommendation because `gemini-egress` (§11) makes the app project a **publication enforcement point**, and a project outside the folder misses the floor, the deny policy and the aggregated sink that make that enforcement auditable | `Assumption:` (topology §2) the app already exists in a project of its own; a live app cannot be re-created without losing chat history and data stores |
...
| `restrictServiceUsage` | allow-list = the "Hosts" row's services plus `cloudkms` (for the CMEK key's use) and `orgpolicy` reads; `aiplatform.googleapis.com` **not** enabled here — and absent from the `fld-gemini-enterprise` folder list of [02-landing-zone-and-tiers.md](02-landing-zone-and-tiers.md) §4.2 | An engine can never be created in the app project by accident |
...
The permission lists were read in full on 2026-09-13 by raw fetch ([discoveryengine roles and permissions](https://docs.cloud.google.com/iam/docs/roles-permissions/discoveryengine), updated 2026-09-10): `roles/discoveryengine.editor` holds `discoveryengine.agents.create`
and `agents.update` but **not** `agents.setIamPolicy`, so CI with `editor` can register an agent
but **cannot share it**; `discoveryengine.agents.setIamPolicy` is held by `roles/discoveryengine.admin`,
`roles/discoveryengine.agentAdmin`, `roles/iam.securityAdmin` and `roles/discoveryengine.agentspaceAdmin`;
`roles/discoveryengine.viewer` does **not** hold `discoveryengine.userStores.listUserLicenses`,
while `agentspaceAdmin` holds it (through `userStores.*`) together with `engines.update`. What
each role *does* in the console is taken from the product pages that name it.

| Group | Role | Level | Standing or PAM | Duty | Source |
|---|---|---|---|---|---|
| `ge-admins@` (P49) | `roles/discoveryengine.agentspaceAdmin` | `GEMINI_PROJECT` | **PAM entitlement `ent-ge-admin`** ([04 §5.2](04-identity-and-privileged-access.md#52-the-catalogue), where its duration and approver are defined); no standing member holds it | Feature toggles, Model Armor, Assistant settings, licences ("Manage users"), agent registration and sharing approval, gateway binding on the Security tab, CMEK registration | [manage-web-app-features](https://docs.cloud.google.com/gemini/enterprise/docs/manage-web-app-features), [enable-model-armor](https://docs.cloud.google.com/gemini/enterprise/docs/enable-model-armor), [register-and-manage-an-adk-agent](https://docs.cloud.google.com/gemini/enterprise/docs/register-and-manage-an-adk-agent) |
...
| CI identity (`CICD_PROJECT`, via WIF) | `roles/discoveryengine.editor` | `GEMINI_PROJECT` | standing, **the one foreign principal** | Import endpoints into `gemini-registry`, write the gateway access policy, **register** agents in the app (§10; `editor` holds `agents.create`/`update`, verified 2026-09-13). **Sharing is not CI's**: `editor` lacks `discoveryengine.agents.setIamPolicy`, so the share is made by `ge-admins@` through PAM (`agentspaceAdmin`) and CI only verifies it; a custom role holding `agents.setIamPolicy` for CI is the recorded alternative, not adopted (it would let the pipeline publish to every user without a human act). Licence listing for P55's quarterly review needs `agentspaceAdmin` through PAM or a custom role with `discoveryengine.userStores.listUserLicenses`, never `viewer` (which lacks it) | access-control ("read and write access to all discovery engine resources") |
| Platform owner | `roles/orgpolicy.policyAdmin` | organisation | PAM entitlement `ent-platform-policy` ([04 §5.2](04-identity-and-privileged-access.md#52-the-catalogue)) | The three managed constraints and the custom constraint of §3 | [configure-allowed-data-sources](https://docs.cloud.google.com/gemini/enterprise/docs/connectors/configure-allowed-data-sources) |
...
| Nobody | `roles/owner`, `roles/editor` (basic) | `GEMINI_PROJECT` | removed after the factory run; incident access only through the PAM catalogue of [04 §5.2](04-identity-and-privileged-access.md#52-the-catalogue) (PAM cannot grant legacy basic roles, [04](04-identity-and-privileged-access.md) §5.1) | HLD §4.4 | — |
...
2 finding. If the drift job's report is absent for 26 h the absence alarm fires to
`platform-security@` ([04-identity-and-privileged-access.md](04-identity-and-privileged-access.md) §2.6).
...
Decision: the platform key ring (HLD §8.3) gains one `europe` multi-region key `gemini-cmek`, in `KMS_PROJECT`, ring `gemini` ([09](09-supply-chain-secrets-recovery.md) §2.4 row; Autokey's compatible-services list read on 2026-09-13 does not name Discovery Engine or Gemini Enterprise, which confirms the explicit key);
...
### 5.4 Conversation retention (P52, under P13)

Conversation history is kept **30 days** (`Assumption:` until the DPO decides under P13; P52),
set on the Assistant tab at runbook step GE-6 (§16); Google's selectable values, the Plus-edition
condition, why the store is not the Art. 12 log, and the fallback when the edition hides the
setting are row R8 of
[08 §5.2](08-data-logging-retention-sovereignty.md#52-the-schedule).

---

## 6. Identity provider and SSO (P51)
...
## 7. The user population: three gates (P55)
...
| Licence | Cloud console → Gemini Enterprise → Manage users; **automatic assignment** at first sign-in, bounded by the two gates above; manual assignment for named exceptions | subscription count *tbd*; quarterly review against `ge-users@` membership | `ge-admins@` via PAM | `discoveryengine.userStores.listUserLicenses` read at the quarterly review through `agentspaceAdmin` via PAM or a custom role holding that permission (`viewer` lacks it, verified 2026-09-13, §4) | licences > members + 5 % → finding; a licence on a suspended account → reclaimed |
...
## 8. Feature Management baseline (P54)
...
## 9. The console Model Armor setting (P53)

The administrator's path: **App → Configurations → Assistant → Model Armor**, enabled by
`roles/discoveryengine.agentspaceAdmin` (`ge-admins@` through PAM) after `roles/modelarmor.admin`
has created the template ([enable-model-armor](https://docs.cloud.google.com/gemini/enterprise/docs/enable-model-armor));
runbook step GE-7 (§16). The value (P53): **on**, template `ge-console-standard` in
`GEMINI_PROJECT`, failure mode **Block all user interactions**; a flip to Allow is a PAM act with
an incident reference. What the setting covers and does not cover, the template's content, the
findings path, the quota, the daily drift check and the failure handling are defined once in
[06 §3.5](06-gateways-model-armor-perimeter.md#35-the-console-setting-tenant-wide-by-rule-per-app-by-mechanism).
...
| Revoke | unshare in the console; row → `retired` | one factory operation `revoke` ([02](02-landing-zone-and-tiers.md) §3.3): endpoint out of `gemini-egress` and `gemini-registry`, `geEngineQuery` binding removed, DELETE on the app's agent resource (documented: DELETE on the agent path), row and card `retired`, `halt_all` for a write agent, project deletion scheduled after the evidence export confirms; a pull request merged by two humans and applied by CI | unshare; supplier row closed | agent owner requests; platform owner approves; a Tier P revoke is also K3 |
| Emergency — levers ordered by speed | unshare (console unpublish), any hour, `ge-admins@` via PAM (no-approval activation with justification if the approver is unreachable) | (1) remove the engine from `gemini-egress`'s access policy — seconds (the HLD names `ge-admins@` through PAM as the actor, while §4 gives the access-policy write to the CI identity; which acts in an emergency is **unverified**, *tbd*); (2) remove the Discovery Engine service agent's `geEngineQuery` binding on the engine — a minute, the agent project's CI; (3) unregister / unpublish in the app. All three are audited into `LOGGING_PROJECT` and the SIEM (§13); K3 (remove `run.invoker` / the query role) and K7 sit behind them | unshare | on-duty admin; recorded as an incident |
...
## 11. The tenant egress gateway `gemini-egress` (P57, detailing HLD P6)
...
| If Stage 1 fails | the compensating control stands: the connector allow-list (§12) plus the per-engine query grant, recorded as a dated row with a re-test at each Agent Gateway release note | — |
...
tenant's build and is outside this page (P59).
...
| Residency | data stores in `eu` only; a data store outside the EU is a dated exception | Gemini Enterprise admin | data store | `gcp.resourceLocations` (folder) plus the drift job's data-store list | refused at creation |
...
Detections on these streams are rule PL-10 of the catalogue of record,
[07 §6.4](07-monitoring-detection-incident-response.md#64-the-platform-set-and-the-agent-set),
owned by the security reviewer: severity 1 for a gateway unbind or console Model Armor change
outside a change window, `StreamAssist` by a robot account, or an engine mutated by the app's
Discovery Engine service agent; severity 2 for a standing admin, an over-wide share, a
`gemini-registry` or gateway-policy write outside CI, or an org-policy write on
`fld-gemini-enterprise`; severity 3 for `StreamAssist` or drift-report absence.
...
and the Model Armor sanitize payloads (30 days, restricted bucket, §13). Chat history and the
Data Access entries are rows R8 and R4 of the retention schedule,
[08 §5.2](08-data-logging-retention-sovereignty.md#52-the-schedule). Each store is also a row of
the TISAX asset classification (`Assumption:` Confidential, HLD §5.1). Chat history is not exported to the evidence lake — the audit table is
the evidence, and copying prompts would defeat the 30-day choice. The DPO's DPIA for the first
...
| App-level `agentspaceUser` to groups; no project-level user role | Gemini Enterprise admin (CI applies it) | the app's IAM policy | drift job | sev 2, removed by CI |
...
| Retention 30 days (`Assumption:` until P13) | DPO decides; admin sets | assistant config | drift job (`GetAssistant`) | drift → restored within a day |
...
| GE-4 | [platform owner] Folder additions: the three managed constraints (`allowedDataSources` and `allowedEgressFqdns` each listing `GEMINI_PROJECT_NUMBER` in `enforcedProjects`), `geDataStoreAclRequired` in dry-run, folder `auditConfigs` for `discoveryengine` `DATA_READ`/`DATA_WRITE` (`gcloud resource-manager folders get-iam-policy` → edit `auditConfigs` → `set-iam-policy`, per Google's page), `restrictServiceUsage` allow-list, `iam.managed.disableAccessPolicyBinding` lifted on `GEMINI_PROJECT` as a factory input with reason "gateway binding" and a review date; the `europe` multi-region key `gemini-cmek` and the `CmekConfig` registration | a `StreamAssist` entry appears in `LOGGING_PROJECT` within 10 minutes of a test question; adding a disallowed source is refused | policies removed by Terraform |
...
| GE-12 | [agent project's CI, per agent] For each `publish_to_gemini: true` row: `geEngineQuery` bound on the engine; endpoint imported into `gemini-registry`; registration in the app (CI); share to the audience group (`ge-admins@` through PAM — CI's `editor` cannot share, §4; CI then verifies); SPIFFE check recorded; denial suite: a colleague outside the group does not see the agent; the audit row names the human | HLD §2.2 admission passes in one CI run | `revoke` |
...
| GE-14 | [platform owner] Reconciliation and drift job extended to the app: agent list, gateway policy, registry, toggles, Model Armor, IAM; penetration test of the front door and the share before S1; [Gemini Enterprise admin] write the values into [../gemini-enterprise.md](../gemini-enterprise.md) and close the Tier C gate | first nightly run zero findings; gate row green in HLD §0.4 | — |

---

## 17. Decisions recorded (P48–P59 in [12-open-decisions.md](12-open-decisions.md))

This page records **P48–P59** (placement and folder constraints, the administration model,
location and encryption, the identity provider, conversation retention, the console Model Armor
setting, the Feature Management baseline, the population gates, the cross-project grant,
`gemini-egress`, connectors, and the one production app); each row, with its options, owner and
gate, is in the register of record,
...
|---|---|---|
| *(Closed 2026-09-13.)* Permission lists of `editor`, `viewer`, `agentspaceAdmin` | read by raw fetch: `editor` registers (`agents.create`/`update`) but cannot share (no `agents.setIamPolicy`); `viewer` lacks `userStores.listUserLicenses`; `agentspaceAdmin` holds `agents.setIamPolicy`, `engines.update` and `userStores.*` | applied in §4, §10.1 and GE-12: CI registers, `ge-admins@` shares through PAM; licence listing through `agentspaceAdmin` or a custom role. `agentspaceUser` and `serviceAgent` lists not re-read — GE-5 |
...
| Whether the Security Health Analytics custom-module resource types include the Gemini Enterprise `Engine` | 91 supported types on 2026-09-13, 14 of them Gemini Enterprise Agent Platform (SHA custom-modules source, HLD §19); not itemised | build of the module |
...
- https://docs.cloud.google.com/iam/docs/roles-permissions/discoveryengine — permission lists of `editor`, `viewer`, `agentspaceAdmin`, read by raw fetch; page 2026-09-10
- Did not render: Agent Registry set-up page (404 at the guessed path)
...
- [00-objective-review.md](00-objective-review.md) — HLD brief item A6
...
- [12-open-decisions.md](12-open-decisions.md) — the register: P48–P59 are this page's rows
