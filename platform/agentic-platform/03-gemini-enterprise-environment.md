...
One spelling to record: Google's set-up page writes the constraint lifted for gateway binding
as `constraints/iam.managed.disableAccessPolicyBindings` (plural, "Disable binding access policy
to resource") while the constraints reference writes `iam.managed.disableAccessPolicyBinding`
(singular). The reference spelling — the HLD's and the fleet's — is used on this page from the
reconcile pass of 2026-09-13; the spelling is confirmed at the first factory run
([06-gateways-model-armor-perimeter.md](06-gateways-model-armor-perimeter.md) §8 rows G3, O1).
...
The permission lists were read in full on 2026-09-13 by raw fetch ([discoveryengine roles and permissions](https://docs.cloud.google.com/iam/docs/roles-permissions/discoveryengine), updated 2026-09-10, read by raw fetch 2026-09-13; an earlier pass
found the page unrendered): `roles/discoveryengine.editor` holds `discoveryengine.agents.create`
...
|---|---|---|---|---|---|
| `ge-admins@` (P49) | `roles/discoveryengine.agentspaceAdmin` | `GEMINI_PROJECT` | **PAM entitlement, 1 h, approver = security reviewer** (HLD §4.4 row); no standing member holds it | Feature toggles, Model Armor, Assistant settings, licences ("Manage users"), agent registration and sharing approval, gateway binding on the Security tab, CMEK registration | [manage-web-app-features](https://docs.cloud.google.com/gemini/enterprise/docs/manage-web-app-features), [enable-model-armor](https://docs.cloud.google.com/gemini/enterprise/docs/enable-model-armor), [register-and-manage-an-adk-agent](https://docs.cloud.google.com/gemini/enterprise/docs/register-and-manage-an-adk-agent) |
...
`platform-security@` ([04-identity-and-privileged-access.md](04-identity-and-privileged-access.md) §2.6; the number aligned 2026-09-13).
...
Decision: the platform key ring (HLD §8.3) gains one `europe` multi-region key `gemini-cmek`, in `KMS_PROJECT`, ring `gemini` ([09](09-supply-chain-secrets-recovery.md) §2.4 row, added 2026-09-13; Autokey's compatible-services list read on 2026-09-13 does not name Discovery Engine or Gemini Enterprise, which confirms the explicit key);
...
argues for short. Decision: **30 days** as `Assumption:` pending the DPO (P13) — this page first
proposed 90 days on the ground that an operator's "what did I ask Wall-E a month ago" is served
by the audit table, not by chat history; [08-data-logging-retention-sovereignty.md](08-data-logging-retention-sovereignty.md)
§5.2 R8 proposed 30 as the `content` ceiling; the reconcile pass of 2026-09-13 recorded **30**
as the one interim number both pages and runbook step GE-6 use (P52 aligned with P113), and
the DPO settles it before Stage 1. Users may delete their own chats sooner;
memory/personalisation is off (§8). The
...
| Absence: no `StreamAssist` entries for 4 business hours; no drift-job report for 26 h (the drift job's own absence alarm, [04-identity-and-privileged-access.md](04-identity-and-privileged-access.md) §2.6) | absence policies | 3 |

These rules are carried in the catalogue of record as PL-10 of
[07-monitoring-detection-incident-response.md](07-monitoring-detection-incident-response.md) §6.4;
other pages cite that id.
...
|---|---|---|---|---|
| P48 | `GEMINI_PROJECT` imported in place under `fld-gemini-enterprise`; the folder gains `discoveryengine.managed.allowedDataSources` and `allowedEgressFqdns` (both with `enforcedProjects` = `GEMINI_PROJECT_NUMBER`, since neither binds a non-VPC-SC project otherwise), the custom-MCP block kept enforced, and `custom.geDataStoreAclRequired` (dry-run first); the project hosts the app, `gemini-egress`, `gemini-registry`, `ge-console-standard`, the CMEK registration and nothing of any agent's | leave outside the folder with project-level re-application (fallback, dated) | platform owner + Gemini Enterprise admin | Tier C |
| P49 | Administration model: `ge-admins@` holds `agentspaceAdmin` only through PAM (1 h, security-reviewer approval); `ge-readers@` standing `viewer`; the CI identity standing `editor` as the one foreign principal; no basic roles; no project-level `agentspaceUser` | standing admin for one person (rejected: no record, no window) | platform owner | Tier C |
| P50 | Location `eu`; CMEK with a `europe` multi-region key registered by explicit `CmekConfig` for everything created after GE-4; the imported app recorded as Google-managed encryption (cannot be protected in place); Google Search grounding off; Enterprise web search off until a row needs it | recreate the app to get CMEK (rejected: loses history and data stores; revisit with P20) | platform owner; ISMS for the CMEK position | TISAX assessment order |
| P51 | Identity provider **Google Identity** for the `eu` location (case (a)); any change is a decision record stating the chat-history loss; P24 answered "no" for the tenant app | Workforce Identity Federation (rejected: no live group re-check on the write path, two-step removal, K5 needs a Google identity anyway) | Gemini Enterprise admin | Tier C |
| P52 | Conversation retention **30 days** as `Assumption:` until the DPO decides under P13, from Google's 1–180-day list (this page's original 90 was aligned with page 08's P113 on the reconcile pass of 2026-09-13); not the Art. 12 log; memory off | 90 (this page's first proposal), 60 (default), 180 | DPO | Stage 1 of the first Tier W agent |
| P53 | Console Model Armor **on**, per app, template `ge-console-standard` in `GEMINI_PROJECT` from the Tier C row of the template standard, failure mode **Block**; a flip to Allow is a PAM act with an incident reference | Allow on failure (rejected: HLD §6.1's rule) | Gemini Enterprise admin; template by platform owner | Tier C |
| P54 | Feature Management baseline of §8 as a committed file; sharing requires admin approval; sharing to groups; skills, memory, session sharing, model selector, image and video generation off | — | Gemini Enterprise admin | Tier C |
| P55 | Population = three gates: OU service toggle (robot OU off), app-level `agentspaceUser` to `ge-users@`/`ge-builders@`, automatic licence assignment within those gates with a quarterly review | manual licences per person (rejected: a console habit without a record) | Gemini Enterprise admin; Service Settings admin | Tier C |
| P56 | The cross-project grant generalised: custom role `geEngineQuery` in every agent project bound on the engine to the app's Discovery Engine service agent, made by the agent project's CI; the documented project-level `roles/discoveryengine.serviceAgent` is a per-agent dated fallback with a mutation detection; registration by CI (`editor` holds `agents.create`/`update`); sharing by `ge-admins@` via PAM because `editor` lacks `agents.setIamPolicy` (verified 2026-09-13) | project-level role everywhere (rejected: carries create/delete/update) | platform owner | every publication |
| P57 | `gemini-egress` bound to the tenant app in three stages (throwaway-app spike → 30-day `DRY_RUN` → enforce); `gemini-registry` in `europe-west1` as a CI-generated working set; the assistant joins the gateway availability domain; rollback by unbinding | not binding (rejected: "secure all agents through it" is untrue without it); binding without dry-run (rejected) | platform owner; Gemini Enterprise admin | the register as publication enforcement (HLD P6) |
| P58 | Connector allow-list as org policy: first-party Workspace sources only at first; custom MCP data stores blocked; egress FQDNs restricted; no connector holds a write credential; every connector a supplier row | console-only allow-list (rejected: not enforced) | platform owner; IT security | Tier C |
| P59 | No second production app; one throwaway app per spike, deleted at the end; a sandbox-tenant app is decided with decision 29's sandbox tenant | a permanent nonprod app in the production tenant (rejected: a second front door with the same licences and audience) | platform owner | — |
