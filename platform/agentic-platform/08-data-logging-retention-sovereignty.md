...
- Maturity: detailed design, written 2026-09-13 under [01-hld.md](01-hld.md). Nothing is built.
  ...
  R-A evidence class of §10 and the evidence perimeter of §13.2. It answers the HLD brief items
  G52–G57 of [00-objective-review.md](00-objective-review.md) §5 and the register gaps PS-11
  (the Data Access and data-at-rest halves; page 09 owns the secrets and key conventions table),
  PS-12 and PS-15 (evidence integrity; the recovery classes stay in the HLD §10).
...
- What this page is not: the SIEM content (HLD §7.3), the incident process (HLD §7.6), the
  recovery drills (HLD §10), the key conventions table (page 09), the compliance mapping rows
  (the compliance mapping page of HLD §14.3). Where those pages need a number from here, the
  number is in §12 "Claims other pages must match".
- Standing constraints, unchanged and relied on: no domain-wide delegation anywhere (every feed
  on this page is organisation-owned or reads through a consented scope); the language model
  holds no credential and reads no store on this page except through the authorised views its
  agent's page names; Eve's independent copy of Google's audit stream is kept, not merged; no
  store on this page is written by a model.
- Conventions: `Assumption:` marks inferred facts; *tbd* marks values nobody has decided; every
  ...
  on 2026-09-13 against the URL in §13 unless the row says "unverified". Decisions this page
  records are numbered **P35–P45, provisional; renumbered in
  [12-open-decisions.md](12-open-decisions.md)**. Store names in `code` are proposals for the
  factory's variables, not created resources.
...
3. **Retention is decided once, per data class, with a floor the platform sets and a ceiling
   ...
   (Cloud Logging buckets and Cloud Storage retention policies alike, §13), so locking on an
...
the factory refuses a manifest without it. [TIS-05, PS-12]
...
| S2 | `walle_workspace_logs` | `WALLE_PROJECT` | BigQuery, `EU` — **becomes an authorised view over the central copy (S12), P38** | `evidence` | today: the org sink `walle-audit-bq`; after P38: none (a view) | `mo-metrics@`, `eve-verifier@` (decision 47) | agent owner | R3 |
...
| S4 | `eve.*` (`findings`, `verdicts`, `incidents`, `pages`, `seeded_fault_runs`), `eve_quality`, `eve_receipts` (view dataset), `eve_advice`, `eve_mirror` (the `walle_audit` mirror, decision 51) | `EVE_PROJECT` | BigQuery, `EU` | `evidence` | `eve-controller@`, `eve-verifier@`; `eve-advisor@` on `eve_advice` only | per [../eve/03-lld.md](../eve/03-lld.md) §8–§9 | Eve owner | R2 |
...
| S12 | **`platform-evidence-logs`** — the locked central log bucket | `LOGGING_PROJECT` | Cloud Logging bucket, `europe-west1`, CMEK (§7.5), Observability Analytics on, **locked after P37's lock trigger** | `evidence` | the sinks of §3 | log views per agent; the SIEM reader; `platform-security@` | platform owner | R3, R4 |
...
| S16 | Regional secrets `<agent>-<purpose>`; KMS rings and keys | secrets: the agent, controller and core projects; keys: `KMS_PROJECT` (Autokey and engine keys — page 09 P38's fifth core project), ring `eve` in `EVE_PROJECT` (approval), ring `supply-chain` in `CICD_PROJECT` (attestors) | Secret Manager `europe-west1`; Cloud KMS `europe-west1`, **HSM for every key by `cloudkms.allowedProtectionLevels` at the folder (page 09 §2.3)** | `secret` | the runbooks' bootstrap; Autokey | the one attached identity each | agent owner; Eve owner for `eve-approval`; platform owner for `KMS_PROJECT` | page 09 |
...
| S25 | Incident records | SecOps case management or the organisation's ITSM (HLD §7.6) | outside the folder | `evidence` → monthly export to S15 | the incident commander | IT security, the DPO | IT security | R11 |
...
- A sink has **one** destination. "Locked bucket and BigQuery dataset" is therefore either two
  ...
  project, which then routes the aggregated audit logs to a log bucket"; §13), with a one-hop
...
- Workspace audit logs are **organisation-level** log entries (`logName` under
  ...
  (org-policy and organisation IAM changes — exactly the `SetIamPolicy`-at-organisation rule of
  HLD §7.3).
- An **intercepting** aggregated sink at a folder "prevents log entries from being routed to
  ...
  entries originate", and its destination must be a project (§13). For Data Access logs that is
...
|---|---|---|---|---|---|---|
| `S-org` | organisation; **no** `--include-children` (organisation-level entries only, so no other folder of the organisation is swept into the platform's store) | `logName:"organizations/ORG_ID/logs/"` — all six Workspace streams (Admin, Enterprise Groups, Login, OAuth Token, SAML, Access Transparency) and the organisation's own Cloud Audit Logs | `logging.googleapis.com/projects/LOGGING_PROJECT` | `roles/logging.logWriter` on `LOGGING_PROJECT` for the sink's writer identity | the daily drift job compares the sink's `filter`, `destination` and `disabled` against Terraform; the SIEM rule "any `logging.sinks.*` change at organisation level" (HLD §7.3) | Workspace rows stop arriving → the absence alarm on `eve_workspace_logs` (60 min in business hours, HLD §7.3) fires from Eve's **independent** sink, which is the reason Eve's sink is kept rather than merged; halt reason `log_pipeline_silent` |
...
`roles/logging.viewAccessor` bound on the view (verified, §13). Any principal holding
...
apply to logs data retained longer than the default retention period", §13):
...
| Data Access logs of §4 | `LOGGING_PROJECT` (`Assumption:` ingestion is billed to the project whose bucket stores the entry, so interception moves the whole Data Access bill onto the platform line; verified on the first invoice, P36) | the service list of §4 (no `allServices`), `storage` only at project level on evidence holders, no Firestore `DATA_READ` on Tier R projects (they hold no control plane) |
...
(HLD §7.3). `exemptedMembers` is empty for every service: no agent principal, no robot, no
deployer is exempt. [MON-05, TIS-06, PS-16]

| Service (`auditConfigs.service`) | Log types | Why this service, which operations matter | Volume driver |
|---|---|---|---|
| `secretmanager.googleapis.com` | `DATA_READ`, `DATA_WRITE` | `AccessSecretVersion` is `DATA_READ`; without it the HLD §8.3 alert "`versions.access` by any identity not attached as the secret's declared reader" has no source | one entry per token refresh — low |
...
| `discoveryengine.googleapis.com` | `DATA_READ`, `DATA_WRITE` | `StreamAssist`, `AnswerQuery`, `GetAgentCard` — **the human's actual requests**, off unless enabled (verified in §13); the source of the human `sub` in the correlation contract (HLD §7.4). Inherited into `fld-gemini-enterprise` from the platform folder, replacing the HLD §3.1 per-folder line | one per assistant turn — moderate |
...
metadata; the subscriptions are IAM-bound and drift-checked); `agentregistry`, `agentgateway` and Agent Runtime — **not on the audit-log services list read on
2026-09-13** (§13; `agentidentity.googleapis.com` **is** on it and has its own row above), so no
Data Access family can be enabled for them; their Admin Activity entries are on by default and
are what the registry-write alert of
[../wall-e/13-agent-interconnection.md](../wall-e/13-agent-interconnection.md) §11 reads.

### 4.2 Owner, verification, failure

| Control | Owner | Resource | Verified by | When it fails |
|---|---|---|---|---|
| DL-4.1 folder `auditConfigs` as in §4.1, no exemptions | platform owner | IAM policy of `fld-agentic-platform` (+ the four project-level `storage` configs) | the drift job (HLD §4.7) reads `folders.getIamPolicy` daily and compares the `auditConfigs` block byte for byte with Terraform; a Security Health Analytics custom module flags any project in the folder whose effective config for the nine services is narrower than the folder's (should be impossible by union semantics; the module proves the semantics still hold) | a diff is a severity-2 finding; a removal at folder level is the SIEM's severity-1 `auditConfigs` rule; **an agent cannot cause this failure** — only a folder or organisation policy admin can, and both are behind PAM |
...
| DL-4.3 the Data Access canary (§4.3) | platform owner | Cloud Run job `platform-logging-canary@CORE_PROJECT` | its own absence alert | absence → severity 1 to the detection desk; halt reason `log_pipeline_silent` for Tier P agents (HLD §7.3) |
...
document, then queries `platform-evidence-logs` through the `security` view for the three
entries with its own `insertId`-independent marker (the canary secret's version label carries
a nonce). All three present within 15 minutes → a heartbeat row in `platform_logs.canary`; any
missing → alert. This is the platform-level twin of Eve's evidence heartbeat (HLD §13.2) and
...
- **Floor.** For class `evidence`: **400 days**, the number every set already uses
  ...
  "at least six months" (verified, §13; they apply from 2027-12-02 for Annex III systems, but
...
- **The lock trigger (P37).** Every store marked "locked" in §5.2 is created **unlocked** with
  ...
  cannot be removed, and the retention period cannot be reduced", §13). Between creation and
  ...
  (HLD §7.3 already names "changes to … locked buckets"). Eve's bucket is the exception that
  is already designed locked at 400 days from Phase 9 of its runbook
  ([../eve/07-build-runbook.md](../eve/07-build-runbook.md)); its lock is timed to the same
  trigger by this page, and the runbook's Phase 9 gains the sentence (§12).
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| R1 | S1 `<agent>_audit` — **designated with the frozen plan as the Art. 12 log** (HLD §14.1) | BigQuery `EU` | 400 (table expiry; partition expiry on partitioned tables) | no (BigQuery cannot lock) — **export locked** (R2) | insert-only writer role; no `dataEditor`/`dataOwner` outside PAM; daily JSONL export to the verifier's locked bucket with a manifest; BigQuery time travel 7 days (detection + recoverable) | yes — ids, surrogates, `params_redacted` | Art. 12/19/26(6); TISAX 5.2.4 | verifier, Mo, custodian (dataset-level) | agent owner | export manifest row counts = partition counts, daily; the drift job's dataset IAM check | count mismatch → severity 2 finding; a deleted table is recoverable from the export and Eve's mirror; the deletion itself is the SIEM's BigQuery Admin Activity rule |
...
| R4 | S11 organisation and project `_Required` (Admin Activity, System Event, Access Transparency); S18 Gemini Data Access entries; S23 SCC findings; PAM grants; S27 | Google-fixed for `_Required`; `europe-west1` for the copies | 400 (`_Required`, Google-fixed, not configurable — verified); 400 in S12 for the rest | Google's | Google's for `_Required`; S12's lock for the copy | low to moderate | TISAX 5.2.4 | as S12 | platform owner | — | the copy in S12 outlives a project deletion; `_Required` does not (a deleted project's logs go with it) — which is why S12 exists |
| R5 | S13 `platform-identity-logs` — Login, OAuth-token Data Access, SAML entries of **every** tenant account | `europe-west1` | **400, `Assumption:`** — this is the row where the DPO's ceiling most plausibly differs from the floor, because the store is every employee's sign-in history, not the robots' | **yes**, on the trigger, at the DPO's number | Bucket lock | yes — every user | TISAX 5.2.4 (privileged-account monitoring: the robots' logins are severity 1); Art. 6(1)(f) balancing, `Assumption:`, DPO | `identity` view only | platform owner; the DPO for the number | as R3 | as R3 |
| R6 | S8 `<agent>-content-logs` + S9 `content_raw` | `europe-west1` / `EU` | **30** (ceiling) | no | none — short-lived by design; restricted fields + one view (enforcement on readers) | yes — names, addresses, memberships, the operator's address, full prompt text | minimisation, documented; the S0→S1 decision on `EVENT_ONLY` vs `NO_CONTENT` ([../wall-e/11-prompt-security.md](../wall-e/11-prompt-security.md) §6) | generated view: `<agent>-operators@`, `platform-security@`; `content_raw`: the de-identify job only | agent owner | drift job: bucket `retentionDays=30`, restricted fields present, no project-level logs reader | a longer retention is a drift finding and a DPIA line |
...
| R8 | S17 Gemini Enterprise conversation history | tenant app in `eu` | **30**, `Assumption:` — Google's selectable values are 1, 30, 60, 90, 120 or 180 days, default 60, deleted automatically by creation date (verified, §13). 30 is chosen because the audit of what the operator asked is in S1 and S18, not in the chat, and the chat is tenant content about named employees. **[03-gemini-enterprise-environment.md](03-gemini-enterprise-environment.md) §5.4 (its P39) proposes 90 as its `Assumption:`** — the two pages disagree on the interim number and say so; both leave the choice to the DPO under HLD P13 before Stage 1, and until then the runbook step GE-6 of page 03 sets whatever number the reconcile pass records (§12 item 12). **Configuring the assistant "requires the Gemini Enterprise Plus edition"** (verified); the tenant's edition is *tbd*, so on another edition the default 60 stands and is recorded as the number | Google's | Google's | yes | minimisation | the signed-in user; the Gemini Enterprise admin | Gemini Enterprise admin | the drift job cannot read this setting (no API surface verified) — **quarterly manual check**, recorded | drift → the admin corrects it; no evidence depends on it |
| R9 | S10 Cloud Trace (`_Trace`); `_Default` project logs; metrics; S22 build logs; S26 billing export | `europe-west1` | 30 (Cloud Trace: 30 days, Google-fixed, verified) | no | none | low with span content off — **`ADK_CAPTURE_MESSAGE_CONTENT_IN_SPANS=false` is the control**, drift-checked | — | builders through PAM | agent owner; platform owner | drift job on the span knob | a `true` on the knob is a severity-2 finding and a DPIA line (traces would then hold content 30 days outside any control) |
| R10 | S7 Firestore; S22 Terraform state | `europe-west1` | life of the agent; PITR 7 days; daily backups kept 35 days (`Assumption:`) | delete protection (not a lock) | PITR + scheduled backups + delete protection (HLD §10 R-B); state bucket versioning + retention policy + second-region copy (R-D) | ids | — | the action service; the verifier | agent owner | the restore drill (HLD §10) | restore boots at `halt_all` |
| R11 | S15 `platform-records`; S21 git decision records; S25 incident records' monthly export | `europe-west1`; git | **10 years** | **yes** | Bucket Lock; branch protection | no | Art. 18; TISAX 1.6; Art. 73 | everyone with the wiki; the assessor | platform owner | drift job | — |
| R12 | Audit Manager reports; tabletop and drill records; the evidence register's artefacts | `platform-evidence` (400) or `platform-records` (10 years) per the evidence register's row | as the row | yes | Bucket Lock | no | HLD §14.3 | as R11 | platform owner | — | — |
| R13 | S19 `walle_metrics` (partitioned on `as_of`), `walle_metrics_archive` (daily snapshots with `expiration_timestamp`), S20 `walle_metrics_private` | `EU` | `walle_metrics`: 400 (matches the floor, [../mo/01-hld.md](../mo/01-hld.md)); archive snapshots: 400; `walle_metrics_private`: life of the agent, then deleted at teardown | no | dataset IAM; MD-13 | surrogates; the mapping | — | per Mo's set | Mo owner | Mo's verify steps | — |
| R14 | S23 SDP data profiles | `EU` | 400 | no | dataset IAM | no (metadata about tables) | — | platform owner, IT security | platform owner | — | — |
| R15 | S24 SIEM | per P10 | **≥ 400 days** — SecOps's default is 12 months (365), extendable to 60 months (HLD §7.1); the order must raise it, or the SIEM is a 365-day window over a 400-day evidence horizon and the Art. 26(6) argument rests on S12 alone (it does anyway; the SIEM is the desk's copy) | the SIEM's | the SIEM's | yes | TISAX 5.2.4 | the detection desk | IT security | the order form | — |
...
both halves; §13). No sink in the Wall-E set routes `login.googleapis.com`, and the robot's
interactive login is a severity-1 event (HLD §7.3). The `S-org` sink carries them into
...
| Who | **`eve-export@EVE_PROJECT`** for Wall-E and Eve — the identity HLD §13.2 already defines for the witness push, extended by one read (dataset-level `READER` on `walle_audit`, made by Wall-E's runbook — a new row for [../project-topology.md](../project-topology.md) §3, §12) and one write (`roles/storage.objectCreator` on Eve's bucket under an IAM condition `resource.name.startsWith(".../objects/exports/")`, create-only, the same form as topology row 16). For the platform verifier: `platform-export@VALIDATOR_PROJECT`, same shape. The exporter holds no delete and no overwrite anywhere; the lock makes that redundant, which is the point |
...
by the detection desk's identity (P10) with the same manifest form; Audit Manager reports go to
...
profile Cloud Logging buckets or Firestore (verified, §13). Two scan configurations, both at
`fld-agentic-platform`, both in the factory's `platform-core` module: [PS-12]
...
### 7.1 Assured Workloads EU Data Boundary for the agents folder: no, not now (P41)
...
will ask for. [PS-13, MON-15]

| Verified on 2026-09-13 (§13) | Consequence |
|---|---|
...
**Decision P41 — No.** The agents folder is not placed in the EU Data Boundary package while
...
`gcp.restrictTLSVersion`, added by this page to the baseline — §12); the package's violation
...
| DL-7.5 Access Approval | prerequisite: Access Transparency enabled (verified). Enabled on: `fld-agents-p` (HLD §3.1 row), `fld-controllers`, `EVE_WITNESS_PROJECT` (on the witness side), and — this page's addition — **`LOGGING_PROJECT` and `CORE_PROJECT`**, the evidence holders. Approvers: `platform-security@` with the platform owner as second; notifications by Pub/Sub to the SIEM and by email to the security reviewer; a request auto-approved "to address time-sensitive outages" is logged `auto approved` and is a SIEM rule at severity 2. Coverage (verified): BigQuery, Cloud Storage, Cloud Logging, Cloud KMS, Secret Manager, Cloud Run, Firestore, Pub/Sub, Model Armor; **not** Agent Gateway, Agent Registry, Agent Runtime — Google-personnel access to an engine is transparent (Access Transparency) but not gated, recorded as a limit in the supplier file. Signing key: Google-managed at Stage 0 (`Assumption:`; an own KMS key is the option if the ISMS asks) | IT security; the platform owner as second approver | the folders and projects above | the drift job reads the Access Approval settings resource per folder and project daily | "the support response time increases by the duration that Customer Care spends waiting for your approval" (verified) — accepted, and the on-call rota of HLD §7.6 is the approver rota; a disabled setting is a severity-1 SIEM rule (an organisation administrator disabling it is exactly the super-admin scenario) | enforcement for the listed services |
| DL-7.6 the witness is EU too | `org-witness` (HLD §13.2) applies the same `gcp.resourceLocations` on its own organisation; its bucket is `europe-west1` and its dataset `EU`; Access Approval and Access Transparency on the witness project | Eve owner (witness administrators) | the witness organisation's policy | the witness's own drift check | — | enforcement |
...
cryptography position; 7.1.2 asks for the data-protection artefacts — §2, §5, §7.5 and the
...
groups, one person" (TIS-05, topology decision 52). Owners are **roles**; the ISMS supplies
...
| `KMS_PROJECT` (page 09 P38: the fifth core project, keys and nothing else) | platform owner | platform owner (class `secret`) | platform owner through PAM; Autokey | security reviewer | quarterly |
...
Page 09 owns the key conventions table (key, algorithm, location, rotation, owner, log) and the
secrets conventions. This page decides only what its own stores need and hands page 09 the rows
(§12). HLD §8.3 is the frame: Cloud KMS Autokey at the folder for the services on its compatible
list (BigQuery, Cloud Storage, Cloud Run, Secret Manager, Pub/Sub, Artifact Registry); Firestore
the one exception (not on the list, Google-managed encryption); Agent Runtime **not** an
exception to CMEK — it takes CMEK through `encryption_spec` with a single-region key the factory
creates because Autokey does not cover it; approval-authority keys at HSM level in
`EVE_PROJECT`. Page 09 then goes further, and this page follows it: **every key on the platform
is HSM by `constraints/cloudkms.allowedProtectionLevels = HSM` at the folder (page 09 P38)**, and
Autokey's keys land in a dedicated **`KMS_PROJECT`** under `fld-platform-core`, not in the
resource's project. One row below **contradicts page 09's P39** ("Cloud Logging buckets stay
Google-managed — an organisation-level setting outside the platform's remit"): Google's page,
re-read on 2026-09-13, says CMEK is configured **per individual log bucket** at creation
("enable CMEK for an individual log bucket"), separate from the organisation-level Log Router
CMEK; page 09's reason therefore does not hold for the central buckets, and this page keeps
CMEK on them. The reconcile pass resolves it (§12 item 2). [PS-11, TIS-13]

| Store | Stance (P43) | Why | Consequence the reviewer must accept |
|---|---|---|---|
| S12, S13 central log buckets | **CMEK with a factory-created key** `platform-logs-europe-west1` in `KMS_PROJECT`, ring `logging`, `europe-west1`, HSM (required by the folder constraint, page 09 §2.3) | Cloud Logging is **not** on Autokey's compatible list (re-verified 2026-09-13, §13), so a CMEK bucket needs an explicit key; per-bucket CMEK is a documented mechanism ("enable CMEK for an individual log bucket"); "you must use a key whose region matches the regional scope of your data"; the bucket's `kmsServiceAccountId` needs Encrypter/Decrypter; **CMEK cannot be added to or removed from a bucket after creation — the key can be changed** (verified) — so the decision is taken here, before Stage 0 creates the bucket | a disabled or destroyed key makes the evidence **unreadable and, after a three-hour buffer, unwritable** ("Logging buffers the most recent three hours of logs"; "all logs are also unavailable for query until key access is repaired" — verified) — a new availability risk with no counterpart on a Google-managed bucket. Mitigations: HLD §10 R-K "disable, never destroy"; the key's `cloudkms` Data Access and Admin Activity logs; a severity-1 SIEM rule on any key state change in the evidence projects; no `cloudkms.admin` outside PAM; and **the second and third copies are not on this key** (Eve's bucket is on Eve's ring; the witness is on Google-managed encryption) |
| S15 `platform-evidence`, `platform-records` | CMEK by Autokey at the folder (Cloud Storage is on the list); the key in `KMS_PROJECT` | HLD §8.3 default; page 09 §2.2 | as above; Autokey keys are HSM |
| S14 `platform_logs` and every BigQuery dataset in §2.2 | CMEK by Autokey at the folder | on the list | a dataset-level default key is set before the sink creates its tables (order matters, as with partition expiry) |
| S5 Eve's bucket, S4 Eve's datasets | **an explicit HSM key `eve-evidence` in ring `eve` of `EVE_PROJECT`** (the ring that already holds `eve-approval`), not an Autokey key — because Autokey's keys land in `KMS_PROJECT`, a platform-owned project, and Eve's copy must survive a platform-line key administrator; a second row for page 09's table (§12) | independence: whoever can disable the platform's keys must not be able to brick Eve's copy; `EVE_PROJECT`'s KMS admin is behind PAM with the Eve owner as approver | Eve's runbook creates one more key; the SHA finding page 09 raises on "a bucket without Autokey CMEK in W+" must accept an explicit key as conforming (§12) |
| S6 witness | **Google-managed encryption, or the witness organisation's own keys — never a key in the tenant's organisation** | the witness exists to survive the tenant's administrators; a tenant key would give them a way to brick it | recorded in the witness runbook (§12) |
| S8 content log buckets | CMEK with a per-agent factory key `<agent>-content-logs` in `KMS_PROJECT`, `europe-west1` (same Cloud Logging rule) — **or Google-managed**, decided per tier by page 09; this page's default: Google-managed for Tier R, CMEK from Tier W (page 09's exception 2 fits these buckets, not the central ones) | a 30-day content store's threat is readers, not key custody; the reader controls of §6.3 carry it | — |
| S7 Firestore | Google-managed (the HLD's recorded exception) | not on Autokey's list | — |
| S16 secrets and approval keys | page 09 | — | — |
| S17 Gemini Enterprise conversation history | Google-managed; CMEK for the app is *tbd* (not verified on this pass) | — | recorded as unverified |
| Key Access Justifications | **not available** (requires Assured Workloads, §7.1) | — | page 09 records "no KAJ" |

The one-line cryptography position for the compliance mapping (5.1): every platform store is
encrypted at rest with Google-managed keys at minimum; evidence and control stores from Tier W
use customer-managed HSM-backed keys held in the project that owns the store; no store's key is
held by a principal the store is meant to survive.

---

## 10. Decisions recorded on this page (provisional; renumbered in [12-open-decisions.md](12-open-decisions.md))

| Id | Decision | Options considered | Owner | Gate it blocks |
|---|---|---|---|---|
| **P35** | Central logging topology: `S-org` (organisation, no children, org-level entries) and `S-folder` (folder, children, **intercepting**, five audit families) both into `LOGGING_PROJECT` as a project destination; fan-out sinks to the locked evidence bucket, the identity bucket, `platform_logs` and per-agent trigger topics; per-agent access by log views and authorised views; Wall-E's two organisation sinks deleted and re-homed; Eve's organisation sink kept | one sink per destination at organisation level (rejected: N destinations, N `configWriter` grants); no interception (rejected: N+1 billed, shortenable copies); interception of all families (rejected: would pull content logs out of the agent's restricted bucket) | platform owner | Tier R (the factory's `platform-core` module) |
| **P36** | Data Access audit configuration at the folder for the services of §4.1, no exemptions, `storage` at project level on the four evidence holders, `iam`/`orgpolicy` at `ADMIN_READ`; cost on the platform line of `LOGGING_PROJECT`; the daily canary | `allServices` (rejected); per-project configs (rejected: disable-able by the project owner) | platform owner (payer); security reviewer for the list | Tier R |
| **P37** | The retention schedule of §5.2: `evidence` 400 days floor, `record` 10 years, `content` 30 days ceiling, `ops` 30; the DPO's ceiling per class before Stage 1 of any Tier W agent; **locked stores locked on the day the ceiling is recorded**, at max(floor, ceiling), never before; the identity streams in their own bucket with their own line | lock at creation on the 400-day assumption (rejected: irreversible in the one direction the DPO can move); no lock (rejected: the super-admin case needs a preventive copy) | platform owner; the DPO for the ceilings | Stage 1 of any Tier W agent (the lock); Stage 0 (the buckets) |
| **P38** | The daily JSONL export with manifests by `eve-export@` into the verifier's locked bucket (`exports/`), then the witness; `walle_audit` plus the frozen plan designated the Art. 12 log; decision 31 and topology decision 51 closed as §5.4 says; `walle_workspace_logs` becomes an authorised view in `LOGGING_PROJECT` | `eve-verifier@` as exporter (rejected: it runs the reconciler; one identity per duty); a separate mirror project (rejected by decision 51) | Eve owner; Wall-E owner for the `walle_audit` grant | Stage 1 |
| **P39** | Sensitive Data Protection discovery at the folder (BigQuery and Cloud Storage), service-agent container `LOGGING_PROJECT`, `secret`-class stores excluded by register label, publish to SCC/BigQuery/Pub/Sub, the declared-versus-discovered reconcile job; one de-identify template `platform-log-deid-v1` (deterministic encryption for identifiers, redaction for credentials) applied daily to `content_raw` → `content_deid` and referenced by the tier Model Armor templates; content buckets with restricted fields, one generated view, `viewAccessor` + `fieldAccessor` for two groups, no linked dataset | a de-identify pass on the log bucket (impossible); SDP as a Model Armor filter only (the status quo, rejected) | platform owner (template); agent owner (bucket, job) | Tier R (discovery); Tier W (de-identify job) |
| **P40** | Five platform data classes (§2.1) as the `data_class` vocabulary; every store classified (§2.2); `walle_metrics_private` class `secret`; asset, data, custodian and reviewer roles per project (§8); the mapping to the organisation's scheme *tbd* by the ISMS | reuse the organisation's scheme directly (blocked: it is *tbd*) | platform owner; ISMS for the mapping | Tier R; the TISAX assessment order for the mapping |
| **P41** | **Assured Workloads EU Data Boundary: no, not now**, for the verified reasons of §7.1; the compensating set of §7.2; adoption later by a new folder and a move analysis; the gap raised with Google in P32's request | yes with three permanent acknowledged violations (rejected: a paper control); yes without the gateway and registry (rejected: they are the design) | platform owner; security reviewer | — (revisit per HLD P12) |
| **P42** | Access Transparency verified on and routed to S12 and the SIEM; Access Approval on `fld-agents-p`, `fld-controllers`, the witness, **plus `LOGGING_PROJECT` and `CORE_PROJECT`**, approvers `platform-security@` + platform owner, Pub/Sub to the SIEM, auto-approvals as a severity-2 rule, Google-managed signing key at Stage 0 | Access Approval organisation-wide (rejected: it slows every support case for projects outside the platform; IT security may still choose it) | IT security | Tier P (per the HLD); the evidence projects at Tier R |
| **P43** | Data-at-rest stance for this page's stores (§9): CMEK with an explicit HSM key in `KMS_PROJECT` on the central log buckets because Autokey does not cover Cloud Logging and add/remove is permanent at creation; Autokey elsewhere per HLD §8.3 and page 09; Eve's copy on an explicit key in `EVE_PROJECT`'s ring `eve`; **the witness never on a tenant key**; no KAJ; the cryptography position sentence. **Contradicts page 09 P39's exception 2 for the central buckets** — resolved at reconcile (§12 item 2) | Google-managed on the central log buckets — page 09's current position (rejected here for the evidence class at the Confidential label, because per-bucket CMEK is available and the choice cannot be made later) | platform owner; page 09 for the table | Stage 0 (the buckets are created once) |
| **P44** | Gemini Enterprise conversation history at 30 days (`Assumption:`; 60 stands where the edition does not expose the setting); the EU-DRZ model-pin rule as a CI check against a committed allow-list | 1 day (rejected: operators lose the thread within a working day); default 60 (rejected: twice the content ceiling) | Gemini Enterprise admin; platform owner for the allow-list | Stage 1 |
| **P45** | Log views per agent capped by Google's 30-per-bucket limit → one evidence bucket per tier folder once a folder passes ~25 agents, handled by a factory tier variable; the security, SIEM and identity views are per bucket | one bucket forever (rejected by the limit); a bucket per agent (rejected: N locks, N keys) | platform owner | the 26th agent in a tier |

Decisions in the agent sets this page touches, each getting a dated line pointing here: Wall-E
17 (retention floor and ceiling → P37), 25 (data-protection question → §5.1's records-of-
processing inputs), 31 (closed by P38); Eve E-14 (→ P37, with the lock-trigger sentence in
Phase 9); Mo M-5 (→ P37, P38); topology 47 (the data-level check now reads the authorised view
of P38), 51 (closed by P38), 52 (owners per project → §8); HLD P13 (this page proposes the
numbers; the DPO decides).
...
none yet. The candidates it names, to be opened only with a decision file: Firestore
...
| Which project is billed for an entry intercepted at the folder and stored only in `LOGGING_PROJECT` | §3.4 | `Assumption:` the storing project; verified on the first invoice (P36) |
| Terraform provider support for `--intercept-children` on folder sinks | §3.2 | a provider issue requesting it exists (§13); if unsupported at build, the two aggregated sinks are created by `gcloud` in the same pipeline, drift-checked like everything else |
...
| An API surface to read the Gemini Enterprise chat-retention setting and the Access Transparency organisation setting from the drift job | §5.2 R8, §7.2 DL-7.4 | none found on this pass; quarterly manual checks are written down instead |
...
| Whether the EU Data Boundary's Vertex AI row covers Agent Runtime | §7.1 | not named on the list as read; irrelevant to P41 (the gateway and registry are absent regardless) but relevant to a later adoption's move analysis |
...
| Access Transparency as a prerequisite of Access Approval, and Access Approval enrolment at folder level | §7.2 DL-7.5 | stated on the Access Approval overview page as read by the previous pass; the supported-services page re-read on 2026-09-13 does not repeat it — kept, not re-verified |
| CMEK for the Gemini Enterprise app's own stores | §9 | not verified |
...
## 12. Claims other pages must match (for the reconcile pass)

1. **[01-hld.md](01-hld.md) §7.1** "one organisation-level aggregated sink" → two aggregated sinks (organisation without children; folder with children, intercepting) into `LOGGING_PROJECT` as a project destination, then fan-out; the substance is unchanged, the count is not. §7.1's `discoveryengine` Data Access line for `fld-gemini-enterprise` is inherited from the platform folder (§4.1). §3.3 gains `gcp.restrictTLSVersion` (deny 1.0/1.1) as a baseline line (§7.1). §8.2 "Access Approval on Tier P, controllers and the witness" gains `LOGGING_PROJECT` and `CORE_PROJECT` (P42). §7.5's table is superseded by §5.2 here (same numbers; the identity streams get their own store and line; `content_deid` added; the lock trigger added).
2. **[09-supply-chain-secrets-recovery.md](09-supply-chain-secrets-recovery.md) §2.2–§2.4 (the key table)** — **one contradiction and three additions.** Contradiction: page 09 P39 records "Cloud Logging buckets stay Google-managed" as exception 2 with the reason "an organisation-level setting outside the platform's remit"; Google's CMEK-for-log-storage page (§13) documents **per-bucket** CMEK at creation, so the reason does not hold and this page's P43 keeps CMEK on `platform-evidence-logs` and `platform-identity-logs` with an explicit HSM key in `KMS_PROJECT` (ring `logging`) — page 09's exception 2 should narrow to the `_Default`/`_Required` buckets and the Tier R content buckets. Additions to its table: class C row `platform-logs-europe-west1` (`KMS_PROJECT`, ring `logging`, `europe-west1`, HSM, rotation 90 days, Logging's `kmsServiceAccountId` as the only user); class C row `eve-evidence` in `EVE_PROJECT` ring `eve` (explicit, not Autokey, for independence — its SK-7 finding "bucket without CMEK in W+" must accept an explicit key); per-agent `<agent>-content-logs` keys from Tier W; "no KAJ" and the cryptography position sentence of §9. The `secretmanager`/`cloudkms`/`agentidentity` Data Access logs it needs are §4.1's rows. This page adopts page 09's `KMS_PROJECT` and HSM-everywhere (P38) throughout §2.2, §8 and §9.
3. **The gateways / Model Armor page**: the tier Model Armor templates reference `platform-log-deid-v1` and `platform-log-inspect-v1` in `CORE_PROJECT`, `europe-west1`, for advanced SDP (§6.3); the sanitize logs stay in the agent's content bucket (not intercepted).
4. **The monitoring / incident pages**: the SIEM reads the `siem` log view of `platform-evidence-logs` (and the `identity` view only if IT security asks); SIEM retention ≥ 400 days on the order; new severity rules: bucket/retention/key-state changes in the three evidence projects (sev 1), Access Approval disabled (sev 1), auto-approved Access Approval requests (sev 2), classification findings (sev 3/2), canary absence (sev 1 for Tier P); the monthly alert/incident export paths of §5.5; the canary job in `CORE_PROJECT`.
5. **[../project-topology.md](../project-topology.md) §3**: new rows — `eve-export@EVE_PROJECT` → dataset-level `READER` on `walle_audit` in `WALLE_PROJECT` (made by Wall-E's runbook); `eve-export@` → `roles/storage.objectCreator` on `exports/` of Eve's bucket (Eve's runbook, conditional binding as row 16); the sink writer identities of `LOGGING_PROJECT` → `roles/pubsub.publisher` on `<agent>-triggers` (replaces row 23); `mo-metrics@` and `eve-verifier@` → dataset-level `READER` on `platform_logs_views` in `LOGGING_PROJECT` (replaces the `walle_workspace_logs` half of rows 5 and 6); the SDP discovery service agent's reads across the folder as a named foreign principal in Eve's drift job's expected set; §1.3 teardown text: the organisation sinks `walle-workspace-audit` and `walle-audit-bq` no longer exist.
6. **[../wall-e/SETUP.md](../wall-e/SETUP.md) Phase 11 and [../wall-e/PREREQUISITES.md](../wall-e/PREREQUISITES.md) §4.1, §10 item 21**: no organisation-level `logging.configWriter` in any agent runbook; `walle_workspace_logs` is a view; item 21 closed by the factory-generated view, restricted fields and field role (§6.3); Phase 12c keeps the bucket and sink, gains the `content_raw` sink.
7. **[../wall-e/11-prompt-security.md](../wall-e/11-prompt-security.md) §4 and §6**: the advanced SDP de-identify *tbd* is decided (P39); `content_raw`/`content_deid` are the sink and the de-identified copy; no linked dataset on the content bucket; `EVENT_ONLY` retention 30 days confirmed as the ceiling.
8. **[../wall-e/ARCHITECTURE.md](../wall-e/ARCHITECTURE.md) §9**: every *tbd* retention row takes its number from §5.2; the "two known residency exceptions" become the six rows of DL-7.2; the model-pin sentence becomes the CI rule DL-7.3.
9. **[../eve/07-build-runbook.md](../eve/07-build-runbook.md) Phase 7 and Phase 9**: the sink filter widened to the six streams (HLD §13.2) — unchanged by this page; Phase 9's `--lock-retention-period` moves to the P37 trigger (after the DPO's ceiling is recorded, before Stage 1), with the interim `UpdateBucket` alert; the witness runbook states Google-managed encryption (P43); `eve-export@` gains the `exports/` duty (§5.4).
10. **[../eve/03-lld.md](../eve/03-lld.md) §8–§9**: `eve.*` 400-day partition expiry confirmed; the export of `findings`, `verdicts`, `incidents`, `pages` daily is P38; `eve_workspace_logs` stays the reconciliation source.
11. **[../mo/01-hld.md](../mo/01-hld.md), [../mo/02-identity-and-access.md](../mo/02-identity-and-access.md)**: `walle_metrics_private` is class `secret` and excluded from SDP discovery; `mo-metrics@`'s `walle_workspace_logs` read becomes a read on `platform_logs_views`; `content_deid` is the store Mo's graders read for content, never `content_raw`.
12. **[03-gemini-enterprise-environment.md](03-gemini-enterprise-environment.md) §5.4, §13, §14 and its P39; [../gemini-enterprise.md](../gemini-enterprise.md)**: the app in `eu`; `discoveryengine` Data Access at the folder — the same decision on both pages; the `ge-requests` log view is in §3.3 here; the EU-DRZ model rule (DL-7.3) and the edition question. **Disagreement to settle: chat retention — page 03 proposes 90 days, this page 30 (P44); both are `Assumption:` under HLD P13 and the DPO decides; the reconcile pass picks one interim number for runbook step GE-6 and the two pages cite it.**
13. **The compliance mapping page** (HLD §14.3): rows 1.3.1/1.3.2 from §2.2 and §8; 5.1 from §9's position sentence; 5.2.4 from §3–§5; 7.1.2 inputs from §5.1; the evidence register's paths from §5.5; the Art. 12 designation of R1.
14. **[12-open-decisions.md](12-open-decisions.md)**: P35–P45 renumbered; agent-set decisions 17, 25, 31, E-14, M-5, topology 47, 51, 52 and HLD P13 dated to this page.
15. **[05-registry-and-autonomy-contract.md](05-registry-and-autonomy-contract.md) §3 manifest fields**: page 05's manifest carries `data_classes[]` (a list, values such as `internal`, `personal`) and `tisax_class`; this page's §2.1 defines a per-**store** `data_class` with five values. Proposed reconciliation: the manifest's `data_classes[]` lists every class of §2.1 the agent's stores use; the project label `data_class` (page 05 §2) carries the highest; `tisax_class` stays the organisation-scheme mapping column of §2.1; the factory reads the per-store class from a `stores:` block the manifest gains (name, kind, class, retention row) — page 05's schema owns the field.
16. **[02-landing-zone-and-tiers.md](02-landing-zone-and-tiers.md) §3 (topology rows 22–23 retired) and §8**: "one sink with `includeChildren`" becomes the two aggregated sinks of §3.2; `KMS_PROJECT` joins the four core projects (page 09 P38) in its folder table and `restrictServiceUsage` allow-list.
17. **[07-monitoring-detection-incident-response.md](07-monitoring-detection-incident-response.md) F2, H-2, SG-01/SG-04**: F2 reads the `siem` view of `platform-evidence-logs` (and `identity` only on request); H-2 and DL-4.3 are two canaries, not one; the SIEM rules this page adds are item 4; `KMS_PROJECT` joins the project list of SG-01 and SG-04.
...
- https://docs.cloud.google.com/logging/docs/audit/services — service names used in §4.1; `agentidentity.googleapis.com`, `discoveryengine.googleapis.com`, `modelarmor.googleapis.com`, `dlp.googleapis.com` on the list; Agent Registry, Agent Gateway and Agent Runtime not on the list (re-read 2026-09-13)
...
- https://docs.cloud.google.com/resource-manager/docs/organization-policy/defining-locations-supported-services — per-service enforcement rows for Cloud Logging, BigQuery, Cloud Storage, Cloud KMS, Secret Manager, Sensitive Data Protection, Agent Registry ("regional resources"), Agent Identity, Gemini Enterprise Agent Platform ("all … resources"), Cloud Run, Firestore, Pub/Sub; Agent Gateway and Agent Runtime not named (re-read 2026-09-13)
...
- [09-supply-chain-secrets-recovery.md](09-supply-chain-secrets-recovery.md) — owns the key table this page's §9 feeds; `KMS_PROJECT` and HSM-everywhere (its P38); its P39 exception 2 conflicts with P43 here
