...
| S16 | Regional secrets `<agent>-<purpose>`; KMS rings and keys | agent, controller and core projects | Secret Manager `europe-west1`; Cloud KMS `europe-west1` (HSM for approval keys) | `secret` | the runbooks' bootstrap | the one attached identity each | agent owner; Eve owner for `eve-approval` | page 09 |
...
metadata; the subscriptions are IAM-bound and drift-checked); `agentregistry`, `agentgateway`,
`agentidentity` — not on the audit-log services list read on 2026-09-13 (§13), so nothing can be
enabled; their Admin Activity entries are on by default and are what the registry-write alert
of [../wall-e/13-agent-interconnection.md](../wall-e/13-agent-interconnection.md) §11 reads.
...
gives "Data Access logs are on" a *last fired* column instead of a checkbox.
...
|---|---|---|---|---|---|---|
| DL-7.1 residency by policy | `gcp.resourceLocations = in:eu-locations` at `fld-agentic-platform` (HLD §3.3). The constraint is enforced at creation for Cloud Logging buckets, BigQuery datasets, Cloud Storage buckets, Cloud KMS, Secret Manager, Sensitive Data Protection resources, Agent Registry regional resources, Cloud Run, Pub/Sub message storage (verified per service, §13). **Agent Gateway is absent from the supported-services list, and Agent Runtime and Discovery Engine were not found on it on 2026-09-13** — for those three, residency rests on the region chosen at creation (`europe-west1`; the app in `eu`) and on the drift job reading the resource's location daily | platform owner | the folder policy | drift job; SHA custom module on any resource in the folder whose `location` is outside the EU set | a resource outside the EU set is created only where the constraint does not reach; the drift job reports it as severity 2 and the factory's next run deletes a non-conforming resource it created | enforcement where listed; **detection** for Gateway, Runtime, Discovery Engine |
...
`EVE_PROJECT`. [PS-11, TIS-13]

| Store | Stance (P43) | Why | Consequence the reviewer must accept |
|---|---|---|---|
| S12, S13 central log buckets | **CMEK with a factory-created key** `platform-logs-europe-west1` in `LOGGING_PROJECT`'s ring, `europe-west1`, HSM protection level (`Assumption:` HSM for consistency with the evidence class; software keys are the cheaper alternative page 09 may choose) | Cloud Logging is **not** on Autokey's compatible list (HLD §19 source), so a CMEK bucket needs an explicit key; "the location of the log bucket must match the location of the key"; the Logging KMS service account needs Encrypter/Decrypter; **CMEK cannot be added, changed or removed after the bucket is created** (verified) — so the decision is taken here, before Stage 0 creates the bucket | a disabled or destroyed key makes the evidence **unreadable and unwritable** — a new availability risk with no counterpart on a Google-managed bucket. Mitigations: HLD §10 R-K "disable, never destroy"; the key's `cloudkms` Data Access and Admin Activity logs; a severity-1 SIEM rule on any key state change in the evidence projects; no `cloudkms.admin` outside PAM; and **the second and third copies are not on this key** (Eve's bucket is on Eve's ring; the witness is on Google-managed encryption) |
| S15 `platform-evidence`, `platform-records` | CMEK by Autokey at the folder (Cloud Storage is on the list) | HLD §8.3 default | as above; Autokey keys are HSM-backed |
...
| S5 Eve's bucket, S4 Eve's datasets | Autokey under Eve's own folder configuration (`fld-controllers`), keys in `EVE_PROJECT` — never a `LOGGING_PROJECT` key | independence: a Wall-E-line administrator who disables the platform key must not touch Eve's copy | — |
...
| S8 content log buckets | CMEK with a per-agent factory key in the agent project, `europe-west1` (same Cloud Logging rule) — **or Google-managed**, decided per tier by page 09; this page's default: Google-managed for Tier R, CMEK from Tier W | a 30-day content store's threat is readers, not key custody; the reader controls of §6.3 carry it | — |
...
| **P43** | Data-at-rest stance for this page's stores (§9): CMEK with an explicit HSM key on the central log buckets because Autokey does not cover Cloud Logging and the choice is permanent at creation; Autokey elsewhere per HLD §8.3; Eve's copy on Eve's keys; **the witness never on a tenant key**; no KAJ; the cryptography position sentence | Google-managed on the log buckets (rejected for the evidence class at the Confidential label — reopens if page 09 or the ISMS decides otherwise) | platform owner; page 09 for the table | Stage 0 (the buckets are created once) |
...
| Agent Runtime and Discovery Engine on the `gcp.resourceLocations` supported-services list | §7.2 DL-7.1 | not found on the page as read; graded detection for those until re-read |
...
2. **Page 09 (secrets and keys)**: the rows of §9 — `platform-logs-europe-west1` (Cloud Logging is not on Autokey's list; key location must match; permanent at creation), Eve's copy on Eve's ring, the witness never on a tenant key, per-agent content-bucket keys from Tier W, no KAJ, the cryptography position sentence — belong in its table; the `secretmanager`/`cloudkms` Data Access logs it needs are §4.1's rows.
...
12. **[../gemini-enterprise.md](../gemini-enterprise.md)**: the app in `eu`, chat retention 30 days (P44), `discoveryengine` Data Access on by folder inheritance, the EU-DRZ model rule, the edition question.
...
- https://docs.cloud.google.com/logging/docs/routing/managed-encryption-storage — CMEK per bucket or as folder/organisation default; "the location of the log bucket must match the location of the key"; cannot be changed after creation; the Logging KMS service account
...
- https://docs.cloud.google.com/logging/docs/audit/services — service names used in §4.1; Agent Registry, Agent Gateway, Agent Identity not on the list as read
...
- https://docs.cloud.google.com/assured-workloads/docs/control-packages/eu-data-boundary — the supported-products list; Agent Gateway, Agent Registry, Agent Identity absent; `gcp.restrictServiceUsage` allow all supported products; `gcp.resourceLocations`; `gcp.restrictTLSVersion`; `compute.disableGlobalCloudArmorPolicy`; "Google Cloud MCP servers aren't supported"; Cloud Logging affected features; Free tier
...
- https://docs.cloud.google.com/resource-manager/docs/organization-policy/defining-locations-supported-services — per-service enforcement rows for Cloud Logging, BigQuery, Cloud Storage, Cloud KMS, Secret Manager, Sensitive Data Protection, Agent Registry, Cloud Run, Firestore, Pub/Sub; Agent Gateway absent
...
- https://docs.cloud.google.com/kms/docs/autokey-overview (as cited by [01-hld.md](01-hld.md) §19) — compatible services; Firestore and Cloud Logging absent
...
- Page 09, secrets and keys conventions (filename *tbd* at reconcile) — owns the key table this page's §9 feeds
