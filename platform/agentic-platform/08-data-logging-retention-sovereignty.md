...
| S4 | `eve.*` (`findings`, `verdicts`, `incidents`, `pages`, `seeded_fault_runs`), `eve_quality`, `eve_receipts` (view dataset), `eve_advice`, `eve_audit_mirror` (the `walle_audit` mirror, decision 51; renamed 2026-09-13 from `eve_mirror`, which is only the witness dataset of S6) | `EVE_PROJECT` | BigQuery, `EU` | `evidence` | `eve-controller@`, `eve-verifier@`; `eve-advisor@` on `eve_advice` only | per [../eve/03-lld.md](../eve/03-lld.md) §8–§9 | Eve owner | R2 |
