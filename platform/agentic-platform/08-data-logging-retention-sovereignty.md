...
- Conventions: `Assumption:` marks inferred facts; *tbd* marks values nobody has decided; every
  ...
  records are numbered **P104–P114**, final ids in
...
| S12 | **`platform-evidence-logs`** — the locked central log bucket | `LOGGING_PROJECT` | Cloud Logging bucket, `europe-west1`, CMEK (§7.5), Observability Analytics on, **locked after P106's lock trigger** | `evidence` | the sinks of §3 | log views per agent; the SIEM reader; `platform-security@` | platform owner | R3, R4 |
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| R1 | S1 `<agent>_audit` — **designated with the frozen plan as the Art. 12 log** (HLD §14.1) | BigQuery `EU` | 400 (table expiry; partition expiry on partitioned tables) | no (BigQuery cannot lock) — **export locked** (R2) | insert-only writer role; no `dataEditor`/`dataOwner` outside PAM; daily JSONL export to the verifier's locked bucket with a manifest; weekly BigQuery table snapshots into a snapshot dataset in `LOGGING_PROJECT` with a 400-day expiry ([09](09-supply-chain-secrets-recovery.md) §3.3, RC-3; aligned 2026-09-13); BigQuery time travel 7 days (detection + recoverable) | yes — ids, surrogates, `params_redacted` | Art. 12/19/26(6); TISAX 5.2.4 | verifier, Mo, custodian (dataset-level) | agent owner | export manifest row counts = partition counts, daily; the drift job's dataset IAM check | count mismatch → severity 2 finding; a deleted table is recoverable from the export and Eve's mirror; the deletion itself is the SIEM's BigQuery Admin Activity rule |
...
is HSM by `constraints/cloudkms.allowedProtectionLevels = HSM` at the folder (page 09 P118)**, and
Autokey's keys land in a dedicated **`KMS_PROJECT`** under `fld-platform-core`, not in the
resource's project. One row below **contradicts page 09's P119** ("Cloud Logging buckets stay
...
CMEK on them. **Resolved on the reconcile pass of 2026-09-13:** this page's position stands for
the two central buckets; page 09's exception 2 is narrowed to the organisation
`_Default`/`_Required` buckets and the Tier R content buckets, and its key table gained the rows
of §12 item 2. [PS-11, TIS-13]

| Store | Stance (P112) | Why | Consequence the reviewer must accept |
|---|---|---|---|
...
| **P112** | Data-at-rest stance for this page's stores (§9): CMEK with an explicit HSM key in `KMS_PROJECT` on the central log buckets because Autokey does not cover Cloud Logging and add/remove is permanent at creation; Autokey elsewhere per HLD §8.3 and page 09; Eve's copy on an explicit key in `EVE_PROJECT`'s ring `eve`; **the witness never on a tenant key**; no KAJ; the cryptography position sentence. **Contradicts page 09 P119's exception 2 for the central buckets** — resolved at reconcile (§12 item 2) | Google-managed on the central log buckets — page 09's current position (rejected here for the evidence class at the Confidential label, because per-bucket CMEK is available and the choice cannot be made later) | platform owner; page 09 for the table | Stage 0 (the buckets are created once) |
| **P113** | Gemini Enterprise conversation history at 30 days (`Assumption:`; 60 stands where the edition does not expose the setting) — the interim number page 03's P52 was aligned to on 2026-09-13; the EU-DRZ model-pin rule as a CI check against a committed allow-list | 1 day (rejected: operators lose the thread within a working day); default 60 (rejected: twice the content ceiling); 90 (page 03's first proposal) | Gemini Enterprise admin; platform owner for the allow-list | Stage 1 |
| **P114** | Log views per agent capped by Google's 30-per-bucket limit → one evidence bucket per tier folder once a folder passes ~25 agents, handled by a factory tier variable; the security, SIEM and identity views are per bucket | one bucket forever (rejected by the limit); a bucket per agent (rejected: N locks, N keys) | platform owner | the 26th agent in a tier |

Decisions in the agent sets this page touches, each getting a dated line pointing here: Wall-E
17 (retention floor and ceiling → P106), 25 (data-protection question → §5.1's records-of-
processing inputs), 31 (closed by P107); Eve E-14 (→ P106, with the lock-trigger sentence in
Phase 9); Mo M-5 (→ P106, P107); topology 47 (the data-level check now reads the authorised view
of P107), 51 (closed by P107), 52 (owners per project → §8); HLD P13 (this page proposes the
...
7. **[../wall-e/11-prompt-security.md](../wall-e/11-prompt-security.md) §4 and §6**: the advanced SDP de-identify *tbd* is decided (P108); `content_raw`/`content_deid` are the sink and the de-identified copy; no linked dataset on the content bucket; `EVENT_ONLY` retention 30 days confirmed as the ceiling.
...
9. **[../eve/07-build-runbook.md](../eve/07-build-runbook.md) Phase 7 and Phase 9**: the sink filter widened to the six streams (HLD §13.2) — unchanged by this page; Phase 9's `--lock-retention-period` moves to the P106 trigger (after the DPO's ceiling is recorded, before Stage 1), with the interim `UpdateBucket` alert; the witness runbook states Google-managed encryption (P112); `eve-export@` gains the `exports/` duty (§5.4).
10. **[../eve/03-lld.md](../eve/03-lld.md) §8–§9**: `eve.*` 400-day partition expiry confirmed; the export of `findings`, `verdicts`, `incidents`, `pages` daily is P107; `eve_workspace_logs` stays the reconciliation source.
...
12. **[03-gemini-enterprise-environment.md](03-gemini-enterprise-environment.md) §5.4, §13, §14 and its P52; [../gemini-enterprise.md](../gemini-enterprise.md)**: the app in `eu`; `discoveryengine` Data Access at the folder — the same decision on both pages; the `ge-requests` log view is in §3.3 here; the EU-DRZ model rule (DL-7.3) and the edition question. **Disagreement to settle: chat retention — page 03 proposes 90 days, this page 30 (P113); both are `Assumption:` under HLD P13 and the DPO decides; the reconcile pass picks one interim number for runbook step GE-6 and the two pages cite it.**
...
14. **[12-open-decisions.md](12-open-decisions.md)**: P104–P114 recorded; agent-set decisions 17, 25, 31, E-14, M-5, topology 47, 51, 52 and HLD P13 dated to this page.
...
16. **[02-landing-zone-and-tiers.md](02-landing-zone-and-tiers.md) §3 (topology rows 22–23 retired) and §8**: "one sink with `includeChildren`" becomes the two aggregated sinks of §3.2; `KMS_PROJECT` joins the four core projects (page 09 P118) in its folder table and `restrictServiceUsage` allow-list.
...
- [09-supply-chain-secrets-recovery.md](09-supply-chain-secrets-recovery.md) — owns the key table this page's §9 feeds; `KMS_PROJECT` and HSM-everywhere (its P118); its P119 exception 2 conflicts with P112 here
