...
- Scope, from the review's HLD brief ([00-objective-review.md](00-objective-review.md) §5.A):
  ...
  chapters 11–13 and `project-topology.md`); gaps PS-01, PS-02 (the landing-zone half —
  Agent Identity itself is the identity page's), SCA-01, SCA-02, SCA-03, SCA-06, SCA-07.
- Standing constraints, unchanged and not re-argued: no domain-wide delegation; the language
  ...
  safety interlocks are plain authenticated REST; Wall-E holds Super Admin (P33) and the tier
  model is designed around that fact.
- Conventions: `Assumption:` marks an inferred fact; *tbd* a value nobody has decided; every
  Google product, constraint, role, API and limit named here was re-verified on 2026-09-13
  against the URL cited beside it (§10) — where a fact could not be verified the row says
  "unverified" and keeps *tbd*. Every control names its **owner** (a role, never a person),
  the **resource** it sits on, how it is **verified** and what happens when it **fails**.
...
| Brief (A1, SCA-07) | Platform tier | Folder |
|---|---|---|
...
Any page in the set that says "T1 agent" means a trigger class and is wrong; the reconcile pass
replaces it with the letter (§9).
...
The HLD's §11.1 matrix is the summary; this is the control list a builder and a reviewer work
from. Every row states the **resource** the control sits on, the **owner** (role), how it is
...
| C2 | Console Model Armor on, tenant-wide | platform | the Gemini Enterprise app's Configurations tab | Gemini Enterprise admin | daily drift check reads the setting | setting off = severity 2 finding; the admin re-enables; every hour off is recorded on the app's compliance row |
...
| — | Agent code | none. A Tier C agent has no code the platform trusts or checks beyond the register row | | | | |
...
| R8 | Registry card generated by CI, resource-level query grant only (decision 42 generalised) | platform | shared Agent Registry in `CORE_PROJECT`; the engine's IAM policy | CI identity | reconciliation: card ≠ register row = drift | drift = finding; a project-level `discoveryengine.serviceAgent` grant on the agent project is a severity 2 finding and is removed |
| R9 | Fleet kill K7 reaches the project (KF-1: the folder allow-list minus `aiplatform` and `run`) | platform | `gcp.restrictServiceUsage` at the tier folder | platform owner (PAM); the K7 job on a severity-1 rule | monthly drill in nonprod, time recorded | a drill over 5 minutes end to end is a finding against the platform, not the agent |
| R10 | Budget, labels, Essential Contacts, PAB binding (§3.6, §3.7) | platform | the project | factory | admission gate; reconciliation | a missing mandatory label fails the factory run before the project is created |
| — | Agent code (CI-checked against the contract) | the agent's repository | `agent-manifest.yaml` with `families[]` all `READ`, `egress:` hostnames, `fingerprint`; the prompt-security chapter's system-instruction rules; taint handling on peer replies (B6) | agent owner | manifest schema check; the injection regression suite of `wall-e/11` §6 as a fixture | CI fails; a manifest declaring any write family in a Tier R folder fails the factory run with `tier_mismatch` |
...
| W9 | Nonprod project under `fld-agents-w-nonprod` with the same manifest, `env=nonprod` (§3.5) | platform | the nonprod project | factory | admission gate: prod promotion is an attestation of the nonprod-tested digest | no nonprod = no prod |
...
| # | Control | Where it lives | Resource | Owner | Verified by | On failure |
|---|---|---|---|---|---|---|
| P1 | The tier gate of HLD §0.4 is green before the credential is granted: Eve observe-and-report live and drilled, the witness organisation, the SIEM with 24x7 acknowledgement, two human super admins, the penetration test, the DPIA started, the signed deviation, the two lists, the perimeter decision | organisational, checked by CI | the register row's `gate_checklist` sub-document, one line per item with a date and a signer | platform owner; security reviewer signs | the admission gate refuses a `tier: P` row whose checklist has an empty line | the row does not merge; no project is made |
...
| P3 | Dedicated Eve (deterministic control path + reporting path), not the generalised platform verifier (HLD §12.4) | platform | `EVE_PROJECT`, `EVE_ADVISOR_PROJECT` | Eve owner (outside the Wall-E line) | Eve's own drift job; the witness absence alarm | Eve silent 15 min = `log_pipeline_silent` halt |
...
| PSA2 | Own folder `fld-agents-p-sa` with policies stricter than P, never looser: `restrictServiceUsage` allow-list holding exactly the services the Wall-E set names; own Model Armor floor with the hard-denied vocabulary detectors; own kill-plane binding (KF-1 and KF-2 attach to this folder independently of `fld-agents-p`) | platform | the folder | platform owner; IT security owns the floor | drift job; the monthly K7 drill exercises the P-SA binding separately | a P-SA policy looser than its parent P folder is a severity 1 finding |
...
| X2 | Opening conditions are the HLD's §11.5 list; each becomes a dated line on the folder's decision file before the allow-list gains one service | organisational | `decisions/` | platform owner; AI-safety reviewer (role does not exist) | the decision file | — |
| X3 | Every containment primitive of HLD §11.4 live and drilled for six months before the first X row | platform | K7 drill records | platform owner; security reviewer | drill record dates | — |

### 1.4 Who runs which control — the staffing view

HLD §11.2 gives the per-tier RACI. The one addition this page makes: **the factory run itself
is a controlled operation** with two phases and two identities (§3.4) — at Tier R the platform
owner runs both; at Tier W and above the privileged phase needs a PAM approval by the security
reviewer, so a factory run for a credential holder is a two-person act from the first day.
...
- Enforcement: `roles/resourcemanager.projectMover` is granted to nobody standing; it is a PAM
  entitlement at `fld-agentic-platform` with the security reviewer as approver and a
  justification naming the decision file (§3.4). A `MoveProject` event in the folder is a SIEM
  rule.
...
and 300 child folders per parent (Resource Manager limits page, updated 2026-09-09, §10).
...
flowchart TB
    ...
    FAP --> GE["fld-gemini-enterprise<br/>discoveryengine Data Access logs on"]
    ...
    WIT["org-witness — a second organisation, not under ORG<br/>EVE_WITNESS_PROJECT (HLD §13.2)"]
```

### 2.2 Every folder, its purpose, its policy additions, who may create below it

| Folder | Purpose | Holds | Policy additions beyond the §4 baseline (exact values in §4.2) | Who may create children | Budget default per project (`Assumption:`, HLD §3.1; numbers are P31) |
|---|---|---|---|---|---|
...
| `fld-platform-core` | Shared services; no agent principal is ever bound here | `CORE_PROJECT`, `LOGGING_PROJECT`, `CICD_PROJECT`, `VALIDATOR_PROJECT` | `gcp.restrictServiceUsage` allow-list of core services (§4.2); `iam.managed.workloadIdentityPoolProviders` limited to the git host's issuer (§3.4); `iam.managed.disableAccessPolicyBinding` enforced (no gateway here) | platform owner via PAM; these four are made once | 300 EUR/month |
| `fld-gemini-enterprise` | The tenant app's project, imported (HLD §2.1) | `GEMINI_PROJECT` | `discoveryengine.googleapis.com` Data Access logs on; `gcp.resourceLocations` gains `eu` for the app's multi-region | nobody — the import is a one-off with a change window | *tbd* (licence-driven) |
...
| `fld-controllers` → `-prod`, `-nonprod` | Eve's shape: the control path and the reporting path; any future Tier W platform verifier instance | `EVE_PROJECT`, `EVE_ADVISOR_PROJECT`; nonprod copies against the sandbox tenant | W-grade baseline; `deny-controllers-foreign`: no `secretmanager` for any principal outside Eve's allow-list; **`aiplatform.googleapis.com` is denied on `EVE_PROJECT` at project level, not on the folder** (the reporting path needs it — HLD §3.1) | `verifier-project` module, privileged phase approved by the Eve owner | 200 EUR/month |
...
### 3.1 Why project-per-agent is forced, restated with today's sources

Two Google facts, re-read on 2026-09-13, make one project per agent the only safe shape:

1. "All Agent Runtime agents deployed within that same project and region must bind to the same
   specific egress and ingress Agent Gateway instances" — the runtime deploy page, updated
   2026-09-08 (§10). Two agents in one project share one egress allow-list, so one agent's
   `egress:` hostnames become the other's.
2. `roles/discoveryengine.serviceAgent`, the documented cross-project grant for the tenant
   app, is **project-wide** over every engine and carries create/update/delete
   ([../project-topology.md](../project-topology.md) §3 row 2). The platform avoids it with an
   engine-scoped custom role (decision 42), but the fallback, if ever needed, must touch one
   agent only.

Add the quota argument (Model Armor's 1,200 QPM sanitize per project, HLD §0.5) and the
teardown argument (D10: deleting the project removes the agent and nothing else), and the
pattern is settled. The cost is hundreds of projects; Resource Manager's limits make that
routine (§2.1), and the CreateProject rate limit ("costs 10 requests per second" of the API
quota, same page) is irrelevant at the factory's pace.
...
|---|---|---|---|
| 1 (engine query by the tenant app's service agent), 2 (fallback) | `publish_to_gemini: true` → custom role `<agent>EngineQuery` holding `aiplatform.reasoningEngines.query`, bound on the engine to `service-${GEMINI_PROJECT_NUMBER}@gcp-sa-discoveryengine…`; row 2 never made by the factory (a dated exception if ever) | `agent-project` | the engine |
| 3 (Eve's invokers on `walle-actions`) | `control_invokers[]`: principals that may call `/v1/control/*` — `roles/run.invoker` service-level; the in-app `CONTROL_CALLER_ALLOWLIST` is generated from the same list | `agent-project` | the action service |
...
| 8–10 (topics, subscriptions) | `event_subscribers[]` → `roles/pubsub.subscriber` on the topic only; the subscription lives in the consumer's project (decision 45) | `agent-project` (topic side); consumer module (subscription) | the topic |
| 11–16 (Eve's carve-outs in `EVE_PROJECT`: `publicKeyViewer`, `verdict_receipts` reader, CI `objectCreator` on `ladder/`) | `verifier.carve_outs[]`, capped at three per verified agent (decision 48) | `verifier-project` | the key, the view dataset, the bucket prefix |
...
| 22, 23 (organisation sink writer identities) | **retired as per-agent organisation sinks**: the one aggregated sink at `fld-agentic-platform` writes to `LOGGING_PROJECT`; per-agent access is a log view. Eve's independent copy stays as the one organisation-level sink outside the aggregate (HLD §7.1) — `verifier-project` makes its dataset and the writer identity's `dataEditor` | `LOGGING_PROJECT` module; `verifier-project` | the destination dataset |
...
|---|---|---|
| `agent-project` | project under the tier/env folder with the id of §3.6; labels; billing link; APIs per tier (the tier's allow-list is the ceiling, the module enables the subset the manifest needs); Essential Contacts (owner group); the budget of §3.7; service accounts per duty (W+), keyless; the engine skeleton with `identity_type = AGENT_IDENTITY` and `encryption_spec` on the folder key ring; egress gateway with default-deny access policy from `egress:`; ingress gateway with `failOpen: false` if `machine_callers` is non-empty; the `iap.egressor` bindings; the action service (W+) with IAM-only invoke and, after P3, the internal load balancer, serverless NEG and PSC endpoint; regional secrets (W+, empty — values are bootstrapped by the agent's runbook, never by the factory); Firestore with PITR, scheduled backups, delete protection (W+); `<agent>_audit` dataset on `audit.schema`, insert-only writer role, the reader ACL; content-log bucket with generated reader IAM and a log view; Pub/Sub topics; the monitoring baseline module of the tier; Model Armor template from the tier standard; the registry card; the `created_by`/`factory_run` labels | the per-project `principalSet://cloudresourcemanager.googleapis.com/projects/N/type/ServiceAccount` entry on `deny-agents-platform`; the `pab-agents` binding for the project's agent identities; the P-SA floor (only for that folder); any dated exception (an org-policy override on the project, a `gmail-api-push@` member) |
| `verifier-project` | Eve's shape from HLD §3.2: the project(s) under `fld-controllers-*`; KMS key ring with an HSM approval key; verifier, gate, reconciler, export identities; `eve.*` datasets; the locked evidence bucket; the independent sink's destination dataset; the reporting-path project with no signer, no invoker, no secret; `aiplatform` **denied at project level** on the control-path project by a project-level `gcp.restrictServiceUsage` denylist | the project-level org policy on the control-path project; the deny policy `deny-controllers-foreign`; the PAB binding; the witness-side push grants are made by the witness administrators, not by any module here |
| `improver-project` | Mo's shape: `MO_PROJECT` under `fld-improvers-*`; metric datasets; the `mo-proposals` drop box; the three-tier IAM seam; scheduled queries; `mo-reporter` job; no invoker anywhere; `aiplatform` enabled at S4 only | `deny-improvers` (no invoker on any credential holder); the PAB binding |
...
(page updated 2026-09-10, §10). The git host itself is P22 and is not decided here.
...
issuer URI only (constraint name verified on the constraints page, §10), so no other provider
...
flowchart LR
    ...
    R1["Routine apply<br/>factory-apply@ via WIF<br/>project, APIs, IAM in project, engine, gateways, datasets, budget, labels, card"]
    P1["Privileged apply<br/>a human with a PAM entitlement (1 h)<br/>deny-policy entry, PAB binding, folder/project org-policy exceptions, floor"]
...
|---|---|---|---|---|
| Routine | `factory-apply@CICD_PROJECT` | `roles/resourcemanager.projectCreator` on the six tier/env folders and on `fld-controllers-*`, `fld-improvers-*` (folder level); `roles/serviceusage.serviceUsageAdmin` on the same folders; `roles/resourcemanager.projectIamAdmin` on the same folders (it must bind duties inside the projects it makes); `roles/billing.user` on the platform billing account (to link projects) and `roles/billing.costsManager` on it (to create budgets); `roles/agentregistry.admin` on the shared registry in `CORE_PROJECT`; `roles/storage.objectAdmin` on the Terraform state bucket; `roles/cloudbuild.builds.builder` in `CICD_PROJECT` | A factory that cannot create projects, enable APIs or bind IAM inside them is not a factory. The power is real and is bounded by *who can make it act*: one repository, one branch, two human reviewers, WIF with numeric-id conditions, no key, and the PAB `pab-core-ci` limiting it to `fld-agentic-platform` | `roles/orgpolicy.policyAdmin`, `roles/iam.denyAdmin`, `roles/iam.principalAccessBoundaryAdmin`, `roles/logging.configWriter` above project level, `roles/modelarmor.floorSettingsAdmin`, `roles/resourcemanager.folderAdmin`, `roles/resourcemanager.projectMover`, `roles/secretmanager.*`, any `run.invoker`, any `iam.serviceAccountTokenCreator` on a credential holder |
| Privileged | the platform owner (Tier R) / the platform owner with the security reviewer as PAM approver (W+) | none standing; PAM entitlements at `fld-agentic-platform`: `roles/orgpolicy.policyAdmin`, `roles/iam.denyAdmin`, `roles/iam.principalAccessBoundaryAdmin`, `roles/modelarmor.floorSettingsAdmin`, `roles/resourcemanager.projectMover`, `roles/resourcemanager.folderAdmin`, each 1 h, justification = the pull request URL (HLD §4.4) | The HLD's "no standing `orgpolicy.policyAdmin` anywhere" (§11.4) holds; every folder-level change is a PAM grant, which is a SIEM event and TISAX access-review evidence | — |

**Verified by:** the drift job asserts the routine identity's binding set equals this table and
that no other principal holds `projectCreator` under the platform folder; a Security Health
...
without a PAM grant in the same hour is a SIEM finding.
...
| Nonprod is **optional at Tier R, mandatory from Tier W**, mandatory for controllers and improvers | register field `env_split: required|optional` derived from `tier` | the admission gate refuses a `prod` W+ row without a `nonprod` project id | gate | row not admitted |
...
0–63 characters of the same alphabet (Resource Manager REST reference, §10). **Labels apply to
...
| `ai_act_class` | `not-ai-system minimal limited-art50 annex-iii-adjacent high-risk` | register | publication gate |
...
`tisax_scope` is **not** a label: the HLD §3.4 calls it "a folder-level label", and labels do
not exist on folders. It becomes the tag `agp-tisax-scope` bound at `fld-agentic-platform` and
overridden at `fld-agents-x` (out) — recorded for the reconcile pass in §9. `tier` and `env`
...
provider documentation, §10.

| Element | Decision | Owner | Resource | Verified by | On failure |
|---|---|---|---|---|---|
| One budget per project | amount = the tier default of §2.2 (P31 numbers) unless the register row sets `budget_eur` higher with a reason; thresholds 50 %, 90 %, 100 % of actual and 100 % of forecast | agent owner (amount); factory (resource) | `google_billing_budget` filtered on the project | drift job; the admission gate requires the budget to exist | no budget = gate fails |
| One aggregate budget per tier | at billing-account level, `budget_filter.labels = {tier: <x>}`, amount = sum of the tier's defaults × 1.2 | platform owner | billing account | quarterly review (P31) | — |
| Notification | Monitoring channel `platform-budgets` (Terraform-managed, the same channel family as the absence policies) plus the owner group by email; `disable_default_iam_recipients = false` so billing admins still see it | platform owner | the channel | a test alert at creation | — |
| Pub/Sub topic `budget-events` in `LOGGING_PROJECT` | every budget publishes; consumers: the billing export dashboard, Mo's cost metric; **no automation disables billing** — a billing disable would stop the evidence pipeline of the project first, which is the opposite of what a stop needs; the stop levers are K0–K7 | platform owner | the topic | — | — |
| Billing export | one BigQuery export in `LOGGING_PROJECT` keyed on the labels of §3.6 (HLD §3.4) | platform owner | the export | Mo's cost pack reads it | — |
| Nonprod | 50 % of the tier default, same thresholds | factory | — | — | — |
...
finding (dry-run page, updated 2026-09-09, §10). Names below were each re-read on
...
| # | Constraint (exact name) | Value at `fld-agentic-platform` | Why | Owner | Verified by | On failure |
|---|---|---|---|---|---|---|
| B1 | `constraints/gcp.resourceLocations` | allowed: `in:eu-locations`; the Gemini folder adds `eu`; `global` is **not** in the list unless the first factory run proves the folder floor-setting write needs it (HLD §3.3; the supported-services page lists Model Armor template creation and Agent Registry regional resources as enforced, not floor settings) | residency for every resource without a per-project checklist (defining-locations page, updated 2026-09-09) | platform owner | drift; the first factory run in nonprod | a refused floor write → `global` added for that resource type only as a dated exception |
| B2 | `constraints/iam.managed.disableServiceAccountKeyCreation` | enforced | keyless everywhere (`wall-e/12` §6) | platform owner | drift; key count = 0 | exception only through PAM, dated |
| B3 | `constraints/iam.managed.disableServiceAccountKeyUpload` | enforced | same; the **managed** form is chosen; the legacy `iam.disableServiceAccountKeyUpload` also exists and is what topology §5 and `PREREQUISITES.md` name — one spelling fleet-wide (§9) | platform owner | drift | — |
| B4 | `constraints/iam.automaticIamGrantsForDefaultServiceAccounts` | enforced | default service accounts get nothing (restricting-service-accounts page, updated 2026-09-09) | platform owner | drift | — |
| B5 | `constraints/iam.allowedPolicyMemberDomains` | allowed: the tenant's customer id; `gmail-api-push@system.gserviceaccount.com` as a named member exception where a Gmail push subscription exists; the `bUNIQUE_ID@gcp-sa-logging` writer identity of the aggregated sink if Google's page proves it needs one (it says BigQuery sink writers may); **no witness exception** (HLD §3.3); nonprod P folders add the sandbox customer id. The managed successor `constraints/iam.managed.allowedPolicyMembers` (`allowedMemberSubjects`, `allowedPrincipalSets`) is verified to exist; the legacy form is kept because the sets, Google's default enforcement for organisations created after 2024-05-03, and the exception syntax all use it — migration is a dated decision when Google announces a legacy retirement | no foreign principals beyond an enumerated list (restricting-domains page, updated 2026-09-09) | platform owner | drift | a binding to an unlisted domain is refused at the API |
| B6 | `constraints/iam.disableCrossProjectServiceAccountUsage` | enforced | the one rule: bindings, not attachments (topology §5) | platform owner | drift | — |
| B7 | `constraints/iam.managed.disableAccessPolicyBinding` | **enforced at `fld-agentic-platform`, lifted (not enforced) at `fld-agents-r`, `-w`, `-p`, `fld-controllers`** because every agent project binds `roles/iap.egressor` on its gateway; stays enforced on `fld-platform-core` and `fld-improvers`. The singular spelling is the one on the constraints index on 2026-09-13; `wall-e/13` §2.2 and §10 write the plural — §9 | the per-project lift the HLD lists as a factory exception becomes a folder value, removing one privileged step per run | platform owner | drift | a binding in core or improvers is refused |
| B8 | `constraints/storage.uniformBucketLevelAccess` | enforced | no ACLs on evidence buckets (Storage constraints page, updated 2026-09-09) | platform owner | drift | — |
| B9 | `constraints/storage.publicAccessPrevention` | enforced — **added by this page** | no bucket on the platform is ever public; the evidence and content buckets are the assets | platform owner | drift | — |
| B10 | `constraints/gcp.detailedAuditLoggingMode` | enforced — **added** | full request/response detail in Cloud Storage Data Access logs on the evidence buckets, which the retention lock and the witness copy depend on (Storage constraints page) | platform owner | drift | — |
| B11 | `constraints/compute.vmExternalIpAccess` | deny all | no VM with a public IP anywhere (constraints index) | platform owner | drift | — |
| B12 | `constraints/compute.skipDefaultNetworkCreation` | enforced — **added** | no default VPC with its permissive firewall in any project; the only networks are the ones the P3 spike's template creates (constraints index) | platform owner | drift | — |
| B13 | `constraints/sql.restrictPublicIp` | enforced — **added**, cheap | no Cloud SQL is planned; if one appears it is private (constraints index) | platform owner | drift | — |
| B14 | `constraints/essentialcontacts.managed.allowedContactDomains` | the tenant domain | Google's notices reach the platform, not a builder's inbox (constraints index) | platform owner | drift | — |
| B15 | `constraints/gcp.restrictServiceUsage` | per folder, **allow-list mode** everywhere (§4.2) | Eve cannot enable `aiplatform`; Tier R cannot enable Secret Manager; Tier X is empty; the first fleet-kill lever. Allow-list and deny-list are mutually exclusive modes of one policy (restricting-resources page, updated 2026-09-09), so KF-1 is "replace the tier folder's policy with the pre-written one that lacks `aiplatform` and `run`" — §9 for the kill-switch page | platform owner; K7 job through PAM | drift; monthly K7 drill | — |
| B16 | `constraints/run.allowedIngress` | allowed: `internal-and-cloud-load-balancing` on `fld-agents-w`, `-p`, `fld-controllers` — **held in Terraform, applied after P3's engine-reach spike** (HLD §3.3, §8.1); values verified `all`, `internal`, `internal-and-cloud-load-balancing` (ingress page, updated 2026-09-04) | credential holders never internet-reachable | platform owner | SHA custom module until applied; constraint after | until P3: finding; after: refused |
| B17 | `constraints/run.allowedVPCEgress` | allowed: `private-ranges-only` on W, P, controllers | no direct egress from credential holders (values `all-traffic`, `private-ranges-only` verified by search on Google's pages; the constraint's own page did not render — the value list is `Assumption:` until the first apply) | platform owner | drift | a revision with another egress value is refused |
| B18 | `constraints/run.allowedBinaryAuthorizationPolicies` | allowed: `default` on W and P (the policy in the same project as the service; Binary Authorization page, updated 2026-09-03) | §9 supply chain | platform owner | drift | unattested image refused |
| B19 | `constraints/iam.managed.workloadIdentityPoolProviders` | at `fld-platform-core`: the git host's OIDC issuer URI only (name verified on the constraints index) | only the factory's pool can federate; §3.4 | platform owner | drift | a second provider is refused |
| B20 | `constraints/gcp.restrictNonCmekServices`, `constraints/gcp.restrictCmekCryptoKeyProjects` | **held, not applied** — *tbd* with the TISAX label (HLD §8.3, P12): Autokey covers the services on its compatible list, Firestore is the recorded exception, and a `restrictNonCmekServices` value that lists Firestore would refuse the control-plane database. Listed so the reviewer sees they were considered (names verified on the Storage constraints page) | — | platform owner; ISMS | — | — |
...
| Folder | Allow-list |
|---|---|
| `fld-platform-core` | `logging`, `bigquery`, `bigquerydatatransfer`, `storage`, `cloudbuild`, `artifactregistry`, `containeranalysis`, `containerscanning`, `binaryauthorization`, `agentregistry`, `apphub`, `pubsub`, `iam`, `iamcredentials`, `sts`, `monitoring`, `cloudasset`, `policyanalyzer`, `cloudresourcemanager`, `serviceusage`, `orgpolicy`, `run` (the K7, reconciliation and drift jobs are Cloud Run jobs), `cloudscheduler`, `iap` (the ladder-state page), `cloudkms` (Autokey key ring), `essentialcontacts`, `billingbudgets`, `secretmanager` **denied by absence** (the core holds no secret; the CI identity has none to read) |
| `fld-gemini-enterprise` | `discoveryengine`, `aiplatform`, `modelarmor`, `agentregistry`, `agentidentity`, `agentidentitycredentials`, `iap`, `networkservices`, `networksecurity`, `dns`, `compute`, `apphub`, `logging`, `monitoring`, `storage` — the tenant app plus what `gemini-egress` needs; confirmed at the import |
| `fld-agents-r-*` | `aiplatform`, `agentidentity`, `agentidentitycredentials`, `agentregistry`, `apphub`, `modelarmor`, `iap`, `networkservices`, `networksecurity`, `dns`, `compute`, `discoveryengine`, `storage`, `bigquery`, `logging`, `monitoring`, `cloudtrace`, `observability`, `telemetry`, `apptopology`, `cloudapiregistry`, `notebooks`, `texttospeech`, `dataform` (the last three because Google lists them as gateway requirements, `Assumption:` console-only), `pubsub`, `essentialcontacts`, `billingbudgets`, `run` (nonprod only, for a Cloud Run reasoning service under P5); **not**: `secretmanager`, `cloudkms`, `firestore`, `cloudtasks`, `cloudscheduler`, `admin`, `gmail`, `chat`, `licensing`, `groupssettings` |
...
The deny policy `deny-agents-platform` and the PAB `pab-agents` are the identity page's to
detail (HLD §4.5; P8, P9). This page fixes only their **attachment shape**, because it is a
landing-zone property:
...
- `deny-agents-platform` attaches at `fld-agentic-platform` once; per project, the factory's
  ...
  organisation-level policy bound per project to
  `principalSet://agents.global.org-ORG_ID.system.id.goog/attribute.container/projects/N`
  — the documented form on the PAB page (updated 2026-09-10; limits: 1,000 policies per
  organisation, 500 rules per policy, 10 policies per principal set) — with eligible
  resources `fld-agentic-platform` only. PAB blocks only the permissions in its enforcement
  version, so it stays detection-grade until P9.
...
| Aggregated sink destination, locked log buckets, log views, billing export, `budget-events` | `LOGGING_PROJECT` | one sink with `includeChildren` instead of N organisation sinks (HLD §7.1) | platform owner | log views per agent |
...
|---|---|---|---|---|
| **P35** | Factory tooling: Cloud Foundation Fabric `project-factory`, `folder`, `organization`, `billing-account` and `agent-gateway` modules pinned at v58.0.0, wrapped by `agent-project` / `verifier-project` / `improver-project`; run by Cloud Build in `CICD_PROJECT`; FAST stages, Config Controller and Infrastructure Manager not adopted (§3.2). **Closes HLD P2** | FAST stages end to end; bespoke; Config Controller; Infrastructure Manager as runner | platform owner | Tier R |
| **P36** | CI identity: one service account `factory-apply@CICD_PROJECT` impersonated through WIF pool `wif-factory` with numeric-id attribute conditions and `main` only; two-phase apply — routine by the service account, privileged by a human through PAM (security reviewer approves at W+); the standing-role table of §3.4 is the ceiling; `pab-core-ci` bounds it to the folder | direct resource access; a standing human deployer; a factory SA with `orgpolicy.policyAdmin` | platform owner; security reviewer | Tier R |
| **P37** | Naming: `fld-` folders, `agp-<tiercode>-<agent>-<env>` project ids, the service-account, group, gateway, dataset and secret patterns of §3.6; enforced by CC-5 and CC-9 (Preview) after dry-run | free-form names with a CI regex only | platform owner | Tier R |
| **P38** | Label taxonomy of §3.6 (thirteen keys, slug values, register holds exact values), CI-enforced and reconciled because no constraint can require labels; tags `agp-tier`, `agp-env`, `agp-tisax-scope` bound at folders for policy conditions; `tisax_scope` is a tag, not a label | labels only; tags only | platform owner | Tier R |
| **P39** | Budget mechanism: one project budget per project at the tier default with 50/90/100 % actual and 100 % forecast thresholds, one aggregate budget per tier by label at the billing account, one Monitoring channel plus the owner group, `budget-events` topic, no automatic billing disable. Amounts stay **P31** | billing-disable automation | platform owner | Tier R |
| **P40** | Non-production split: nonprod optional at R, mandatory from W and for controllers and improvers (two folders added: `fld-controllers-nonprod`, `fld-improvers-nonprod`); no prod credential in nonprod; sandbox tenant for P/P-SA; promotion by attestation; Agent Platform Threat Detection on in nonprod one agent at a time | nonprod mandatory at R; nonprod as an OU (rejected by decision 29's reversal) | platform owner | Tier W |
| **P41** | Baseline additions beyond HLD §3.3: `storage.publicAccessPrevention`, `gcp.detailedAuditLoggingMode`, `compute.skipDefaultNetworkCreation`, `sql.restrictPublicIp`, `iam.managed.workloadIdentityPoolProviders` (core); the managed key-upload spelling fleet-wide; the legacy domain-restricted-sharing constraint kept; `iam.managed.disableAccessPolicyBinding` lifted at the agent and controller folders instead of per project; the 14-day dry-run rule; CMEK constraints held | as listed | platform owner; security reviewer approves | Tier R |
| **P42** | Custom constraints adopted now: CC-1, CC-2 (gateway), CC-3 (Cloud Run Binary Authorization by the v1 annotation — **closes the Cloud Run half of P4**), CC-5, CC-9 (naming, Preview), CC-7 (`AuthProvider`); held: CC-4 (with B16); spikes: CC-6 (AllowPolicy: PAM interaction, agent-principal eligibility), CC-8 (`identityType` — **the remaining half of P4**), CC-11 (sinks). Label enforcement by constraint is impossible (CC-10) | — | platform owner | D2's enforcement grade |
| **P43** | A tier is a folder; a tier change is a new project plus `revoke`; projects are never moved after creation except the `GEMINI_PROJECT` import; `projectMover` only through PAM with the security reviewer; only humans write `tier` | project moves with a change window | platform owner | — |
| **P44** | `gcp.restrictServiceUsage` in allow-list mode on every folder with the lists of §4.2; KF-1 is the pre-written replacement policy lacking `aiplatform` and `run`; P-SA's list is generated from Wall-E's register row and the factory refuses a manifest outside it | denylist mode | platform owner; security reviewer | Tier R; K7 |
| **P45** | The shared / per-agent split of §5, with the evidence lake, ladder-state page, K7 job, reconciliation and drift jobs in `CORE_PROJECT`; template standard and schemas as modules in the repository, instantiated per project | the review's A3 list | platform owner | Tier R |
| **P46** | The approval surface is per agent, never a shared page; the routing library and IAP audience pattern are shared | one shared IAP approval page | platform owner; security reviewer | Tier W |
| **P47** | Promotion map of §8: chapters 11–13 and `project-topology.md` are seeds referenced by the platform pages named there; the per-agent content stays in the agent sets with a framing note | copy the chapters into the platform set | platform owner | the reconcile pass |
...
## 9. Claims other pages must match (for the reconcile pass)

| Claim on this page | Page(s) that must agree | Nature |
|---|---|---|
| `tisax_scope` is a **tag** (`agp-tisax-scope`), not a folder label — labels do not exist on folders | [01-hld.md](01-hld.md) §3.4 says "a folder-level label" | HLD correction |
| The constraint is `constraints/iam.managed.disableAccessPolicyBinding` (singular) and is **not enforced** at the agent and controller folders as a folder value, replacing the per-project lift | `wall-e/13` §2.2 and §10(c)-1 write `disableAccessPolicyBindings` (plural); HLD §3.2 lists the per-project lift as a factory exception; topology §5 says "lifted on `WALLE_PROJECT` only" | spelling and mechanism |
| The managed key-upload constraint `iam.managed.disableServiceAccountKeyUpload` is the fleet spelling | topology §5 and `wall-e/PREREQUISITES.md` §4.2 use the legacy `iam.disableServiceAccountKeyUpload` (both names exist) | spelling |
| `gcp.restrictServiceUsage` runs in allow-list mode on every folder; **KF-1 is a policy replacement**, not "adding to a denylist" | HLD §11.4 KF-1 row says "denylist on the tier folder adding `aiplatform` and `run`"; the kill-switch page | mechanism |
| Two new folders: `fld-controllers-nonprod`, `fld-improvers-nonprod`; P-SA has `-prod`/`-nonprod` children | HLD §3.1 tree and table; Eve's and Mo's staging pages | tree |
| Nonprod optional at Tier R, mandatory from W | HLD §3.1 nonprod row ("one nonprod project per agent") | scope |
| Custom constraint CC-3 (Cloud Run Binary Authorization by annotation) is verifiable today — P4's Cloud Run half closes | HLD §3.3 last row and P4 | decision update |
| The approval surface is per agent (P46) | the identity page, `wall-e/12` §5.3; the review's A3 | decision |
| The factory identity holds no `orgpolicy.policyAdmin`, `iam.denyAdmin` or PAB admin standing; every folder-level change is a PAM grant | HLD §4.4 entitlement table (adds `projectMover`, `folderAdmin`, `principalAccessBoundaryAdmin` durations); the identity page | mechanism |
| Project ids follow `agp-<tiercode>-<agent>-<env>`; the sets' `WALLE_PROJECT` etc. remain handles | `gcp-projects.md` inventory; `wall-e/SETUP.md` §1.1 D1 | naming |
| The agent-principal eligibility under `iam.allowedPolicyMemberDomains` is unverified (CC-6 spike) | `wall-e/12` §1.4 if it asserts otherwise; the identity page | fact status |
| `run.allowedVPCEgress` value list is `Assumption:` until the first apply | HLD §3.3 | fact status |
| "T1 agent" style phrases refer to trigger classes, never tiers | every agent-set page | vocabulary |
