...
| `fld-platform-core` | Shared services; no agent principal is ever bound here | `CORE_PROJECT`, `LOGGING_PROJECT`, `CICD_PROJECT`, `VALIDATOR_PROJECT`, `KMS_PROJECT` (the Autokey key project — keys and nothing else; added 2026-09-13 on the reconcile pass, [09-supply-chain-secrets-recovery.md](09-supply-chain-secrets-recovery.md) P118) | `gcp.restrictServiceUsage` allow-list of core services (§4.2); `iam.managed.workloadIdentityPoolProviders` limited to the git host's issuer (§3.4); `iam.managed.disableAccessPolicyBinding` enforced (no gateway here); `run.allowedIngress` and `run.allowedBinaryAuthorizationPolicies` as W (the K7, drift, reconciliation and canary jobs are Cloud Run on the kill and evidence paths — [06](06-gateways-model-armor-perimeter.md) §4.3, [09](09-supply-chain-secrets-recovery.md) §1.5) | platform owner via PAM; these five are made once | 300 EUR/month |
...
| B21 | `constraints/cloudkms.allowedProtectionLevels`, `constraints/cloudkms.disableBeforeDestroy`, `constraints/cloudkms.minimumDestroyScheduledDuration` | `HSM`; enforced; `30d` — at `fld-agentic-platform`, preceding the first factory run (added 2026-09-13 on the reconcile pass; [09](09-supply-chain-secrets-recovery.md) §2.3, P118) | every key on the platform is HSM; no key is destroyed without a disabled window | platform owner | drift; a monthly negative test creates a SOFTWARE key in nonprod and expects refusal | constraint drift: severity 1 |
| B22 | `constraints/gcp.restrictTLSVersion` | deny `TLS_VERSION_1` and `TLS_VERSION_1_1` at `fld-agentic-platform` (added 2026-09-13; the one line of the EU Data Boundary policy set the baseline lacked — [08](08-data-logging-retention-sovereignty.md) §7.1; value syntax confirmed at the first apply) | no legacy TLS on any platform endpoint | platform owner | drift | — |
...
Reconcile pass run on 2026-09-13: every HLD row named below was corrected in
[01-hld.md](01-hld.md) with a dated line; the agent-set rows are left to the propagation stage
(HLD §18).
