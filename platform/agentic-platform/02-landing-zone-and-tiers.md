...
| X2 | Opening conditions are the HLD's §11.5 list; each becomes a dated line on the folder's decision file before the allow-list gains one service | organisational | `decisions/` | platform owner; AI-safety reviewer (role does not exist) | the decision file; the drift job (X1) | the allow-list stays empty; any service added without the dated decision line is severity 1 to `platform-security@` and reverted by the platform owner |
...
| `fld-controllers` → `-prod`, `-nonprod` | Eve's shape: the control path and the reporting path; any future Tier W platform verifier instance | `EVE_PROJECT`, `EVE_ADVISOR_PROJECT`; nonprod copies against the sandbox tenant | W-grade baseline; `deny-eve-project-foreign` on `EVE_PROJECT` (HLD §4.5; name aligned 2026-09-13): no `secretmanager` for any principal outside Eve's allow-list; **`aiplatform.googleapis.com` is denied on `EVE_PROJECT` at project level, not on the folder** (the reporting path needs it — HLD §3.1) | `verifier-project` module, applied end to end under `ent-factory-singleton` approved by the Eve owner or the security reviewer (P142) | 200 EUR/month |
...
|---|---|---|---|
| 1 (engine query by the tenant app's service agent), 2 (fallback) | `publish_to_gemini: true` → custom role `geEngineQuery` (P56; this row said `<agent>EngineQuery` until 2026-09-13) holding `aiplatform.reasoningEngines.query`, defined in the project by `agent-project` and bound on the engine by the agent project's CI as `<agent>-deployer@` (P56, P142) to `service-${GEMINI_PROJECT_NUMBER}@gcp-sa-discoveryengine…`; row 2 never made by the factory (a dated exception if ever) | `agent-project` | the engine |
...
| The factory identity holds no `orgpolicy.policyAdmin`, `iam.denyAdmin` or PAB admin standing; those three are organisation-level entitlements (`ent-platform-policy`), the folder-scoped ones `ent-folder-admin`, the move `ent-project-move`; `factory-apply@`'s `projectIamAdmin` is conditioned and absent from P-SA and controller folders (P142) | HLD §4.4 entitlement table and §18 item 25; the identity page §3, §5.2; page 09 §1.7; page 05 §4 | mechanism (§3.4's Privileged row corrected 2026-09-13; the earlier text claimed folder-level entitlements for organisation-only roles) |
...
| `run.allowedVPCEgress` value list (`private-ranges-only`, `all-traffic`) verified on 2026-09-13 | HLD §3.3 (aligned the same day) | fact status |
