...
- Maturity: detailed design, written 2026-09-13 under [01-hld.md](01-hld.md) §8.3 (key
  ...
  the supply-chain, secrets, recovery and containment brief items of
  [00-objective-review.md](00-objective-review.md) §5 H, which list the gap ids they close; the
  PAM entitlement table itself is
  [04 §5.2](04-identity-and-privileged-access.md#52-the-catalogue).
- What it does not re-argue: Wall-E holds Super Admin (P33); the tier model (P1); the perimeter
  spikes (P3); the PAM entitlements (HLD §4.4, catalogue in 04 §5.2). Where this page needs one of those, it points.
- Conventions: as the HLD. Every Google product, launch stage, constraint, role, field and
  ...
  *tbd*. `Assumption:` marks an inference. Decisions this page makes are **P115–P124** in
...
4. **Code execution (§4).** Below Tier X there is none, and the page says how "none" is
   ...
   `europe-west1`; the Tier X choice of GKE Agent Sandbox now rests on
   isolation, egress, identity and launch stage, not residency. GKE Agent Sandbox is GA per
   Google's own announcement, which moves P27's stage condition to "met" but opens nothing — §4.1.
...
| P, P-SA | nonprod project **and the sandbox Workspace tenant** ([02 §3.5](02-landing-zone-and-tiers.md#35-the-non-production-split-p40)); the nonprod robot account holds the same role in the sandbox tenant | as W + the two-lists denial test (every hard-denied operation refused in every lane), Eve's seeded-fault run against the nonprod engine, K5/K6 drill in the sandbox tenant |
...
| Readers | Exactly one attached service account per secret with `roles/secretmanager.secretAccessor` on that secret (never project-level); humans read secrets only through the PAM entitlement `ent-secret-read` (30 minutes, security reviewer approves) | [04 §5.2](04-identity-and-privileged-access.md#52-the-catalogue) |
| Data Access logs | Folder-level `auditConfigs` with `DATA_READ`/`DATA_WRITE` on `secretmanager.googleapis.com` and `cloudkms.googleapis.com`, no exemptions | [08](08-data-logging-retention-sovereignty.md) §4.1 (the `secretmanager` and `cloudkms` rows) |
...
|---|---|---|
| BigQuery datasets, Cloud Storage buckets, Cloud Run, Secret Manager, Pub/Sub, Artifact Registry | **Cloud KMS Autokey**, configured at `fld-agentic-platform` and inherited; keys are HSM; granularity per Google: one key per bucket/dataset, one key per project-location for Cloud Run and Secret Manager. Covers the platform evidence lake `platform-evidence` and `platform-records` and the `platform_logs` dataset ([08](08-data-logging-retention-sovereignty.md) §2.2 S14, S15); on a dataset a sink writes into, the dataset-level default key is set **before** the sink creates its tables (order matters, as with partition expiry) | "Autokey configurations are inherited by child resources"; "Protection level: HSM"; per-service granularity — https://docs.cloud.google.com/kms/docs/autokey-overview (2026-09-03) |
| Autokey key project | A **dedicated key project `KMS_PROJECT` under `fld-platform-core`** — the fifth core project (P118). Google: "keys created by Autokey are created in the designated key project for that folder". It holds keys and nothing else, `restrictServiceUsage` allow-list = `cloudkms` only, no agent principal ever bound | Same page; the HLD's four core projects become five ([12](12-open-decisions.md) §7 row 6; [02](02-landing-zone-and-tiers.md) §2) |
...
| Cloud Logging log buckets | Logging is not on Autokey's list, so no bucket gets a key by inheritance. **P119, exception 2 (narrowed 2026-09-13 to agree with P112):** the organisation `_Default`/`_Required` buckets and the Tier R content buckets stay **Google-managed**; the two central buckets `platform-evidence-logs` and `platform-identity-logs` take an **explicit HSM key** `platform-logs-europe-west1` in `KMS_PROJECT` (ring `logging`) at creation, and Tier W+ content buckets a per-agent key `<agent>-content-logs` — because Google documents CMEK **per individual log bucket**, set at creation and never addable later, distinct from the organisation-level Log Router CMEK ([08](08-data-logging-retention-sovereignty.md) §9, P112). Mechanics: the key's region must match the bucket's; the bucket's `kmsServiceAccountId` needs Encrypter/Decrypter; CMEK cannot be added to or removed from a bucket after creation, but the key can be changed. Consequence the reviewer accepts: a disabled or destroyed key makes the evidence **unreadable at once and, after Logging's three-hour buffer, unwritable** — an availability risk a Google-managed bucket does not have. Mitigations: disable-never-destroy (§3.5 R-K); the key's `cloudkms` Data Access and Admin Activity logs; a severity-1 SIEM rule on any key state change in the evidence projects; no `cloudkms.admin` outside PAM; and the second and third evidence copies are not on this key (Eve's copy is on `eve-evidence`, the witness on Google-managed encryption). For a Tier R content bucket Google-managed is enough because a 30-day content store's threat is its readers, not key custody — the reader controls of [08](08-data-logging-retention-sovereignty.md) §6.3 carry it | https://docs.cloud.google.com/logging/docs/routing/managed-encryption-storage (re-read 2026-09-13); Autokey list (above) |
| Eve's evidence copy: the locked bucket `gs://${EVE_PROJECT}-eve-evidence` and the `eve.*` datasets | **Explicit HSM key `eve-evidence` in ring `eve` of `EVE_PROJECT`** (the ring that holds `eve-approval`), not an Autokey key: Autokey's keys land in `KMS_PROJECT`, a platform-owned project, and Eve's copy must survive a platform-line key administrator; `EVE_PROJECT`'s KMS admin is behind PAM with the Eve owner as approver; Eve's runbook creates the one extra key, and SK-7 accepts it as conforming | Independence (P112) |
| Witness stores (`eve_mirror` dataset and the witness bucket in `EVE_WITNESS_PROJECT`) | **Google-managed encryption, or the witness organisation's own keys — never a key in the tenant's organisation**; recorded in the witness runbook | The witness exists to survive the tenant's administrators; a tenant key would give them a way to brick it (P112) |
| Gemini Enterprise conversation history (assistant chat sessions) | Google-managed: the imported app predates any `CmekConfig` registration and cannot become CMEK-protected in place (`Assumption:`, [03](03-gemini-enterprise-environment.md) §5.2). Whether the `CmekConfig` key `gemini-cmek` (§2.4) covers the assistant's conversation store of an app created after GE-4 is **unverified** | [08](08-data-logging-retention-sovereignty.md) §2.2 S17; [03](03-gemini-enterprise-environment.md) §5.2 |
...
| C | `gemini-cmek` (the tenant app's `CmekConfig` for data stores and apps created after GE-4 — [03](03-gemini-enterprise-environment.md) §5.2, P50) | KMS symmetric | `KMS_PROJECT`, ring `gemini`, location **`europe` multi-region** (Gemini Enterprise requires a multi-region `europe` key for an `eu` app; HSM availability in `europe` and its acceptance under `gcp.resourceLocations` are **unverified** and confirmed at the first factory run) | HSM | factory (`tenant-app` module writes the key; the `CmekConfig` registration is the Gemini Enterprise admin's act through PAM) | the Discovery Engine service agent and the Cloud Storage service agent of `GEMINI_PROJECT` (`roles/cloudkms.cryptoKeyEncrypterDecrypter` on the key only) | — | automatic, 90 days set explicitly by the factory (`Assumption:` the crypto standard) | `cloudkms` Data Access; a key state change is severity 1 (every post-GE-4 data store becomes unreadable) | platform owner (key); Gemini Enterprise admin (registration) |
| C | `platform-logs-europe-west1` (the two central log buckets `platform-evidence-logs`, `platform-identity-logs`; P112) | KMS symmetric | `KMS_PROJECT`, ring `logging`, `europe-west1` | HSM | factory, before Stage 0 creates the buckets (CMEK cannot be added later) | Logging's `kmsServiceAccountId` as the sole Encrypter/Decrypter | — | automatic, 90 days (`Assumption:`) | `cloudkms` Data Access; a key state change is severity 1 (PL-11) — disable-never-destroy, since a disabled key makes the evidence unreadable | platform owner |
| C | `eve-evidence` (Eve's locked bucket S5 and `eve.*` datasets S4, [08](08-data-logging-retention-sovereignty.md) §2.2) | KMS symmetric | `EVE_PROJECT`, ring `eve`, `europe-west1` — **explicit, not Autokey**, so that a platform-line key administrator in `KMS_PROJECT` cannot brick Eve's copy | HSM | Eve's runbook (`verifier-project` module) | the bucket's and datasets' service agents | — | automatic, 90 days | as above | Eve owner |
...
which [08](08-data-logging-retention-sovereignty.md) §9 cites: every platform store is
...
store is meant to survive; no Key Access Justifications (they require
...
| SK-3 | Access by a non-reader alerts | detection | detection desk of the tier | folder log-based alert | monthly negative test: a PAM-elevated human reads a nonprod secret and expects the page | a read with no page: log-pipeline incident (the absence rules of [07 §7](07-monitoring-detection-incident-response.md#7-pipeline-heartbeats-and-the-log_pipeline_silent-halt) also fire) |
...
For every agent the factory sets the daily JSONL export with manifest into the verifier's locked
bucket ([08](08-data-logging-retention-sovereignty.md) §5.4, P107), weekly BigQuery table
snapshots of every audit table into a snapshot dataset in `LOGGING_PROJECT` with a 400-day
expiry — read-only, in the base table's region (`EU`), because time travel covers only seven
days ([08](08-data-logging-retention-sovereignty.md) §5.2 R1) — and, for Tier P, Eve's mirror
and the witness copy (HLD §13.2). The recovery consequence: the agent's project can be deleted
after the export confirms (HLD D10), because the `revoke` factory operation checks the last
export's row count against the table before it schedules deletion.
...
| RC-3 | Daily evidence export + weekly snapshots; locked bucket | enforcement (retention lock) + detection (absence alarm) | platform owner | the verifier's locked bucket ([08](08-data-logging-retention-sovereignty.md) §5.4); snapshot dataset in `LOGGING_PROJECT` | absence alarm on the export heartbeat; the quarterly R-A drill | a missed export: severity 2 same day; two missed: severity 1 (HLD `log_pipeline_silent` halt reason for Tier P) |
...
| GKE | no GKE cluster exists outside `fld-agents-x` | **`container.googleapis.com` is absent from every tier folder's `gcp.restrictServiceUsage` allow-list except `fld-agents-x`** ([02 §4.2](02-landing-zone-and-tiers.md#42-gcprestrictserviceusage-per-folder--the-allow-lists) lists every folder's allow-list; none but `fld-agents-x` could name `container`, and that one is empty) — a cluster cannot be created | SHA finding on any `container.googleapis.com/Cluster` outside `fld-agents-x` | **enforcement** (the strongest of the three) |
...
stated in the register row. Agent Runtime Code Execution (EU-resident, §4.1) is the recorded second option, not adopted today because its
...
The eight properties the objective review's gap register named ([00-objective-review.md](00-objective-review.md) §4): three the sets already had,
...
The HLD's primitive list names five more rows beside the eight, each held here: the
**model-free monitor with halt authority** (exists — Eve's shape; the platform's AI-control
primitive; tiers P and X; §5.2), **re-qualification on model change** (buildable; W+; §5.2),
**monoculture** (policy; X — advisory monitors on a second model family, P26; §5.2),
**transitive capability through peers** (buildable; R+ — `agent` principals L0 for writes,
tainted, `iap.egressor` and gateway bindings limited to manifest-named entries; §5.2, CT-7) and
**deception-robust monitoring** (research-grade; X; §5.3).
...
| Deception-robust monitoring ("a trusted monitor watching an untrusted model") | Research. The shape in the AI-control literature assumes a weaker trusted model auditing a stronger untrusted one plus human audit of a sample; nothing on Google Cloud packages it; Semantic Governance's own page says its verdicts "may not be accurate" (HLD §2.1) | HLD §11.4 (graded research in §5.1); no product to cite | The model-free monitor of §5.2 plus blind human grading of a sample (the ladder's evidence rule) — an honest subset: it catches what the catalogue and the predicates can express and what a human grader reads |
...
Until then the `gcp.restrictServiceUsage` allow-list of `fld-agents-x` stays **empty**: each
condition becomes a dated line on the folder's decision file before the allow-list gains a single
service, and every containment primitive must be live and drilled for six months before the first
Tier X register row — a missing drill month resets the six-month clock, and a row merged without
that history is refused by the admission gate ([02](02-landing-zone-and-tiers.md) §1.3, controls
X1–X3). None of this is in reach in 2026. The platform hosts an AGI-class agent the day those hold and
...
This page's decisions are **P115–P124**, recorded with their options, owners and gates in
[12-open-decisions.md](12-open-decisions.md) §2, §3 and §6. The HLD rows they refine — P4 (the
custom-constraint spike), P27 (stage condition met, Tier X closed), §9's "W and P folders"
(P115), §8.3's Firestore and Logging exceptions (P119), §3.1's core projects (P118), §10 R-B
(P120) and §11.4's code-execution constraint (P121) — are recorded in those rows and in
[12-open-decisions.md](12-open-decisions.md) §7 rows 5, 6 and 8.
...
|---|---|---|
| Agent Runtime Code Execution launch stage, isolation technology and sandbox egress path | whether it can replace or complement GKE Agent Sandbox at Tier X (§4.3); its residency is settled (§4.1: `europe-west1` and the `eu` multi-region) | the Sandboxes pages state a stage and an egress model, or a nonprod sandbox's egress is observed in the gateway's logs |
...
| Whether `gemini-cmek` covers the Gemini Enterprise assistant's conversation history | §2.2 row; [08](08-data-logging-retention-sovereignty.md) §2.2 S17 | the `CmekConfig` page, or the store's encryption read after GE-4 |
...
- The review reports under `.agent-work/review/` (outside the wiki): `platform-security.md` gaps 8, 9, 12, 14; `scale-and-agi.md` Gap 9; `tisax.md` §4.7, §4.8.
