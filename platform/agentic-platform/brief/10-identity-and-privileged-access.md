...
This chapter answers the first risk theme of Chapter 3, tenant compromise through the robot credential, from the identity side; Chapter 11 answers it with detection and Chapter 15 with Wall-E's code. It also carries part of two other themes: notional two-person rules, by saying which Google enforces and which rest on people; and a controller reachable by what it controls, since Eve stays outside the fleet kill switch.
...
7. **No domain-wide delegation, for any agent, at any stage** (a constraint of Chapter 4). Delegation is limited only by OAuth scopes, so a compromised delegate can act as anyone, a super admin included, and a delegated verifier could impersonate the robot it verifies. Each robot holds its own consented refresh token and is never impersonated; no other agent holds a Workspace credential. Granting delegation is under multi-party approval and on the hard-denied list ([page 04 §1](../04-identity-and-privileged-access.md#1-principles-inherited-stated-once)).

## Agent Identity for every reasoning layer

Every layer that reasons runs under a Google-issued agent identity, not a service account someone could impersonate, with 24-hour certificates and bound tokens.

For Agent Runtime the identity type is GA; the factory sets it and CI refuses an engine config without it. The constraint that would make Google enforce it is spike P4 (Chapter 7, Landing zone and the tier model); until then it is a CI check graded detection.

Agent Identity for Cloud Run is Preview on 2026-09-13 and goes to production when GA (P5, open); meanwhile a production Cloud Run reasoning service on a service account is a dated Tier R exception, refused from Tier W. Gemini Enterprise no-code agents carry the product's own identity.
...
They never hold a secret read, an IAM write, a deploy permission, a key or anything outside `fld-agentic-platform`.

**Machines that are not agents** are keyless service accounts, one per duty, each holding exactly the rows the factory generates from the register.
...
Naming "all agents in a project" is not settled: Google's allow-policy, boundary and deny pages use three different spellings. The factory emits the allow spelling for allow and deny and the boundary spelling for boundary bindings. Whether a deny policy accepts an agent principal set is `Assumption:` until a throwaway engine proves it (P8, narrowed by P61), and the template is corrected from that evidence, never from a page. Register row 11 stays open: Wall-E's identity page carries only the allow spelling ([register §7](../12-open-decisions.md#7-values-that-differ-between-pages)).

## Service-account policy

Seven rules are set at the folder, applied by the factory and asserted by the drift job ([page 04 §2.3](../04-identity-and-privileged-access.md#23-service-account-policy)).

- **SA-1, no keys.** Also denied; an existing key is severity 1.
- **SA-2, no cross-project attachment.** Every crossing is a binding on a resource.
- **SA-3, one account per duty.** An account that reads a credential never holds a deploy permission, and the reverse.
- **SA-4, no standing owner or editor after the factory runs.** The CI identity's limits are Chapter 7 (P142).
...
A human is never bound directly to a resource; a group is, carrying the irreversible security-group label that admits only domain members.
...
The **control groups** (owners, security, approvers, app admins, Eve's owners, Wall-E's operators) are on a hand-managed, two-human-merged list the job refuses in code. A live membership change without a merge is a severity-1 page, never auto-corrected: a robot that could edit `eve-owners@` would be a path around Eve's independence (P65, proposed).
...
- **R1, secrets.** Exempt: each action service's own account and `eve-controller@`, limited to their one secret.
...
- **R5, governance and evidence.** No exceptions: policy, sinks, log buckets, IAM, boundaries, custom roles, floors, registry entries and PAM.
...
Every permission name was checked on 2026-09-13 against Google's list of deniable permissions, because an unlisted name raises no error and silently never matches. The check changed three things:
...
No standing owner exists anywhere. Humans reach dangerous roles through entitlements that require a ticket or incident id, notify `platform-security@` and are reviewed quarterly. Facts verified on 2026-09-13 shape the catalogue:

- **No self-approval, enforced by Google.** One-person operation at Tier R activates without approval on a mandatory justification, recorded as such, until a security reviewer exists.
...
- **Organisation-only roles.** Organisation-policy, deny and boundary administration sit at the organisation, the highest entitlement, with two approvers.
...
The catalogue ([page 04 §5.2](../04-identity-and-privileged-access.md#52-the-catalogue)) covers project repair (up to two hours), deploying a credential holder outside CI (never approved by the agent's owner), reading one secret (30 minutes), folder and platform policy, tenant app administration, the `GEMINI_PROJECT` move, singleton applies, witness export repair and two K7 entitlements.

The K7 entitlements need no approver, because a fleet stop must not wait at 03:00; their second person is the page every activation sends, and the two-human recovery. A standing catalogue role is severity 1 and is removed by the pipeline. A grant outside a change window without an incident id is severity 2 (P62, proposed; [page 04 §5.1](../04-identity-and-privileged-access.md#51-facts-verified-on-2026-09-13)).

## Human access to control surfaces

Every control surface (approval pages, Eve's console, ladder-state page, registry viewer, `k7-console`) is a Cloud Run service with IAP on it, admitting one group under an access-level condition. If any of the three is missing, the pipeline removes the proxy's invoker binding, so the surface goes dark rather than open.
...
"Signed in with a security key" comes instead from two Workspace settings: key-only 2-Step Verification and hourly Google Cloud re-authentication with a key (P63, proposed).

No human holds invoker on an action service, impersonates a credential holder, signs in to `walle@` or `eve@` after bootstrap, or drives the Admin console under the robot's session ([page 04 §6.3](../04-identity-and-privileged-access.md#63-the-access-levels), [§6.4](../04-identity-and-privileged-access.md#64-the-surfaces), [§6.5](../04-identity-and-privileged-access.md#65-what-a-human-never-does))

## Break-glass

Break-glass covers the two failures PAM cannot: PAM itself broken, or every approver unreachable. On Google Cloud it is two Cloud Identity accounts, `brk-gcp-1@` and `brk-gcp-2@`, in their own unit, with key-only 2-Step Verification and no recovery options. They hold standing Organization Administrator and PAM administrator, because break-glass is for when PAM cannot be used.
...
- **Placement and sign-in.** Both sit in `/Automation/Service Identities`, with the tenant's own hardware-key-only 2-Step Verification, since Google's admin enforcement is a gradual, edition-limited rollout. Two keys per account are in the safe.
- **No recovery.** An unmonitored recovery channel is a takeover channel, so there are none.
...
The robot unit also carries an Admin console Context-Aware Access level that no device satisfies. It is graded **detection-plus-friction**, `Assumption:`, until Google confirms whether it binds super admins (P7, open); no Admin SDK API access level exists. What survives Super Admin is the rule that an interactive login is an incident ([HLD §4.6](../01-hld.md#46-the-robot-user-accounts-and-context-aware-access); [page 04 §8.2](../04-identity-and-privileged-access.md#82-session-controls-on-the-privileged-tier)).
...
Eve's daily roster check and the SIEM page the second human and the desk on any role added or missing, or an admin's security settings changed. A third human is allowed only as a dated hand-over exception.
...
With multi-party approval on, a second administrator must approve changes to sensitive Workspace settings, including role assignments, domain-wide delegation, 2-Step Verification, recovery, SSO and Context-Aware Access. P66 (proposed) turns it on for every covered setting before the super-admin grant. It is **the only two-person rule Google enforces over the robot's credential**: that credential alone cannot complete a role assignment, even though Wall-E's code already refuses one.
...
The job's attested image reads four committed lever files and activates its own 30-minute entitlement with the case id. The rules that may call it are Chapter 11, Monitoring, detection and incident response; that path stays dry-run until the SIEM's outbound identity exists (P10). Eve only recommends K7 to a human. If the job cannot activate its grant, the contingency is a workload-federation principal, then a narrow standing custom role as a dated residual.
...
- **The change.** A pull request reverting the four files, reviewed by the security reviewer (and the second human when P-SA is in scope), applied through the two-approver platform-policy entitlement; the exit is the drift job's zero diff.
...
K0 to K6 are per-agent switches, owned by Chapter 15, Wall-E, the doer. K7 replaces none of them; KF-3 is K2 for a whole folder, and KF-2 and KF-4 are K3 for every agent at once.
...
One tension is unresolved: the monthly drill wants all four levers within 5 minutes, but deny changes can take 7 minutes or more to propagate, and page 04 does not say whether KF-2 is timed at application or propagation.
