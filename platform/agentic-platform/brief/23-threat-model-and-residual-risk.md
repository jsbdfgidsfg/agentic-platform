...
This chapter is where the brief's case is tested. Chapters 5 to 18 each answered one or more of the seven risk themes of Chapter 3, The problem and its risks; here every answer is set against the adversaries it has to survive, and what remains is written down with its owner and its disposition.
...
The design says it without euphemism: the organisation is giving a language model a path to administrative operations on its Workspace tenant, and then removing the human from that path one step at a time. That is a legitimate thing to build and it is the highest-blast-radius system in the estate. Every control exists so that when the model is wrong, manipulated or triggered at the wrong moment, the outcome is an audit row and a refused approval rather than an incident ([honest risk statement](../../wall-e/06-security-guardrails.md#the-honest-risk-statement)).

Since 2026-09-13 that path ends at a super-admin account (P33, decided). Three sentences follow and are not softened anywhere in the design: the action service is the only gate; the consented OAuth scope set is the only ceiling Google enforces; detection is the primary control, not a check on a role that already refused ([HLD, what this reverses and what it costs](../01-hld.md#what-this-reverses-and-what-it-costs)). Three properties carry most of the remaining weight: the model never holds the credential, the model cannot authorise itself, and the model cannot change its own leash.
...
What changed on 2026-09-13 is where the first, second and seventh rest. They used to rest on a Workspace role that refused; they now rest on the policy chain of the action service, on the hard-denied list refused in every lane, and, for the settings it covers, on Workspace multi-party approval, which is a two-person rule Google itself enforces. The rows are not yet ratified: the owner's signature on the two lists (P29) is open, and the ceiling is only as firm as that signature and the denial test suite that proves it.

### Who and what the platform defends against

The threat model assumes four kinds of adversary: an outsider who writes content the agent reads, from mail to display names and error text; an outsider holding a stolen credential or a compromised workload; an insider with legitimate access, the administrator included; and a supplier, from the model provider to a third-party agent. Non-malicious failure counts too: an invented call, a trigger storm, a poisoned upstream record, stale state between plan and execution, an outage of the controller ([threat model](../../wall-e/06-security-guardrails.md#threat-model)).

One row dominates all the others. A leaked robot credential is recorded as a **tenant compromise**: the whole tenant, and through the power to grant Organization Administrator, a path into the GCP organisation including Eve's and Mo's projects. It is bounded only by custody, the scope split and detection latency, and it is accepted as a signed deviation, row R-01 of the risk register.

## What prompt-based defence cannot do

Every agent's system instruction says that retrieved content is data, never instruction. The design keeps that instruction and assumes it will sometimes fail, because instruction and attack arrive on the same channel and the model cannot reliably tell them apart ([what prompt-based defence cannot do](../../wall-e/06-security-guardrails.md#what-prompt-based-defence-cannot-do)). Four rules follow, and they apply to every agent on the platform:

- No operation is promoted because "the prompt handles it".
- No autonomous run chooses a message recipient; templated notification to fixed recipients is a separate operation in its own family.
...
The same reasoning grades content screening: a probabilistic classifier is never a trust boundary, whatever its placement (Chapter 10, Agent Gateway, Model Armor and the perimeter). What stops an injection is structural: no credential in the model's process, a tainted run capped, explicit targets, and approvals minted where the model cannot reach.

## The adversaries, the controls that stop them and the detections that see them

Each entry below names the enforcement-grade control that stops the threat, the detection that sees it and its latency, the human who is paged, and the runbook ([runbooks](../07-monitoring-detection-incident-response.md#11-runbooks-per-scenario)). One paging rule applies to all the super-admin rules: they are hosted in the SIEM, never in Wall-E's project, severity 1, and page the detection desk and the second human outside the Wall-E administration line in parallel, escalating to the incident commander from IT security at twice the acknowledgement target ([super-admin set](../07-monitoring-detection-incident-response.md#62-the-super-admin-set-workspace-side--siem-hosted-severity-1-owned-by-it-security)). The acknowledgement targets for Tier P are 15 minutes in business hours and 60 outside, and K5 within 30 minutes of acknowledgement, all Assumption until the managed detection contract and the tabletop fix them ([targets](../07-monitoring-detection-incident-response.md#92-the-on-call-tool-escalation-and-acknowledgement-targets)). Below Tier P the honest number is "next business morning".

### A leaked robot token — RB-01

Nothing stops a stolen refresh token from working. Before the leak there is custody (Chapter 8, Identity, privileged access and the fleet kill switch); after it, a narrowed reach and a race. The narrow client keeps a Google-enforced scope ceiling on everything unattended; the broad client does not; the `cloud-platform` scope is consented on neither, checked in CI, so a token cannot call Google Cloud directly.

Detection is SA-05 (a token from a client outside the two committed ones), SA-07 (a robot event with no audit row within five minutes), SA-08 (the robot in an application it never needs) and SG-01 (an organisation-level IAM change by the robot), with Eve's minute-latency reconciliation and daily roster check as a second, independent evaluator. Containment is K0 on both action services in under a minute, then K4 and K5 — revoke the grants and suspend the account — by the on-duty human super admin. An access token already issued lives up to 60 minutes after suspension, so the desk watches that hour and pre-stages K6, a human super admin removing Super Admin from the robot, which is the switch that survives a token already minted. There is no machine-invocable account stop (P16, proposed none now), because an automatic lock-out is also a path a compromised monitor could pull.
...
Once the Organization Administrator path is taken, an approval can be forged or a kill undone inside the tenant's organisation. What has already left — the export, the alarm, the rota records, objects in locked buckets — cannot be reached: no witness principal holds a grant in the tenant's organisation, and the witness's super admins are not the tenant's. The end state that removes the forgery path is P15, proposed.

### An interactive login on the robot — RB-02

After bootstrap nobody signs in to `walle@` or `eve@`, so SA-04 is severity 1 with no triage question. Nothing enforcement-grade refuses the sign-in: the Admin console access level is friction, not counted (P7, below). Key custody is the prevention; response is K0, ending sessions, K5, and a physical accounting of both keys. An interactive session matters more than a token because it opens the console, and the console opens the path into the GCP organisation.

### Self-silencing of monitoring — RB-04, with RB-05 for an innocent outage

A super admin can stop in one console visit the stream every rule reads, so the design refuses to treat silence as calm. The Workspace stream's absence alarm (H-1, 60 minutes in business hours, 240 outside) lives in Eve's project and in the witness, and on firing sets `log_pipeline_silent`: no autonomous work for every agent Eve verifies and `halt_all` on both Wall-E lanes, cleared only by one human call that is itself a report about the administrator ([heartbeats](../07-monitoring-detection-incident-response.md#7-pipeline-heartbeats-and-the-log_pipeline_silent-halt)). SA-03 catches the posture change by any actor; SG-02 to SG-04 catch sink, audit-configuration, policy and project changes and may pull K7 automatically, dry-run until the SIEM's identity holds the invoker grant. The halt is the enforcement; everything else sees. The event names for the sharing and export switches are unverified, so until observed the absence alarms are the detector.

### An out-of-catalogue super-admin action, or a human authority anomaly — RB-06

The stop for the robot is the hard-denied list, refused in every lane with a breaker trip and a severity-1 page, and band B's rule that uncatalogued super-admin work needs a requester and a different approving human super admin. The stop for human super admins is narrower: Workspace multi-party approval on the settings it covers. SA-01 fires on any robot method outside the committed allow-list not claimed by a band B audit row within five minutes; SA-02 on any role creation or `makeAdmin` by anyone; SA-06 on the daily roster diff; SA-09 on control-group membership; SG-05 on privileged access outside a change window. When the actor is the administrator, the second human is the sole recipient of the report and a different super admin suspends the actor's role pending review. Two colluding human super admins remain a residual: they could do the same work without the robot.

### A compromised CI pipeline or deployer, a forged approval, image drift — RB-07

Anyone who can ship a revision of the credential holder bypasses every control in its code at once ([known structural weaknesses](../../wall-e/ARCHITECTURE.md#11-known-structural-weaknesses), weakness 12). The stops are a deploy grant that is never standing (a one-hour entitlement approved by a second reviewer), Binary Authorization admitting only attested images, and Eve's approval key held where no Wall-E principal can sign with it. Detection is SG-07, PL-06 and Eve's `bad_approval` finding. Post-deploy drift on Cloud Run is detection-grade, because continuous validation exists only for GKE (Chapter 13, Supply chain, keys and recovery). The residual stays: a compromised pipeline is a compromised action service, and under Super Admin that is a tenant compromise.
...
Chapter 16, Eve, the independent controller, owns the explanation ([when Eve is compromised](../../eve/06-failure-modes.md#when-eve-is-compromised)). For the threat model: the attacker gains halt and demote freely — a denial of service a human clears in seconds — and signing only for cells already marked binding, within the envelope's lifetime, still subject to hold windows, vetoes, budgets and Wall-E's full per-item policy re-run. It cannot raise a level, execute a Workspace write or change its own thresholds. Detection is more than three Eve demotions in an hour (severity 2), the blind human sample, the SIEM's independent evaluation of the same rules, and Mo never taking numbers about Eve from Eve's verdicts alone. Containment is a human revoking Eve's invoker grants on Wall-E's services and disabling — never destroying — the key version. A forged approval carrying a valid signature puts the key path in question and is run under RB-07. The named residual is false verdict receipts, bounded only by the blind sample.
...
No organisation policy can require registration, so the shadow-agent control is detection-grade and says so. Daily reconciliation against independent inventories finds an agent with no register row; a factory-bypass project loses its invoker grant, and the agent cannot be published or called through the platform because the front door's egress allow-list is generated from the register ([reconciliation](../05-registry-and-autonomy-contract.md#6-reconciliation-against-independent-inventories); Chapter 9, Registry, governance and the autonomy contract). A silent registered agent at Tier P is severity 1 within an assumed 15 minutes. Third-party agents, MCP servers, apps and models enter only through the supplier procedure, L0 for writes and tainted (P138, proposed; Chapter 14, Scale and AGI readiness). Only a card written outside CI has a runbook, RB-06.

## What a compromise of each project reaches

"Compromise" means project-owner rights or any identity in the project. The topology page states the reach of each concrete project ([compromise reach](../../project-topology.md#12-what-a-compromise-of-each-project-reaches-afterwards)); the classes are drawn here.
...
Three points carry the argument. First, separate projects turn a policy into a structure: the credential holder has no principal in the project that holds Eve's key, so a compromised Wall-E project cannot mint an approval, though only against the project's IAM. Second, the platform HLD adds the Tier P-SA row: interactive use of the robot account or a `cloud-platform` consent reaches the tenant and the GCP organisation, and the witness is not reachable; the console mechanism by which a super admin reaches Organization Administrator is an Assumption to verify before the grant ([trust boundaries](../01-hld.md#15-trust-boundaries--the-template-every-agent-inherits)). That row lists the Admin console access level among the preventions, while the HLD's list of what the platform does not do and register row P7 keep that level out of every safety case; this brief follows the second. Third, the reach of the core projects is not yet written in the topology's form, although HLD §15 requires a per-tier table; the brief states only what their detections treat as severity 1.

## Facts kept out of the safety case

Four facts are named in the design as not counted in any safety case until answered ([what the platform does not do](../01-hld.md#16-what-the-platform-does-not-do)):
...
Other Google facts unverified on 2026-09-13 are carried where they bite ([HLD sources](../01-hld.md#19-sources)): the witness's Cloud Identity edition (P14, open, a grant gate); whether a delegated admin can suspend a super admin (P16); the tenant's Workspace edition, which decides one feed; Google's per-service TISAX coverage (P32, open; R-05); the event names of the sharing and export switches (SA-03); registry Data Access logging, where Google's two pages disagree and the canary's first registry row decides (P80); and the "records nothing" list of super-admin views that leave no event, Assumption per capability until verified before the grant. Two operational values stay Assumption until drilled: that running Cloud Run instances refuse new requests under the service-denial lever, and the acknowledgement and K5 targets.

## How each risk theme of Chapter 3 ends

| Theme | Answered in | Residual that remains | Disposition |
|---|---|---|---|
| Tenant compromise through the robot credential | Chapters 8, 11, 15 | A leaked token or interactive login is still a tenant compromise with a path into the GCP organisation; a token survives up to 60 minutes after suspension; the single process and the pipeline stay single points | **Accepted** with compensation as R-01 (signatures pending); the thirteen compensations hold the grant shut until green |
...
Residual tables inside each design page are not a register, and TISAX control 1.4.1 wants one with owners and acceptance. The security reviewer keeps it, the ISMS mirrors it under its own ids, and it is refreshed at every stage decision and management review ([risk register](../11-tisax.md#10-the-risk-register-p140)). Likelihood and impact are scored one to three, an Assumption until the ISMS maps its own scale, so the register is legible before that mapping exists. Every row names a treatment (accept, mitigate, transfer, avoid), an owner and, for accepted rows, who signs and where.

Row one is the super-admin deviation, R-01: likelihood 1, impact 3, accepted with compensation, to be signed by the security reviewer and entered by the ISMS in the P33 record ([decision record](../../../decisions/2026-09-13-wall-e-holds-super-admin.md)); the signature is pending on 2026-09-14. The two sources disagree on the owner: page 11 §10 names the platform owner, the P33 record names the security reviewer, as Chapter 21, TISAX, records. How the deviation is kept honest is Chapter 21, TISAX. Rows R-02 to R-17 run from the one-person organisation and the unbuilt witness to single-region operation and model monoculture. Four rows — the one-person organisation, the unbuilt witness, the missing restore, penetration-test and audit records, and the unstarted data-protection artefacts — score likelihood 3 "today" and are mitigated by gate: each closes when what it describes exists.

The rule that makes the register bite: an accepted row without its signed acceptance file at the TISAX assessment order stops the order. P140 is proposed.

## Residual risks knowingly accepted in Wall-E's design

Wall-E's design records its accepted residuals separately from its defects ([accepted residuals](../../wall-e/14-hld-challenge.md#residual-risks-knowingly-accepted)). They are: two volume questions open until the pilot's account count is recorded; the action service as a single process, narrowed by the asymmetric Eve key, a create-only plan store and the per-endpoint allowlist; resource-level grants that whoever edits Wall-E's project IAM can rewrite, bounded by drift checks; a compromised action service laundering a forged trigger, narrowed by corroboration against Google's log; attribution of a chat requester resting on a record the action service writes; a catalogue that never covers an administrator's long tail, now carried by bands B and C; and irreversibility as a matter of degree, since a sent mail cannot be unsent.

Each is reopened when its premise moves ([reopen when](../../wall-e/14-hld-challenge.md#reopen-when)): the account count is recorded; a second or non-super-admin operator is added (fired on 2026-09-13, answered for band B); the agent is shared beyond operators; Eve's design is set against these residuals (outstanding since 2026-09-12); an F3 or F3b cell is proposed to leave L1; Google ships an acting console assistant or a no-code admin flow under a non-user principal; a deprovisioning connector appears; the model pin, agent framework or Model Armor filter version changes; Agent Gateway's `europe-west1` availability is settled; cost exceeds its estimate by a factor or the Stage 1 review falls due; or a severity-1 event or false automatic demotion occurs.

## The residual risks a sponsor accepts by signing

In order of consequence, in plain language, for Chapter 1 to summarise:

1. **A stolen robot credential is a takeover of the Workspace tenant and a route into the cloud organisation.** It is met by key custody, detection within minutes and people pulling switches, not prevented; a token can keep working for up to an hour after the account is suspended. To be signed by the security reviewer as R-01; pending, and the role is unfilled on 2026-09-14.
2. **Inside the organisation, the controller watches a super admin but cannot structurally stop one.** A super admin who reaches the cloud organisation could forge an approval or undo a kill; only the evidence, alarm and pager held in the separate witness organisation are beyond reach, and the end state that closes the gap is proposed, not built.
3. **The safety of the design depends on people who do not exist yet.** One administrator holds every role on 2026-09-14; the tiers that need a second person stay closed until that person exists, which is a delay, not a control failure.
