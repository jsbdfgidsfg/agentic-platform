...
It answers three risk themes of Chapter 3, The problem and its risks, at the level of structure: tenant compromise through the robot credential, a controller reachable by what it controls, and manufactured silence. Where each element the objective names is designed is the traceability of Chapter 2, The objective and what it demands; the terms enforcement-grade and detection-grade are Chapter 4, The principles.
...
**The front door.** One Gemini Enterprise app per tenant, in `GEMINI_PROJECT`, fronts every human-facing agent. It signs the human in with the tenant's Google identity, and its Data Access log is the one record that names the human; downstream the human travels as a surrogate in the correlation keys, never as a credential. Agents no human talks to, such as Eve's control path and Mo's jobs, are not published in it (Chapter 6, The Gemini Enterprise environment).

**The tenant egress gateway.** The app reaches agents through `gemini-egress`, whose access policy is generated from the register, so an unregistered engine is unreachable from the front door by network rather than by convention. Agent Gateway supports the app only in egress mode, so the front door is fenced on its way out; the fence refuses nothing until the last of its three stages (P57, a spike; staging in Chapter 6).

**The agent's ingress gateway.** Every engine a machine calls, the tenant app included, sits behind its own project's ingress gateway with a Model Armor extension that fails closed. The screen sits where the agent's owner cannot remove it and where its failure stops the call. The gateway and Model Armor form an availability domain with an `Assumption:` 99.5 % monthly objective; an outage fails every fail-closed agent closed, by design (Chapter 10, Agent Gateway, Model Armor and the perimeter).

**The reasoning layer.** The engine runs under Agent Identity, a keyless Google-issued principal with a daily certificate. It holds no credential, the folder deny policy refuses it every secret, and its egress gateway's default-deny allow-list names its action service and no Workspace or Admin SDK host. Tool arguments are generated text: whatever an injected document persuades the model to request, it can only request it from deterministic code. Cloud Run reasoning services carry a dated service-account exception until Agent Identity for Cloud Run leaves Preview (P5, open; Chapter 8, Identity, privileged access and the fleet kill switch).

**The action service.** From Tier W, a Cloud Run service in the agent's project is the only holder of the credential to the system of record. It takes a named operation with parameters, checks the catalogue, policy chain and autonomy ladder, writes a write-ahead audit row, calls the API and verifies by re-reading. Here a model's proposal becomes, or fails to become, an action. Wall-E has two such services so its broad scopes never share a process with the catalogue (Chapter 15, Wall-E, the doer).

**The system of record.** A business system for a Tier W agent; the Workspace tenant for a Tier P agent. For the Super Admin robot Google refuses nothing beyond the consented OAuth scopes, so the action service is the only gate and detection is the primary control for that tier ([P33](../../../decisions/2026-09-13-wall-e-holds-super-admin.md)).
...
- **The evidence plane**, two aggregated sinks into `LOGGING_PROJECT` with locked buckets, the evidence lake in `CORE_PROJECT`, Eve's own Workspace sink and the SIEM feeds, holds Google-written records where the agent and its owner cannot delete them (Chapter 11, Monitoring, detection and incident response; Chapter 12, Data, logging, retention and sovereignty).
...
- **The fleet kill switch K7** is pulled from `CORE_PROJECT` by a human through PAM or by a deterministic job on a severity-1 SIEM rule, never a model. It stops every agent principal in the tier folders and spares the controllers, core and tenant-app folders so Eve keeps paging (P70, proposed; Chapter 8).

### What is enforced and what is only detected

Every layer mixes both grades, and the design names which is which ([HLD §0.2](../01-hld.md#02-the-six-containment-primitives-and-how-each-is-graded)). At the **front door**, sign-in and the access levels on control surfaces are enforcement; nightly matching of console-made Tier C agents to register rows is detection. At the **gateways**, the egress allow-list and the fail-closed ingress extension are enforcement, while the Model Armor project floor on the model's own calls fails open and is detection. The constraint binding each engine to its gateways counts as enforcement only after a throwaway engine has been refused in nonprod; until then a CI check stands in, graded detection. In the **reasoning layer** the deny policy and the Principal Access Boundary (PAB) are enforcement, but the deny policy's principal-set spelling is an `Assumption:` until proven (P8, a spike) and the constraint forcing Agent Identity is a spike (P4), so that half is a CI check. In the **action service**, catalogue, policy chain, ladder and hard-denied list are enforcement because they sit outside the agent's process and fail closed, yet they are code in the component under audit; invocation is IAM-only until the perimeter spike passes (P3). At the **system of record** under Super Admin, the scopes and Workspace multi-party approval over role assignments (P66, proposed) are the only refusals Google makes; the SIEM's super-admin set and Eve's minute-latency reconciliation are detection. **Beside the path**, the verifier is enforcement by the absence of a model, the evidence plane is detection with an absence alarm, and K7 is enforcement, with its first lever's effect on running Cloud Run instances an `Assumption:` measured in the monthly drill.

## 5.2 Trust boundaries B1–B7

The seven boundaries are Wall-E's five generalised plus two fleet boundaries. Every agent's design page lists all seven with the platform mechanism, what the agent's code adds and the grade, so no owner can quietly drop one and hundreds of agents can be compared on the same seven lines ([HLD §15](../01-hld.md#15-trust-boundaries--the-template-every-agent-inherits)).

**B1, human and reasoning layer.** The model sees the request, never the credential; the human travels as a surrogate. Platform: Gemini Enterprise sign-in and request log, `gemini-egress`, Model Armor on the tenant, Identity-Aware Proxy with Context-Aware Access on control surfaces. Agent: its prompt-security design. Enforcement, from Tier C. The identity the app passes is asserted, not cryptographically bound, so an action service re-checks group membership on every write.
...
**B3, action service and system of record.** The only credential holder; scopes as the Google-enforced ceiling; every write pre-stated, verified and audited. Platform: keyless service account, regional secret, IAM-only invocation and then internal-only ingress once P3 passes, the perimeter. Agent: operation implementations and inverses. Grade: enforcement by scope, plus detection at minute latency for a super admin, because the Super Admin decision left this boundary nothing Google enforces except the scope split. From Tier W.
...
Arrows run from the principal's home project to the resource carrying the grant; the dotted arrow is a named exception (section 5.4). On 2026-09-14 every project is planned except `GEMINI_PROJECT`, which exists (`Assumption:`) and is to be imported (Chapter 6).

### One folder, folder classes, one project per agent

Every platform project sits under `fld-agentic-platform` in the tenant's GCP organisation, in `europe-west1` with BigQuery in `EU`, made by the factory and never by hand after Stage 0. The folder carries what Google enforces once: the organisation-policy baseline, deny policy, PAB bindings, Model Armor folder floor, aggregated sink, Data Access audit configuration and PAM entitlements. Below it, folders by class let one class be stricter without per-project exceptions: no model API on the controller's authority path, no secrets for the improver, an exact service list for the super-admin singleton, an empty allow-list for the closed Tier X. The tree, and why Google forces one project per agent, are Chapter 7.
...
A principal in one project reaches a resource in another only through a grant on that resource (a service, job, dataset, topic, key, bucket or engine), never through a project-level role in the other project. A project-level role is lateral reach by definition: when the three agents shared a project, its owner could grant itself signing on Eve's approval key. Separate projects with resource grants make the absence structural, because the credential holder has no principal where the key lives. Where Google offers no resource-level form, the grant is dropped or becomes a named exception; absences that matter are anti-grants asserted by the drift job. BigQuery shows the pattern: a dataset-level reader in the data project plus a query-job role at home, so no project-level data viewer exists anywhere. The rule is charter demand D7 of Chapter 4 ([project-topology §3.1](../../project-topology.md#31-notes-the-table-cannot-hold)).

### The named exceptions

A few platform principals serve the whole fleet. They are listed completely, with level and maker, and each is in Eve's drift job's expected set, so any further foreign principal in `EVE_PROJECT` still fails:
...
Each is acceptable because it belongs to the platform, holds a read-only role or none standing, lives where no agent principal exists, and sits on Eve's anti-grant list.

### The grant table is a factory input

[Project-topology §3](../../project-topology.md#3-cross-project-grants) is the complete table: 46 rows with principal, resource, level, maker and source, rows 45–46 proposed with an `Assumption:` until Eve's E-21 is signed. Each row is made in the resource's project by its owner; the factory takes the row as module input, and credential-path bindings are applied in the factory's privileged phase by a human through PAM. Nothing crosses on the strength of a sentence elsewhere, and after Stage 0 no grant is hand-made. Some rows await verification, notably whether an engine-scoped query role suffices for the tenant app across projects (row 1).

### What the rule does not bound

Every row bounds a service account or group; none bounds `walle@`, which can reach Organization Administrator. The table keeps an uncompromised Wall-E from Eve's key; a compromised credential is answered by detection and the witness (Chapter 19, Threat model and residual risk).

## 5.5 What is shared in platform-core and what stays per agent

Share what must be single to be true: one inventory, one evidence copy, one custodian, one supply chain, one kill switch outside every agent project. Keep per agent whatever is a trust boundary or would turn one compromise into all (P45, proposed; [page 02 §5](../02-landing-zone-and-tiers.md#5-platform-core-what-is-shared-what-stays-per-agent-p45-p46)).

Shared: the register and shared Agent Registry, evidence lake, drift, reconciliation and K7 jobs in `CORE_PROJECT`; sink destinations and locked buckets in `LOGGING_PROJECT`; the supply chain in `CICD_PROJECT`; the validator in `VALIDATOR_PROJECT`. The Model Armor template standard, monitoring baseline, schemas and the hostname set every egress policy needs are shared as modules instantiated per project; gateway hostname matching is exact, so one versioned list is the only way one change fixes the fleet.

Per agent: engine and gateways; action services, OAuth clients and secrets, since agents sharing a secret project share a credential; control plane and `ladder.yaml`; audit dataset; Model Armor templates and project floor, as quota is per project; verifier keys, since a shared approval key is one key to steal.

### The shared approval surface, refused (P46)

One shared approval page was a candidate and is refused (P46, proposed). The page must verify the exact request hash against the action service that will execute it, so a shared page would be a write path into every credential holder: one compromise, every agent. Only routing is shared, as one audience-group pattern and one token-verification library; the cost is one surface per Tier W or higher agent. Page 04's PAB rules list the approval surface among the core resources an agent may reach ([page 04 §4.3](../04-identity-and-privileged-access.md#43-the-policy-and-its-grade--p9-answered)), which reads against its per-agent placement in [page 02 §5](../02-landing-zone-and-tiers.md#5-platform-core-what-is-shared-what-stays-per-agent-p45-p46).
...
Chapter 19 carries what remains: a super-admin credential ignores folder and grant table alike; Eve's control path is detectively independent inside the organisation until P15; the PAB for Cloud Run invocation, the Admin console access level (P7) and the deny-policy spelling (P8) stay out of every safety case; the front door's network fence against unregistered engines is not in force until `gemini-egress` enforces.
