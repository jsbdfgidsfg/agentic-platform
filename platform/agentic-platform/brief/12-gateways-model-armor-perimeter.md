...
Hostile instructions arrive with what a run reads, not with how it started. Wall-E's design inventories every channel through which attacker-writable text reaches a model ([wall-e 11 §1](../../wall-e/11-prompt-security.md#1-the-injection-surface)), and every Tier W and higher threat model fills in the same template: the operator's prompt, documents, tool results (anyone who can set a display name writes to the model), the robot's mailbox, peer agents' output and the session history.

No prompt can be a boundary (Chapter 19, Threat model and residual risk). The answer is structural: the model holds no credential, and a run that reads attacker-writable content is tainted and capped (Chapter 15, Wall-E, the doer). Screening adds a block in front of the model on the one channel screenable before it runs, evidence joined to the audit row, a flag for the approver, and sensitive-data catches in both directions. It never carries a safety case: a classifier Google calls possibly inaccurate cannot guard a credential that can suspend an employee.
...
Every Agent Runtime engine above Tier C is created bound to its project's egress gateway ([06 §2.1](../06-gateways-model-armor-perimeter.md#21-the-rule-as-the-factory-and-the-folder-enforce-it)). Two folder custom constraints refuse an engine bound elsewhere, and daily reconciliation treats an unbound engine as severity 2, unpublished within one business day. Google allows one gateway of each kind per project-region, one of the facts forcing one project per agent (Chapter 7, Landing zone and the tier model). Bound engines must postdate 2026-04-29 and cannot have revisions, so promotion is an attestation.
...
The allow-list is generated from the register row and manifest, never hand-written ([06 §2.2](../06-gateways-model-armor-perimeter.md#22-the-egress-allow-list-is-generated-never-hand-written)): the agent's own action service from Tier W, the essential Google endpoints (one versioned value, since wildcards are unsupported), and the exact hostnames of declared MCP servers and read APIs, each with a supplier row. What matters most is what is absent: Secret Manager, Firestore, BigQuery, the Admin API, every Workspace host and every other agent's action service, a second enforcement point for "the agent can read no secret". Nonprod gateways start in dry run to inventory what an agent really calls; prod enforces from the first deploy (a per-agent prod dry run was rejected as an unscreened window per agent), and a destination seen only in nonprod is a finding against the manifest, never a reason to widen prod. `gemini-egress` is the one exception (Chapter 6, The Gemini Enterprise environment). Kill switches, halt reads, reconcilers, sinks and task deliveries never pass a gateway, so an outage never disables stopping an agent.

### Ingress: machine-called engines, fail-closed, `streamQuery` only

An engine is machine-called when anything other than a human in the tenant app may invoke it, as the register row's invoker list says ([06 §2.3](../06-gateways-model-armor-perimeter.md#23-the-ingress-gateway-who-is-machine-called-and-the-streamquery-rule)). Tier R agents published only in the app are egress-bound only; Tier W and above always have both gateways. The ingress gateway's Model Armor runs with `failOpen` false and a one-second timeout. It screens only `streamQuery` of ADK engines, so every machine caller must use `streamQuery`: CI checks callers, the dispatcher library offers nothing else, and a `query` call is severity 3 because it silently removes the only enforcement-grade prompt screen.
