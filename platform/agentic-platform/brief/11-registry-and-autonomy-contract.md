...
By the end of this chapter the reader knows why the platform's inventory of agents is a YAML file in git rather than a Google product, how an agent that bypassed that file is found, and what an agent must pass before it reaches production. They know how autonomy becomes versioned data that only humans raise, on evidence a machine recomputes, and that anything with standing lowers in seconds, and what that contract costs in independence at Tier W.
...
- **Independent observation**: what Google's services say exists — Cloud Asset Inventory over the platform folder, Security Command Center AI Protection's AI asset inventory over the organisation, and the Gemini Enterprise app's agent list. No agent owner writes any of it.
- **The manifest**: `agent-manifest.yaml` in the agent's repository, stating what the agent is allowed to be — families, ceilings, egress, peers, stores, fingerprint — hashed into the register row.
...
Four facts, re-verified on 2026-09-13, stop Agent Registry from being the record ([registry page §2.1](../05-registry-and-autonomy-contract.md#21-the-google-facts-the-topology-rests-on-re-verified-2026-09-13-11); [platform HLD §5.1](../01-hld.md#51-the-inventory-of-record-is-a-git-file-not-a-product)). Its IAM is **project-level only**: no policy exists on an entry, so a viewer sees every entry and an editor can alter every entry, including the `readOnlyHint` and `destructiveHint` annotations Google itself warns about. **Automatic registration lands only in the runtime's own project**, so a central inventory needs manual cross-project registration, Google's documented pattern for a governance project. **No organisation policy can require registration.** And the SCC AI asset inventory **holds no owner and no classification**, the first things a TISAX assessor or the EU AI Act asks for.

Two more facts shape the details. The `Service` resource has no labels or annotations field. And the agent framework treats a card fetched from the registry as trusted configuration, skipping a same-origin check, so a registry write carries the weight of a deploy. A product like that is a good view and a poor record.

### One shared registry, written only by CI (P71)

Per-agent registries, per-tier registries and one shared registry were weighed. **P71, proposed**, chooses one shared registry in `CORE_PROJECT`, `europe-west1`, since manual registration is refused in the `eu` and `us` multi-regions ([registry page §2.2](../05-registry-and-autonomy-contract.md#22-options-and-the-decision-p71)). `CORE_PROJECT` is already where no agent principal holds any role, the only place a project-level registry role is safe; per-agent registries would have made every owner an editor of their own discovery record.

Agent projects never enable `agentregistry.googleapis.com`: it is absent from every tier folder's service allow-list, so automatic registration has nowhere to land. That rests on an `Assumption:` that automatic registration needs the API in the runtime's project, which Google does not state. A nonprod spike at the first factory run tests it, and tests that an engine and its egress gateway still work with the API disabled; if either fails, a CI-generated working-set registry per agent project is added by a new dated register row. The one other registry by design is `gemini-registry` in `GEMINI_PROJECT`, the tenant gateway's working set, never an inventory. The default quota is 100 entries per type per project; an increase is requested at 60 % occupancy, is not guaranteed, and per-tier-folder registries are the fallback. Humans, the detection desk, Eve's owners and the `gemini-egress` generator read the registry; no agent principal does.
...
A CI schema check fails the merge on a missing mandatory field ([registry page §3.2](../05-registry-and-autonomy-contract.md#32-mandatory-fields-per-tier)). The groups matter more than the list: an immutable `agent_id` keying every audit row and ladder file; a declared intended purpose signed by the owner's name in the merge commit, the EU AI Act anchor; owner group and cost centre; tier, risk class, data, TISAX and AI Act classes (TISAX class confidential by `Assumption:` until the organisation's scheme exists); model pin, gateway and agent principal; from Tier W a named grader with declared hours, a `verifier_owner` outside the administration line of the agents verified, the manifest hash and contract version. `privilege` is `none` below Tier P, and CI fails a second `super_admin` row while one is not retired. Tier X rows exist only to be refused: ceiling pinned at L0, never published.
...
With no labels on the entry, **P72, proposed**, writes a fixed first description line — `meta:` with `agent_id`, `tier`, `owner`, `risk_class`, `ai_act_class`, `tisax_class`, `status` and `register_sha`, the hash of the merged row ([registry page §3.1](../05-registry-and-autonomy-contract.md#31-where-the-metadata-lives-p72)). "Card differs from row" becomes one string comparison: the row wins and CI rewrites; a rewrite by anyone but CI is a severity-2 write alert first.

## Finding shadow agents

### The sources

The job compares register and registry with three observed sources ([registry page §6.1](../05-registry-and-autonomy-contract.md#61-the-sources-and-what-each-is-verified-to-provide)). Cloud Asset Inventory lists reasoning engines, Cloud Run services and Gemini Enterprise engines, exported daily from the folder and followed by a feed; the registry is not an asset type, so it is read directly. Whether the SCC AI asset inventory can be exported or read by API is unverified, so a human reads it weekly until proven. The Gemini Enterprise listing method is *tbd*; meanwhile the app administrator files a nightly console export and the gap is recorded.

The job is a second schedule of the drift job under `platform-drift@CORE_PROJECT` (P73, proposed): a full run daily at 06:00 Europe/Paris, an incremental run within 15 minutes of a feed event, SQL only. Reusing that principal avoids a second folder-level exception; two added role names, the asset viewer and the SCC findings viewer, are unverified until the factory run.

### What each difference means

An engine, agent-shaped service or app agent with no row is a **shadow agent**: severity 2 at Tiers C to W, severity 1 at Tier P, unpublished within one business day ([registry page §6.2](../05-registry-and-autonomy-contract.md#62-the-set-differences-severities-and-the-action-per-difference)). If its project has no `agent` label the factory was bypassed, and the job cuts the project's invoke or query grant; otherwise an automated pull request creates the row as `suspended`. A registry entry with no row is a shadow card, which CI deletes. A console agent with no row is a shadow Tier C agent; a builder's second occurrence removes them from the builders group. An asset SCC sees outside the folder gets no automatic action: agents living under the folder is policy, not constraint, and this is the check that finds the exception. The same run finds `prod` rows with nothing behind them, overdue reviews, silent agents, peer-binding drift, gateway-unbound engines and Tier W verifiers without a recent seeded-fault record. Results are insert-only for 400 days and go to the SIEM; while a Tier P row exists, the job's heartbeat also reaches the witness organisation.

### Why detection-grade is acceptable

No organisation policy can require registration, so the control is **detection-grade, stated as such**. It suffices because what an unregistered engine needs to be useful — a `gemini-egress` entry and a query grant — is generated from the register, and a custom constraint refuses an engine without a gateway. A shadow agent can exist; it cannot be published or called through the platform.

## Governance policies

The registry policies make "only CI writes" a mechanism ([registry page §4](../05-registry-and-autonomy-contract.md#4-governance-policies)). The routine factory identity alone holds the registry admin role; editor and user roles go to nobody; repair is a one-hour PAM entitlement the security reviewer approves. Every write not made by CI with a pipeline run id alerts, drilled monthly by a nonprod console edit that must page within five minutes. Reads are logged under P80, proposed; the product's audit-logging page documents those logs while Google's general services list omits them, so they count as unverified until the canary's first run (same section; Chapter 12, Data, logging, retention and sovereignty). Cards are generated and refused if a skill names a write, approval or control operation. Annotations are rendered from the manifest's risk tier, and the action service has no registry client, so nothing in the registry changes what an agent may do. Bindings come only from `peers[]`.

**Semantic Governance never sits on an authority path.** It is Preview, Google warns its verdicts may be inaccurate, and it removes proposed tool calls inside the reasoning layer. A gateway may use it as an extra refusal, which costs nothing when wrong; it never approves, its verdict is never evidence, and the audit schema has no column for it. Wiring it into approval is refused at admission, or severity 2 and reverted if found later.

## Admission and revocation

### The admission gate

Admission is one CI run in `CICD_PROJECT` whose code owners are the platform owner and IT security ([registry page §7.1](../05-registry-and-autonomy-contract.md#71-admission-end-to-end)). After the human steps — row and manifest written, reviewed for the tier, merged — the pipeline continues only if the baseline is applied, the row validates with its classes and signed purpose, the image is attested or the bundle hash recorded, the card registers, the compliance entry exists, the first heartbeat reaches the tier's desk within 30 minutes (Chapter 11), the gateway is bound, the floor conformant and egressor bindings match `peers[]`. From Tier W it also needs a restore drill, a validator-signed manifest and a drill record younger than 30 days, which the detailed step names the seeded-fault run ([platform HLD §5.3](../01-hld.md#53-governance-policies-and-the-admission-gate); [registry page §7.1](../05-registry-and-autonomy-contract.md#71-admission-end-to-end)). Only then does CI grant the app's service agent query rights on the engine alone, add the `gemini-egress` entry and set `pilot`; sharing to named groups is a human act through PAM, and who registers the agent in the app is a point on which two canonical pages disagree (Chapter 6, The Gemini Enterprise environment). A failed step stops everything after it. `pilot` to `prod` is a human pull request whose scorecard the validator recomputes.

### Revocation and suspension

Retirement is the factory's `revoke`: share, `gemini-egress` entry and query grant removed in that order, Tier W and above halted, registry entry deleted, evidence export confirmed, project deletion scheduled at 30 days, and the **TISAX 5.3.3 return-and-removal row** written ([registry page §7.2](../05-registry-and-autonomy-contract.md#72-revocation-and-suspension)). The row stays in git as `retired`, the platform's history of agents. Suspension, which any operator may trigger, removes reach, halts Tier W and above and deletes nothing. A revocation or suspension not confirmed within 15 minutes is severity 1, then a manual project cut-off, then the fleet kill switch on the tier folder, which leaves the registry intact so recovery knows what to restore (Chapter 8).

## The compliance registers are exports

The TISAX agent register and the EU AI Act register are CI exports of the register, regenerated on every merge and never hand-edited, so neither can drift from what runs ([registry page §8.1](../05-registry-and-autonomy-contract.md#81-the-tisax-agent-register-isa-131-132-533)). The TISAX export carries owner, classification, location, suppliers, privilege, stores, review and removal records for ISA 1.3.1, 1.3.2 and 5.3.3, with protection need derived by the ISMS's mapping as an `Assumption:`. An assessor sampling live projects repeats the reconciliation job's work, so its zero-difference report is attached.
...
Wall-E's ladder is adopted unchanged as the platform's ([Wall-E ladder §1](../../wall-e/05-autonomy-ladder.md#1-the-eight-rules)). Autonomy belongs to a pair of operation family and trigger class, never to an agent. Humans raise, machines lower. One notch, one family, on evidence. Reversibility outranks risk tier. Every level is enforced in the action service, and the agent never learns why it was refused. Every write is verified by re-reading. There is no silent level. Evidence, not calendar: time alone promotes nothing.
