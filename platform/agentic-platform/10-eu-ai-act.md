...
- **Art. 26 as deployer of the app**: use per Google's documentation; the conversation store
  is not the platform's Art. 12 log (03 §5.4, P52); retention is the DPO's (P13; interim 30
  days).
...
| Art. 5 | No prohibited practice | §3.1 negative determination, dated; re-run per family addition | legal | E-01 | part of the Art. 6(4) file; the admission gate refuses a register row or a new family without a dated negative determination | no dated determination: the row or family is refused at admission; a practice found prohibited after admission: the family is removed by `revoke` of the playbook, the row `suspended`, and legal opens an Art. 73 assessment |
...
| Art. 14 / Art. 26(2) | Human oversight designed in; overseers with competence, training and authority; automation-bias measure; stop | §4.6: the ladder as the oversight design; named overseer roles; level caps per family; the K0 drill as the Art. 14(4)(e) test; operator self-grading as the automation-bias control | agent owner, Eve owner, ISMS (roster) | E-08 | drill records; roster review quarterly | a roster review overdue or a K0 drill missed in the quarter: the affected families are capped at L3 by the ladder validator and no stage opens until the review or drill is recorded; a named overseer without the training record is removed from the approver group by the factory |
...
| Art. 16 | Provider obligations list | this table is the map; (b) identity on the system = the agent card and the Gemini Enterprise description; (l) accessibility of the operator surface *tbd* | AI compliance owner | E-05 (the stage snapshot, which carries this table) and E-14 | the stage-snapshot checklist lists each Art. 16 point (a)–(l) with its evidence id or `n/a` and a reason | a point with neither evidence nor a reasoned `n/a`: the stage snapshot is not tagged and the stage does not open; (l) accessibility stays *tbd* and is recorded as an open residual in §6 until decided |
| Art. 17 | Quality management system, proportionate to the organisation's size | §4.9: the ISMS as the QMS host; the wiki's decision records, CI gates, the ladder, Mo and the incident process mapped to Art. 17(1)(a)–(m); **built only if a row is `high_risk`**, kept ready otherwise | ISMS, platform owner | E-14 | annual internal audit slot (HLD brief 79) | an audit slot missed: severity 3 finding to the ISMS; a row reclassified `high_risk` while the QMS is not built: the row is `suspended` until the QMS mapping is signed |
| Art. 18 / Art. 47 | Keep technical documentation, QMS documentation and the declaration ten years | 08 R11: decision records, compliance snapshots, drill records — **10 years**, locked | platform owner | E-05 | bucket lock; drift job | lock removed or retention shortened: severity 1 to `platform-security@`, the pipeline re-applies (a locked retention cannot be shortened, so the finding is a new bucket without lock); a missing record is an evidence gap in the register of §5 |
...
| Art. 26(1) | Use per instructions | the instructions page is the operators' page; band B and C exist so that out-of-catalogue use is *documented* use | agent owner | E-07 | the monthly out-of-catalogue report (band B and C use) reviewed by the agent owner; the verifier refuses plans the manifest does not cover | use outside the instructions: the verifier refuses the plan; repeated band-C use of one pattern in a month is a finding to the agent owner, who either adds the family through a pull request or narrows the instructions |
| Art. 26(5) | Monitor; inform the provider of risks and serious incidents | provider = deployer, so internal; the desk's checklist ends every severity-1 case with the Art. 3(49) question (07 RB-10) | incident commander | E-10 | case flags; the monthly SOC metric counts severity-1 cases closed without the `art73_assessed` flag | a severity-1 case closed without the flag is reopened by the desk and assessed by the AI compliance owner within one business day; a pattern is a severity-2 finding against the incident process |
...
| Art. 27 / Art. 26(8) | FRIA; public-authority registration | `Assumption:` private organisation, not providing a public service, no Annex III 5(b)/(c) system — **do not apply**; entity and date recorded at P23; revisited if a subsidiary in scope is a public-service provider | legal | E-01 | annual review | review overdue: severity 3 to legal; a subsidiary found in scope: FRIA required before the next stage opens for any system that subsidiary deploys |
...
| Chapter V | GPAI | §3.5 one-liner; supplier rows | platform owner, legal | E-11 | supplier file; annual supplier review ([11](11-tisax.md) §8) | provider documentation absent or out of date at review: severity 3 to legal; a new model pin without it is refused by the re-qualification check (P124) |
