...
- Objective restated 2026-09-13: Mo measures Eve as well as Wall-E
  ([../agentic-platform/01-hld.md](../agentic-platform/01-hld.md) §13.3, §18 item 20); every
  grant that crosses a project is a row of
  [`../project-topology.md` §3](../project-topology.md#3-cross-project-grants).
...
- **None of it exists, and the ownership is split by project.** Mo's accounts are never
  ...
  [07-build-runbook.md](07-build-runbook.md), together with every in-project grant. Each
  cross-project grant is made by the runbook that owns the resource — Wall-E's, Eve's, the
  factory or the validator custodian — never by Mo's, and is listed with its maker in
  [`../project-topology.md` §3](../project-topology.md#3-cross-project-grants) (§2 below gives
  the row numbers). **No BigQuery read grant for Mo exists today**: `add_dataset_access` is called
...
decision is reversed: drop it from Phase 13b (an edit to `walle_setup.py`, which gains
...
| Write access to anything in `walle_audit` — or to anything else in `WALLE_PROJECT` | **None** | `mo-metrics@` holds `dataViewer` there, never `dataEditor`. Mo's only write targets are inside `MO_PROJECT`: `walle_metrics`, `walle_metrics_archive`, `walle_metrics_private`, `walle_metrics_views` and the drop box |
| Write access to anything in `EVE_PROJECT`; any read on `eve.grades_blind`, `eve.review_queue_blind`, `eve.incidents.narrative` or any other `eve` dataset than `eve_quality`; an invoker on any Eve service; any grant on the `eve-approval` key | **None** | `mo-metrics@` holds one `READER` there, on `eve_quality` only, which exposes authorised views with no free-text column. Mo is never Eve's grader and nothing Mo writes reaches Eve except a pull request a human merges into `eve/config` |
...
| Any **project-level** role in `WALLE_PROJECT`, `EVE_PROJECT` or `GEMINI_PROJECT` | **None** | The topology's one rule. Mo's only bindings outside `MO_PROJECT` are resource-level: in `WALLE_PROJECT` a dataset access entry on `walle_audit` and a Cloud Run service binding on `walle-actions`, plus, at S3, one on the linked Spans dataset; one dataset access entry on `eve_quality` in `EVE_PROJECT`; one on `platform_logs_views` in **`LOGGING_PROJECT`** (topology row 40, made by the factory), which replaces the `walle_workspace_logs` entry; and a proposed one on `eve_grades` in `VALIDATOR_PROJECT` (topology row 46, E-21 of Eve's set). Asserted by MD-9b and by Wall-E's drift job ([`../project-topology.md`](../project-topology.md) §3 row 26) |
...
One row per in-project grant, with the resource it is scoped to. "Exists today" is `no` for
every row. Cross-project grants are listed by their row in
[`../project-topology.md` §3](../project-topology.md#3-cross-project-grants), which carries
their level, maker, reason and verification; none of them exists today either.
...
| `roles/bigquery.dataEditor` (dataset `WRITER`) | dataset `${MO_PROJECT}:walle_metrics_views` — views only, no tables | `MO_PROJECT`, Mo's runbook (Mo-2) | The agent-facing authorised views ([§3](#3-the-authorised-view-layer)) can be created and replaced under the pinned identity rather than a human's. This write is also why a view is never authorised on `walle_audit`: `mo-metrics@` can redefine every view in this dataset | no |
...
Cross-project, for `mo-metrics@` — each dataset-level, made in the resource's project by its
owner, never by Mo's runbook, with the job in `MO_PROJECT`:

| Read | Topology §3 row | From |
|---|---|---|
| `${WALLE_PROJECT}:walle_audit` — T0 computes every metric from `actions`, `runs`, `plans`, `approvals`, `verifications`, `config_versions`, and the new `grades`, `proposal_verdicts`, `drills` and `ladder_events` tables | 6 | S0 |
| Google's admin events: dataset `${LOGGING_PROJECT}:platform_logs_views`, which replaces the former `walle_workspace_logs` read in `WALLE_PROJECT` (P104, P107) | 40 (and 6) | S0 |
| `${EVE_PROJECT}:eve_quality` — the Eve quality pack ([03-metrics-contract.md](03-metrics-contract.md) §7.3) and the `metric_divergence` check; [M-11 · 52](08-open-decisions.md) read (d). The validator custodian's separate `READER` on it (P30) is not Mo's | 28 | Eve's observe-and-report layer |
| The linked Spans dataset (`_AllSpans`) in `WALLE_PROJECT`, after a human holding `roles/observability.editor` there creates the link at [07-build-runbook.md](07-build-runbook.md) Mo-10; unverified, [M-11 · 52](08-open-decisions.md) (b) | 7 | S3 |
| `${VALIDATOR_PROJECT}:eve_grades` (`grades_eve`), proposed with E-21 | 46 | Wall-E's Stage 1 |
...
| `roles/storage.objectCreator` | bucket `mo-proposals`, in `MO_PROJECT` | `MO_PROJECT`, Mo's runbook (Mo-9) | Write one proposal bundle per object. Create only: no read-back, no overwrite, no delete | no |
| `roles/run.invoker` | service `walle-actions` in `WALLE_PROJECT` — cross-project, service-level | Topology §3 row 8 | The weekly `plan_hash` recomputation through the two read endpoints. SETUP Phase 10's invoker loop covers `$SA_AGENT`, `$SA_DISPATCH`, `$SA_EVE`, `$SA_ACTIONS` and `$SA_OPS_CALLER` today — **no Mo principal**, so this binding is new even though [ARCHITECTURE](../wall-e/ARCHITECTURE.md) §7.5's allowlist row is not | no |
| ~~`roles/agentregistry.viewer`~~ | ~~project `${WALLE_PROJECT}`~~ — **removed** | Topology §3 row 11 (anti-grant) | Retired unless a verified resource-level binding replaces it (§6) | **no**, and must stay no |
...
`walle_metrics` and `walle_metrics_views` ([§2.1](#21-mo-metrics--the-only-identity-that-reads-raw-rows)) and can therefore `CREATE OR REPLACE` these views. A view authorized on
...
The absent column is the second of the six mechanisms that hold the code/model boundary; all
six, and the one place the split is a process boundary inside T1 rather than an IAM boundary,
are in [01-hld.md § The deterministic boundary](01-hld.md#the-deterministic-boundary-and-how-it-is-enforced).
...
**Mo reading Eve does not change this.** The edge runs one way: `mo-metrics@` reads
`eve_quality` in `EVE_PROJECT`. Nothing crosses back — no Eve identity, and not the validator
custodian's `READER` on `eve_quality`, is a binding in `MO_PROJECT` — so the assertion below
and MD-9b hold as written, and Eve's drift job must list `mo-metrics@${MO_PROJECT}` on
`eve_quality` as an expected foreign principal and nothing else of Mo's.
...
Each claim on this page is falsifiable only if a denial test exercises the grant that is absent:
the model tier selecting raw or free-text columns, the reporting tier reading `walle_audit` or
calling a control endpoint, a bundle overwritten or read back, an enforcement identity reading
Mo's datasets, a Mo identity reaching a secret, and Mo reading Eve beyond `eve_quality`. Every
BigQuery test submits its job in the identity's home project and names tables fully qualified,
so a 403 comes from the resource rather than from a missing `bigquery.jobs.create`. The tests,
`MD-1` to `MD-15` with their commands and expected results, are
[07-build-runbook.md § Phase Mo-12](07-build-runbook.md#phase-mo-12--the-denial-tests-authored-from-mos-side).
...
Wall-E's runbook gains only the cross-project grants on its own resources (the `walle_audit`
read and the `run.invoker` on `walle-actions`, keyed on a new `MO_PROJECT` config key), the full
`mo-analyst@${MO_PROJECT}` email in the read-endpoint allowlist, the containment denial row,
the `MO_PRINCIPAL` resolution with Phase 13b's registry grant removed, and corrections to the
team contract and to ARCHITECTURE weakness 11. Eve's runbook gains `eve_quality` and Mo's
`READER` on it. The full list, with targets and references, is
[08-open-decisions.md § What this design forces on Wall-E's set](08-open-decisions.md#what-this-design-forces-on-wall-es-set)
(changes 3, 9–12, 15, 19 and 20).
...
Cross-project claims, verified 2026-09-13:
