...
- Objective restated 2026-09-13; see the platform HLD
  ([../agentic-platform/01-hld.md](../agentic-platform/01-hld.md) §13.3, §18 item 20). Mo now
  measures Eve as well as Wall-E: `mo-metrics@` gains one cross-project, dataset-level `READER`
  on `eve_quality` in `EVE_PROJECT`, made by **Eve's** runbook, with no binding in `MO_PROJECT`
  (§2.1, [M-11 · 52](08-open-decisions.md) read (d)). The drop box is renamed `mo-proposals`.
...
- **None of it exists, and the ownership is split by project.** Mo's accounts are never
  ...
  and 8, §7.1). **Added 2026-09-13:** Eve's runbook owns, the same way, **exactly one
  cross-project grant on Eve's resources** — dataset-level `READER` on `eve_quality` for
  `mo-metrics@${MO_PROJECT}` — and makes no binding in `MO_PROJECT` in return (platform HLD
  §13.3, §18 items 17 and 25). **No BigQuery read grant for Mo exists today**: `add_dataset_access` is called
...
live, and — since 2026-09-13 — `EVE_PROJECT` is where `eve_quality` lives; a principal here
...
| Any **project-level** role in `WALLE_PROJECT`, `EVE_PROJECT` or `GEMINI_PROJECT` | **None** | The topology's one rule. Mo's only bindings outside `MO_PROJECT` are three resource-level ones in `WALLE_PROJECT` — two dataset access entries and one Cloud Run service binding — plus, at S3, one on the linked Spans dataset, and since 2026-09-13 one dataset access entry on `eve_quality` in `EVE_PROJECT`. Qualified 2026-09-13 (review-findings pass): the `walle_workspace_logs` entry moves to one dataset access entry on `platform_logs_views` in **`LOGGING_PROJECT`** (topology row 40, made by the factory), and a proposed one on `eve_grades` in `VALIDATOR_PROJECT` (topology row 46, E-21 of Eve's set), so Mo's foreign bindings are in `WALLE_PROJECT`, `EVE_PROJECT`, `LOGGING_PROJECT` and, once E-21 is signed, `VALIDATOR_PROJECT` — each resource-level. Asserted by MD-9b and by Wall-E's drift job ([`../project-topology.md`](../project-topology.md) §3 row 26) |
...
| `roles/bigquery.dataViewer` (dataset access entry `READER`) | dataset `${WALLE_PROJECT}:walle_workspace_logs` — **cross-project, dataset-level** | `WALLE_PROJECT`, by Wall-E's owner, the same way | **Moved 2026-09-13 (P104, P107; [../project-topology.md](../project-topology.md) row 40):** the logs read is no longer on `${WALLE_PROJECT}.walle_workspace_logs` — Wall-E's organisation sink is deleted and the surface becomes the authorised view `platform_logs_views.walle_workspace_logs` in `LOGGING_PROJECT`, over `platform_logs`, filtered to `admin.googleapis.com`; `mo-metrics@` holds dataset-level `READER` on `platform_logs_views`, made by the factory's `platform-core` module, not by Wall-E's runbook. Audit completeness joins Google's org-level admin events to Wall-E's requester and approver rows. Queried from BigQuery, never from org-level Cloud Logging, whose `_Default` bucket is fixed at 30 days for organisations | no |
...
| `roles/bigquery.dataViewer` (dataset access entry `READER`) — **added 2026-09-13** | dataset `${EVE_PROJECT}:eve_quality` — **cross-project, dataset-level**; the dataset holds Eve's Mo-readable surfaces (`findings`, `verdicts`, `attestations`, `pages`, `incidents` minus narrative, `seeded_fault_runs`) through authorised views with no free-text column, and never `grades_blind` or `review_queue_blind` | `EVE_PROJECT`, by the Eve owner from **Eve's runbook** (platform HLD §18 item 17) — never Mo's runbook, and **no binding of any kind in `MO_PROJECT`** in return | T0 computes the Eve quality pack ([03-metrics-contract.md](03-metrics-contract.md) §7.3) and the `metric_divergence` differential check. The validator custodian holds a second, separate `READER` on the same dataset (P30), which is how the gate re-derives Mo's Eve numbers; that grant is not Mo's. This is read (d) of [M-11 · 52](08-open-decisions.md); the new topology row is platform HLD §18 item 25. `Assumption:` `eve_quality` is `EU`, like every Mo dataset — a cross-location join fails outright — confirmed by Mo-2's verify block | no |
...
**Unchanged by Mo reading Eve (2026-09-13).** The new edge runs one way: `mo-metrics@` reads
...
numbered `MD-1` to `MD-15` (MD-15 added 2026-09-13). The identity-shaped ones are listed here, with that number, because
...
| MD-8 | `mo-analyst@` writes a second object over an existing bundle name in `mo-proposals` | **Fails.** `objectCreator` does not permit overwrite | §1 — a duplicate bundle fails rather than replacing |
...
| MD-15 (added 2026-09-13) | **Paired.** (a) `mo-metrics@` selects from a view in `` `${EVE_PROJECT}.eve_quality` ``, from a job in `MO_PROJECT`. (b) `mo-metrics@` selects from `eve.grades_blind` and from the source `eve` dataset; `mo-analyst@` and `mo-narrator@` select from `eve_quality` | **(a) succeeds; (b) 403 accessDenied**, every time | §1.1, §2.1 — Mo reads Eve's quality surfaces through `mo-metrics@` only, and never Eve's blind grades |
...
**And on Eve's set (added 2026-09-13, change 20).** One identity-shaped change: Eve's runbook
creates `eve_quality` and makes the dataset-level `READER` for `mo-metrics@${MO_PROJECT}` (and,
under P30, for the validator custodian); Eve's Phase 9 check "no `userByEmail` entry for
`mo-metrics@` on `eve` — ever" is narrowed to the non-quality datasets; Eve's drift job expects
that one Mo principal on that one dataset. Targets: [../eve/07-build-runbook.md](../eve/07-build-runbook.md),
[../eve/02-identity-and-auth.md](../eve/02-identity-and-auth.md) — owned by the Eve owner
(platform HLD §18 item 17).
