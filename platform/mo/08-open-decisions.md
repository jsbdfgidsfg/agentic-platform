...
The second is the **seventeen changes Mo forces on Wall-E's set**. Three of them are
...
| **M-6** · 47 | **The pilot OU account count (Wall-E decision 5).** | Mo's scorecard has no denominator. At an unknown and possibly small volume, the coupled floor of 35 decided items and a 30-day window may never both be satisfiable for a low-volume family. | Size it before S2. Mitigate by measuring the promotion sample **since dwell start** rather than over a fixed 30 days. Write down the honest consequence before it is argued about: some cells sit at L2 or L3 permanently, and the scorecard reports `insufficient_data` rather than pretending. | Before S2 | [05-staging.md](05-staging.md), [03-metrics-contract.md](03-metrics-contract.md); Wall-E [decision 5](../wall-e/09-open-decisions.md) |
...
- **43 decides whether L4 exists in practice.** Strict scoping is correct and brutal: the
  sample resets on any fingerprint change, `n` falls below 35, and the scorecard says not
  ready with the reason `evidence earned under fingerprint X, current Y, n=4`. That is an
...
Seventeen changes, ordered by whether they block Mo entirely. Each row names the file to edit
...
| **2** | New table `walle_audit.ladder_events`: one row per effective-level transition, with `ts`, `family`, `trigger`, `from_level`, `to_level`, `origin ∈ {human, operator, eve, breaker}`, `reason_code`, `incident_ref`, `decision_ref`, `config_version`, `override_epoch`, `review_verdict ∈ {pending, upheld, false_positive}`, `review_ref` | [`../wall-e/03-lld.md`](../wall-e/03-lld.md) storage; [`../wall-e/SETUP.md`](../wall-e/SETUP.md) Phase 7 | C45 / C46 | This is what C45/C46's "halts, overrides and breaker demotions logged as rows" has to become. Automatic demotions deliberately write no decision file, and `config_versions` is not stated to gain a row for them, so **dwell, the ratchet, two-demotions-in-90-days, more-than-three-demotions-in-an-hour and the false-positive review are all uncomputable today** by anything that reads BigQuery. Extending `config_versions` was considered and rejected: a ladder event is not a config version, and the review verdict has no home there |
| **3** | BigQuery grants for Mo's three service accounts, plus the `walle_metrics` and `walle_metrics_archive` datasets and the authorised-view registration on `walle_audit`'s access list | [`../wall-e/SETUP.md`](../wall-e/SETUP.md) Phases 7 and 13b; [`../wall-e/setup/walle_setup.py`](../wall-e/setup/walle_setup.py) `add_dataset_access` | B1 | The shared data plane [08](../wall-e/08-team-eve-mo.md) promises is **entirely unbuilt**. `add_dataset_access` is called exactly twice today, for `walle-actions@` and the `walle-audit-bq` sink writer, and for neither Eve nor Mo. Every grant in [02-identity-and-access.md](02-identity-and-access.md) is a build task, not a check |
...
shows up in the scorecard rather than as a quietly wrong number. Changes **9** to **17** are
document and runbook edits, cheap individually, and **15** is the one that a security
reviewer should refuse to sign without.
