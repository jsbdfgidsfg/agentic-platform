...
- Objective restated 2026-09-13 (platform HLD
  ...
  proposes changes to Eve, and Wall-E holds Super Admin.
...
Two structural properties from Mo's [thesis](01-hld.md#thesis) carry the weight: **Mo's
absence is strictly more restrictive than its presence** — the mirror of what
[08](../wall-e/08-team-eve-mo.md) item 7 requires of Eve, applied to the one member of the team
with no enforcement duty — and **every number Mo publishes is re-derivable, and the gate
re-derives it rather than believing it** (for numbers about Eve, under the source rule and only
once P30's grant exists). A compromised or hallucinating Mo can write a persuasive paragraph but
cannot produce a false number that survives CI, which is why most of the rows below end in "and
nothing moves" rather than in a compensating control.
...
lists name none of them, and that is tested as a row in the denial suite that checks both
the 403 and the policy (change 15 in [08-open-decisions.md](08-open-decisions.md), authored in
[07-build-runbook.md](07-build-runbook.md) as MD-9 and MD-9b). It is also the only mechanism behind
[M-1](08-open-decisions.md), which asks whether `mo-metrics@` may write anything at all: the reading this
...
The same holds for Eve: **Eve reads nothing Mo writes**, so a Mo outage leaves
Eve's thresholds, fast drop to advisory, halts and reporting exactly as they were; what is lost
is Mo's view of Eve's quality and the Eve proposals.
...
| **An automatic demotion turns out to be a false positive** | The ratchet applies in full, with no exception. The demotion is not silently absorbed: `review_verdict = false_positive` with a named reviewer and a `review_ref`, and a published breaker false-positive rate. When it rises, the finding is about Eve or the metric, and Mo's proposal targets the metric definition, never the ladder — or an Eve threshold through the closed Eve types ([04-artefacts-and-proposals.md](04-artefacts-and-proposals.md) §3.5) |
| **Mo's numbers about Eve are wrong, or only Eve vouches for them** | Assertion A10 refuses to mark any Eve figure evidence-eligible whose sources include none of `grades_eve`, `seeded_fault_runs`, golden-replay results or Wall-E's `eve_last_seen`, so a compromised Eve writing false receipts cannot become Mo's evidence. A11 diffs Mo's ten metrics against Eve v0's and flags `metric_divergence` as an Eve finding. Until the custodian holds `READER` on `eve_quality` (P30) no Eve figure is evidence and every Eve bundle is advisory. **Residual:** the `self_reported` figures (pages, acknowledgement times) are Eve's account of itself, labelled as such |
...
Neither is Mo's to fix, and Mo does not route around either. Without the write-ahead
`walle_audit.grades`, `proposal_verdicts` and `drills` tables, plan precision and drill
freshness are not computable at all; without `walle_audit.ladder_events`, dwell, the ratchet,
the demotion budgets and the false-positive review are not — the tables, their columns and why
Firestore reads or a dwell guessed from `config_versions` are refused are changes 1 and 2 in
[08-open-decisions.md](08-open-decisions.md#blocking--mo-computes-almost-nothing-without-these).
The correct behaviour while they are missing: say `not computable`, report no cell ready, and
let the gap be visible in every weekly digest until someone builds the tables.

### 3.3 Fingerprint churn may make L4 unreachable, and that is a decision, not a bug

Promotion evidence is scoped to the current ten-field fingerprint and the demotion denominator
is not — the tuple, the asymmetric rule and why a fingerprint bump can never empty the demotion
evidence are [03-metrics-contract.md](03-metrics-contract.md#11-attribution-the-fingerprint-and-the-scoping-rule)
§11. The failure consequence: taken strictly, a prompt edit on a Tuesday resets the promotion
sample for that cell, so at a realistic change cadence a cell may never hold 35 decided items
under one fingerprint and L4 becomes unreachable in practice. The design does not soften the
rule: Mo publishes both the strict count and a proposed material-subset count from S2, and the
choice is [M-2](08-open-decisions.md), due before the first L4 promotion is argued; a coarser
subset needs its own decision record and the security reviewer, because it reopens exactly
what C15 closed.

### 3.4 A false-positive demotion is reviewed, never absorbed

The ratchet ([../wall-e/05-autonomy-ladder.md](../wall-e/05-autonomy-ladder.md#6-who-may-raise-who-may-lower)
§6) applies in full, with **no false-positive exception**; a demotion later judged wrong is
recorded and counted by the false-positive review of
[03-metrics-contract.md](03-metrics-contract.md#93-the-ratchet) §9.3, and its finding targets
the metric definition, an Eve finding or an Eve threshold proposal — **never the ladder**. That
is the shape the Eve obligation E35 requires, and it matters because the alternative (an
exception path in the ratchet) converts every argued-about demotion into a negotiation.

The one thing the verdict changes is the `demotions_90d` count that sets `redesign_required`
([03-metrics-contract.md](03-metrics-contract.md) §8); the hold, the incident note and the fresh
decision record still apply, and the row stays in `ladder_events` forever. That stops the
residual false-demotion rate compounding unattended against a threshold of two, and the
published false-positive rate is the visibility control on anyone reaching for the verdict too
often.
...
Six things this design does not close. None of them is mitigated by another control listed
...
calculation — carry a golden fixture each ([03-metrics-contract.md](03-metrics-contract.md)
...
to one human ([M-7](08-open-decisions.md)).
...
**R6 — Mo improving Eve concentrates authorship on the pilot.** Eve's own
limit 5 already says the same human may write `thresholds.yaml`, approve `eve_authority:
binding` and grade the blind sample. Mo authoring Eve threshold diffs adds a fourth hand to that
list. The reviewer rule of [04-artefacts-and-proposals.md](04-artefacts-and-proposals.md) §3.6 —
Eve's second reviewer is neither the ladder owner nor Mo's CI operator — closes it only when
those are different people; until the second human outside the Wall-E administration line
exists (platform HLD §13.2), `eve_threshold_loosen` is not mergeable and the residual is that
Eve's thresholds do not improve through Mo at all.
...
| A number about Eve is evidence-eligible without an independent source, or Mo and Eve v0 disagree | A10 fails the run; A11 sets `metric_divergence` on the Eve scorecard | [03-metrics-contract.md](03-metrics-contract.md) §6, §7.3 |
...
- [08-open-decisions.md](08-open-decisions.md) — changes 1, 2, 11, 13, 14 and 15, and open decisions M-1 to M-7
