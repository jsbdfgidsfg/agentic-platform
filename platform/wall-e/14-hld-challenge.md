...
service in the same project — not a second Agent Runtime engine, because what is under test
is deterministic Python. The factual lens corrected the cost of the separate-tenant option
...
| **A separate evidence project with a project lien** | Puts the promotion evidence outside Wall-E's teardown reach. | One more project and one more IAM boundary to maintain. | **Deferred to Stage 1**, with the cheap half (write-ahead grades to BigQuery, teardown guards) done now — new decision 31 (C46). |
...
| **Eve's trust root stays inside Wall-E's project through the pilot** | C11 was refuted on the ground that this belongs to Eve's design and decision 18, before S4. | The pilot is capped at L3, where Eve gates nothing. The three missing drift rows are carried as a residual edit, and moving Eve's evidence copy out of Wall-E's reach becomes a requirement at S3, when the acceptance test starts relying on it. |
