...
| [ ] | D11 | What does Stage 0 actually provision? | Phase 1 | Phase 15, the KMS key, the Eve allowlist entries and `walle-events` have no consumer yet. Stage 0 should provision what Stage 0 uses. | [09](09-open-decisions.md) decision 36 |
...
| [ ] | **The folder decided before Phase 6, and the project created inside it.** `gcloud projects create` takes `--organization` **or** `--folder`, not both, and no phase moves the project afterwards | The Phase 12c conformance floor is pinned at `folders/${FOLDER_ID}` and, per Google, folder-level floor settings apply only to projects inside that folder. A project parented straight to the organisation inherits nothing from it, and `./walle armor` still succeeds (§10 item 20). | `gcloud resource-manager folders list --organization="$ORG_ID"`, then after Phase 6 `gcloud projects describe "$PROJECT" --format="value(parent.id)"` equals `$FOLDER_ID` and `gcloud projects get-ancestors "$PROJECT"` shows the organisation above it. If Phase 6 has already run with `--organization`, move it: `gcloud beta projects move "$PROJECT" --folder="$FOLDER_ID"`. | Phase 6 (decision), Phase 12c (`./walle armor`) | org admin |
...
## 5. GCP projects (the M0 host project, and `$PROJECT`)
...
| [ ] | Two **separate** Desktop OAuth clients, one for the robot and one for Eve, never reused, under the project's own Internal, In-production consent screen | One client, one token. Google invalidates the oldest refresh token without warning once one Google Account holds more than 100 live refresh tokens for a single OAuth client ID, and a separate, larger limit applies per account across all clients. External plus Testing expires refresh tokens after seven days. | Two client IDs recorded in the build log. Both marked Trusted (§3.2). | Phase 9 (M5, M6), Phase 15 | you |
...
| [ ] | Whether the Gemini Enterprise app lives in `$PROJECT` or in another project, and Discovery Engine read access on that project for your own account if it is another one | `./walle register` addresses the app as `projects/$PROJECT/locations/$GEMINI_APP_LOCATION/collections/default_collection/engines/$GEMINI_APP_ID`, using your own `gcloud` access token. [12](12-agent-identity.md) §2 assumes the app is **not** in Wall-E's project. If it is elsewhere, the app read only warns, the D8 location check is skipped with it, and the M8 verifier returns SKIP — you confirm the registration and the app location by eye instead. | `gcloud projects describe <gemini-app-project> --format="value(projectId,projectNumber)"`; `roles/discoveryengine.viewer` on that project for `OPERATOR_EMAIL` | Phase 13 (`./walle register`) | you |
...
13. **Registry viewer holders disagree.** [SETUP.md](SETUP.md) Phase 13b prose says "viewer to the readers". Its command grants `roles/agentregistry.viewer` to `eve-controller@` only. The script grants it to `eve-controller@` and `MO_PRINCIPAL`. Both say "nobody else" in prose, and [SETUP.md](SETUP.md) names "an editor" as the reason, but neither writes `roles/agentregistry.editor` or `roles/agentregistry.user`, so the excluded role names appear nowhere an operator can check them against.
...
15. **Discovery Engine service agent project number.** [SETUP.md](SETUP.md) Phase 12 builds the service agent from `$PROJECT_NUMBER`, Wall-E's project. [12](12-agent-identity.md) §7 says it is the Gemini Enterprise app project's number. There is no config key for the app project number. `./walle register` and the M8 verifier also assume the app is an engine in `$PROJECT`, so both the app read and the D8 location check silently degrade when it is not.
...
| — | Gemini Enterprise Admin (`roles/discoveryengine.agentspaceAdmin`), checked on the app's User permissions tab (§3.1); app location `eu` or `global` (D8) |
