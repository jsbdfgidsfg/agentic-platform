...
- Placement: the inventory below is `WALLE_PROJECT`'s only; [../project-topology.md](../project-topology.md)
  is the authority for every grant that crosses a project.
- Scope: the robot holds Super Admin ([02](02-identity-and-auth.md)), under the platform HLD
  ([../agentic-platform/01-hld.md](../agentic-platform/01-hld.md) §12, §13.1 and §18 items 2 and 10);
  reversals of design decisions are dated in place.

Written so that someone other than you could build it.

## GCP resource inventory

| Resource | Name | Notes |
|---|---|---|
| Project | `WALLE_PROJECT` (tbd; `PROJECT` inside the setup script and runbook) | Wall-E only: one `reasoningEngine`, its three services (`walle-actions`, `walle-actions-super`, `walle-dispatcher`), five secrets, Firestore, `walle_audit`, `walle_workspace_logs`, the topics. Under `FOLDER_ID` beside `GEMINI_PROJECT` (the app), `EVE_PROJECT` and `MO_PROJECT`. Nothing of Eve's, Mo's or the app's is created here — [../project-topology.md](../project-topology.md) §2. |
...
| Service account | `walle-actions-super@` | Runs `walle-actions-super`. Only reader of the **broad** credential secrets. Reads nothing of `walle-actions@`'s. |
| Service account | `walle-agent@` | Runs the agent. Reads no secret. |
| Service account | `walle-dispatcher@` | Invokes the agent. |
| Cross-project bindings on Wall-E's resources | `eve-controller@EVE_PROJECT`, `mo-analyst@MO_PROJECT`, `mo-metrics@MO_PROJECT`, `service-GEMINI_PROJECT_NUMBER@gcp-sa-discoveryengine` | No Eve, Mo or app identity is created here, and every grant to them is resource-level, never a project-level role (anti-grant row 26). The bindings on Wall-E's resources are rows 1 (`walleEngineQuery` on the engine), 3 and 8 (`run.invoker` on `walle-actions`), 27 (`walle-actions-super`, halt path only; no Mo identity), 36 (`platform-drift@CORE_PROJECT`: per-service `roles/run.invoker` on the `/v1/control/halt` endpoint of `walle-actions` and `walle-actions-super`, resource-level and generated from the manifest, carried in that row's P73 extension beside its folder-level `roles/iam.securityReviewer`; basis platform HLD §18 item 25), and 4, 6, 21 and 34 (dataset-level readers) of [../project-topology.md](../project-topology.md#3-cross-project-grants) §3. |
| Cloud Run | `walle-actions` | Band A. Auth required. Ingress: see the note below. |
| Cloud Run | `walle-actions-super` | Bands B and C. Auth required, same ingress rule as `walle-actions`, own service account, own secrets, same policy library and hard-denied list from the same commit. No Scheduler or Pub/Sub target. |
...
| Secret Manager | 5 regional secrets in `europe-west1`: `walle-oauth-client`, `walle-refresh-token` (narrow, read by `walle-actions@`), `walle-super-oauth-client`, `walle-super-refresh-token` (broad, read by `walle-actions-super@`), `walle-confirm-hmac` | See [02](02-identity-and-auth.md). Eve's secrets and key are in `EVE_PROJECT`, not here |
| BigQuery dataset | `walle_audit`, EU | On the platform **`audit.schema`** (§"Storage"): the eight contract tables plus the agent extension table `generic_requests`, insert-only, partitioned by day, with expiry. Cross-project readers hold **dataset-level** `roles/bigquery.dataViewer`, never project-level, and their jobs run and are billed in the reader's project ([../project-topology.md](../project-topology.md#3-cross-project-grants) rows 4, 6, 21, 34). **No Mo authorised view is ever authorised on this dataset** — Mo's views in `MO_PROJECT.walle_metrics_views` are authorised on `walle_metrics` inside `MO_PROJECT` ([../project-topology.md](../project-topology.md) §3 row 9, an anti-grant) |
...
**Ingress note.** Internal-only ingress blocks the agent, because Agent Runtime egresses from a
Google-managed tenant project that Cloud Run counts as external; the routes that would admit it
and the folder rule that applies `run.allowedIngress` once spike 1 passes are
[../agentic-platform/06-gateways-model-armor-perimeter.md §4.3](../agentic-platform/06-gateways-model-armor-perimeter.md#43-credential-holders-are-never-internet-reachable-as-a-folder-rule)
(facts in §4.1). A PSC *interface* on the agent is not the answer on its own: `run.app` traffic
keeps taking the Google network path unless private DNS peering is added, and enabling it also
removes the agent's internet egress (verified September 2026).

**So IAM is the enforced boundary**, and it has to do more work than it appears to.
`run.invoker` is granted **per service, not per path**, so granting it to the
...
is narrowed below `run.invoker`. The agent therefore does hold IAM on the control endpoints,
and only the allowlist keeps it off them. Two mechanisms for `walle-actions`, both required,
and a third for the second service:
...
1. **A per-endpoint caller allowlist inside the service**, keyed on the verified identity
   ...
   SPIFFE id for an agent identity, whichever the spike in [12](12-agent-identity.md) shows.
   It is application code doing platform work, so it is printed in full below
   (§"The allowlist of `walle-actions`, per endpoint") and tested per row rather than described.
   In short: execute and plan endpoints `walle-agent@` only; control and approval endpoints
   Eve's identities and `walle-operators@`, never the agent; read endpoints `mo-analyst@` and
   `eve-console@`, never the control list — [../project-topology.md](../project-topology.md) §3.1.
2. **Operators need a binding at all.** Grant `roles/run.invoker` to `walle-operators@`
   ...
   handle. The platform rule is that no human holds `run.invoker` on an action service
   directly; the CLI path impersonates `walle-operators-caller@` and the
...
3. **The allowlist of `walle-actions-super`**, generated like the first from
   the manifest's `invokers` block and from nothing else. `/v1/execute-generic` and
   `/v1/handoff`: the agent's identity only, with a human `chat` principal only.
   `/v1/generic/{id}/approve`: the `walle-approvals-super` IAP surface's own service account
   (name *tbd*), which forwards the IAP-asserted approver, for tier `SUPER` (a **different** human
   super admin from the requester); the band-A approval surface for tier `WRITE` (one operator), as
   the interface table below and §"Band B" state. `/v1/generic/{id}/veto`: that surface and the
   band-A approval surface. `GET /v1/generic/{id}`: the approval surfaces.
   `/v1/control/halt`: `eve-controller@EVE_PROJECT`, `platform-drift@CORE_PROJECT` and the
   approval surfaces, which carry an operator's halt (no human holds `run.invoker` on this service;
   an operator's halt on `walle-actions` stops this lane too, §"The policy engine") — and for
   Eve, **no other endpoint** on this service. `eve-verifier@EVE_PROJECT` holds the halt
   endpoint too, because Eve's reconciler limb raises the
   super-admin-lane halts ([../project-topology.md](../project-topology.md) row 27; Eve 03 §14);
   it holds no other endpoint here either. Never `walle-dispatcher@`, never `walle-tasks@`, never
   any Mo identity, never `eve-console@`. The denial suite asserts each refusal.

**The allowlist of `walle-actions`, per endpoint.** The foreign emails are written in full
(`…@EVE_PROJECT.iam.gserviceaccount.com`, `…@MO_PROJECT.iam.gserviceaccount.com`) because they are
compared byte for byte. `eve-controller@EVE_PROJECT` and `mo-analyst@MO_PROJECT` hold `run.invoker`
on the whole service, and it is these rows, each with its negative test, that keep Mo to two read
endpoints and Eve off `/v1/execute`.

| Endpoint | Allowed callers, and nobody else | Negative test that must pass |
|---|---|---|
| `POST /v1/execute` | `walle-agent@` (or the agent principal) | Eve and an operator both get 403 |
| `POST /v1/plans` | `walle-agent@` (or the agent principal) | Eve and an operator both get 403 |
| `POST /v1/tasks/item`, the Cloud Tasks worker endpoint | `walle-tasks@`, the OIDC identity of the queue ([02](02-identity-and-auth.md)) | The agent gets 403. This is the most execution-capable path in the system |
| `POST /v1/plans/{id}/approve` | The approval surface's verified human assertion; `eve-controller@EVE_PROJECT` (the manifest's `invokers.approve`) | The agent gets 403 with `approver_is_agent`, a hard invariant; the breaker trips |
| `POST /v1/plans/{id}/veto` | Members of `walle-operators@`; `eve-controller@EVE_PROJECT` | The agent gets 403 |
| `POST /v1/control/halt` | Members of `walle-operators@` (through item 2's caller identity or the approval page); `eve-controller@EVE_PROJECT`; `eve-verifier@EVE_PROJECT`; `platform-drift@CORE_PROJECT` | The agent gets 403; an operator token is **accepted** |
| `POST /v1/control/demote` | Members of `walle-operators@`; `eve-controller@EVE_PROJECT`; `eve-verifier@EVE_PROJECT` | The agent gets 403 |
| `GET /v1/plans/{id}` | `eve-controller@EVE_PROJECT`, `eve-console@EVE_PROJECT`, `mo-analyst@MO_PROJECT`, members of `walle-operators@` | The agent gets 403; `mo-analyst@` gets 403 on every endpoint but `GET /v1/plans/{id}` and `GET /v1/runs/{id}` |
| `GET /v1/runs/{id}` | `eve-controller@EVE_PROJECT`, `mo-analyst@MO_PROJECT`, members of `walle-operators@` | As the row above |
| `GET /v1/operations` | `walle-agent@` | *tbd* (catalogue introspection, from which the agent's tools are generated) |
| `GET /v1/ladder` | Members of `walle-operators@`, `eve-controller@EVE_PROJECT`, `eve-console@EVE_PROJECT`. **Never the agent** | The agent gets 403. The effective matrix tells a steered model exactly which family and trigger pair executes unattended right now, which is reconnaissance rather than information |
| `GET /healthz` | `eve-controller@EVE_PROJECT`, members of `walle-operators@`. The unauthenticated response carries liveness only, never the last-audit-write timestamp | An anonymous caller sees no timing signal |

**The two lists as configured.** The control-caller list of `walle-actions` is one line of Wall-E's
configuration, set and asserted by Wall-E's runbook (CC-30):

```
CONTROL_CALLER_ALLOWLIST="${SA_EVE},${SA_EVE_VERIFIER},${OPERATORS}"
```

Its entries are `eve-controller@${EVE_PROJECT}.iam.gserviceaccount.com`,
`eve-verifier@${EVE_PROJECT}.iam.gserviceaccount.com` and the operators group. The **read-endpoint**
allowlist is a separate list (`eve-console@` for `GET /v1/plans` and `GET /v1/ladder`; `mo-analyst@`
for `GET /v1/plans` and `GET /v1/runs`) and never admits any principal to approve, veto, halt,
demote or execute. Three properties of the control line matter:

- `${SA_EVE_VERIFIER}` is there so that `eve-reconciler` can halt and demote without holding the
  signing role.
- `${OPERATORS}` must be present: a list holding Eve alone leaves **no human able to halt**, which
  the build asserts as a failure. Unverified: under item 2's qualification the CLI caller's token
  carries `walle-operators-caller@`, not a member's email, and how `${OPERATORS}` matches it is
  *tbd*.
- Eve's entries are created at **S3 entry**, not Stage 0: there is nothing for an allowlisted
  principal to call before Eve exists, and an allowlisted principal with no consumer is a standing
  invitation.

The denial tests are exercised from commit one by a **CI-only stub caller** standing in for
`eve-controller@`: the agent denied on `/v1/control/demote` (403) and on approve
(`approver_is_agent`, hard invariant, breaker trips), Eve denied 403 on `POST /v1/execute`, and an
operator token accepted on halt. The stub exists in the test suite only; no fault-injection or
test-mode path is reachable in any admitted image, on either side, and the seeded-fault exercise
uses a separate sandbox deployment and dataset.

**Operator authorisation is not circular, deliberately.** The control and approval endpoints
authorise operators against a **committed operator list carried in the deployed configuration**,
never against a live Directory read, so K0 halt and K1 demote work when the Workspace credential is
revoked, expired or rate-limited, which is exactly when they are most needed. The live Directory
re-check (policy chain step 5) applies to the **write path only**, where failing closed denies an
action rather than removing a brake. The committed list and the group are reconciled daily and any
divergence alerts.
...
a small set of plan, control and read endpoints. **One execution endpoint per lane**: `/v1/execute` on `walle-actions` (band A), `/v1/execute-generic` on `walle-actions-super`
...
| GET | `/v1/plans/{id}`, `/v1/runs/{id}` | `eve-controller@EVE_PROJECT`, `mo-analyst@MO_PROJECT` — both holding `roles/run.invoker` on `walle-actions` from other projects — and operators; `eve-console@EVE_PROJECT` on plans only | Read a plan or run with pre-state |
| POST | `/v1/control/halt` | operator, Eve (`eve-controller@EVE_PROJECT`, `eve-verifier@EVE_PROJECT`), `platform-drift@CORE_PROJECT` | Set or clear a halt mode |
| POST | `/v1/control/demote` | operator, Eve (`eve-controller@EVE_PROJECT`, `eve-verifier@EVE_PROJECT`) | Lower a level. **Refuses any raise.** |
| GET | `/v1/ladder` | operators, `eve-controller@EVE_PROJECT`, `eve-console@EVE_PROJECT`; **never the agent** | Effective levels, config version, halt state |
| GET | `/v1/operations` | agent | Catalogue introspection; the agent's tools are generated from this |
| POST | `/v1/tasks/item` | `walle-tasks@` (Cloud Tasks) | Execute one released plan item through the full policy chain |
| GET | `/healthz` | `eve-controller@EVE_PROJECT`, operators; unauthenticated: liveness only | Liveness plus last successful audit write |

**`walle-actions-super` (bands B and C)**:
...
| POST | `/v1/control/halt` | `eve-controller@EVE_PROJECT`, `eve-verifier@EVE_PROJECT`, `platform-drift@CORE_PROJECT`, the approval surfaces | Set or clear a halt mode on this service (clearing is human-only, as on `walle-actions`) |
...
### `POST /v1/execute-generic` (band B)
...
is exactly what an injection produces. Having the service mint the approval fixes only half of
it: if the agent still posts the approval and names the approver, the service can verify that the
named person is an operator but has no way to verify that they said anything.
...
either the action service cannot verify Eve's signature, or it can also mint one. A random
symmetric secret readable only by Eve makes the Eve-gated level unbuildable, and the obvious
fix at build time, letting the service read it too, would quietly destroy the property the whole
controller role rests on.
...
Trust attached only to *how a run started* misses that injection arrives with *what a run
reads*. Those are independent axes, and conflating them leaves the largest hole in the
design: a **scheduled** run that reads a hostile group name, display name or audit-log
parameter would have a WRITE_HIGH ceiling of L4, while the same text arriving by mail is capped
at L0.
...
Fail-closed behaviour stated only for the audit sink and the protected-principal cache would
leave the rest fail-open in practice, so:
...
<a id="the-three-bands-added-2026-09-13"></a>
### The three bands
...
| Band | Service and endpoint | Credential | Levels | Executes | Who asks, who approves |
|---|---|---|---|---|---|
| **A** catalogued autonomous work | `walle-actions` `/v1/execute`, `/v1/plans` | narrow client | the ladder per (family, trigger); `WRITE_HIGH` never L5; the only band that can ever climb | yes, per level | requester in `walle-operators@` (re-checked live); approvals per the ladder level; second operator at L3 |
| **B** uncatalogued super-admin work | `walle-actions-super` `/v1/execute-generic` | broad client, own service account, own secret, own audit rows | **permanently L3**, `chat` only (`principal.type == human`); tier `SUPER` two-person; every other trigger L0 — code, not config | yes, once, after approval and hold | `WRITE`: requester in `walle-operators@`, one approver. `SUPER`: requester a human super admin, approver a **different** human super admin, change-ticket reference and `hold_minutes` mandatory (§"Band B") |
| **C** console-only work | `walle-actions-super` `/v1/handoff` | none used to write; the broad client reads Reports to verify | none | **no** — a human super admin does the step | the human does it |

Grade of the three bands: enforcement (code), platform HLD §13.1 item 1.

**What each band is.**

- **Band A** is the typed catalogue on the ladder (families F1–F10 and what grows into it), the
  **declared intended purpose** of the system. Its lane keeps the policy chain, the pre-state
  predicate, the inverse, the taint declaration, protected principals, verify-by-re-read and Eve at
  L4, all unchanged.
- **Band B** is a generic Admin SDK request validated against the pinned Discovery document, with
  the method tiered by a committed table, pre-state by the matching `get`/`list`, no inverse (so
  treated as irreversible), the taint bit and every hard-denied row applying, never in any
  `playbook.uses`, and the full canonical request, the Discovery revision and both humans on the
  audit row (§"Band B"). Challenge C35's rejection of a generic Admin SDK proxy
  ([14](14-hld-challenge.md)) is re-argued: this lane loses the inverse, the pre-state predicate
  and the taint declaration of band A, and needs none of them because it is never above L3.
- **Band C is defined by fact, not by list.** The Cloud Identity Policy API mutates only DLP rules
  and DLP detectors (v1) and, in v1beta1 only, the conflicting-accounts provisioning setting
  (verified; source in [01-hld.md](01-hld.md) §"Sources for the 2026-09-13 corrections"); DWD
  client authorisation is Admin console only. Everything the API reads but cannot write is band C:
  2SV enforcement and methods, sign-in challenges, password policy, session control, API controls
  and app trust, Marketplace, data regions, Gmail routing, Drive/Meet/Calendar tenant settings,
  billing, the Transfer tool. The hard-denied list applies to this lane too: a handoff request for
  DWD, for another admin's security settings or backup codes, for the super-admin self-recovery
  setting, or for deletion of any admin or of the tenant account is refused, not turned into console
  steps (§"Band C"); a human super admin who wants those does them without the robot, and Eve's
  tenant-integrity rules see it either way.
- **No browser-driving or computer-use automation of the Admin console under the robot's session,
  ever.** "An interactive login is an incident" is the one control that survives Super Admin, and it
  stays an incident.
...
<a id="band-b--the-generic-lane-discovery-pinned-added-2026-09-13"></a>
### Band B — the generic lane, Discovery-pinned
...
**live, fail-closed** `users.get` `isAdmin` on `walle-actions-super`'s own credential, read-only,
because the band-B lane never holds the narrow token (`a:requester_not_super_admin`);
...
robot as requester or approver is `p:escalation_denied`. Both humans go on the audit row. The live
super-admin check runs for `SUPER` only, never for `WRITE`; it closes decision 28 for this lane, and
band A is unchanged.
...
<a id="the-hard-denied-list-added-2026-09-13"></a>
### The hard-denied list
...
| The control groups — `walle-operators@`, `walle-protected@`, `walle-super-approvers@`, `walle-owners@`, `eve-owners@`, `ge-admins@`, `platform-approvers@`, the `mo-*` groups — and any group whose computed closure reaches one | `p:self_modification_denied` | — |
...
| `users.makeAdmin` (any target, any `status`); any `roleAssignments.insert` of Super Admin or of a role carrying admin-role management; any MPA approval by the robot; the robot as requester or approver | `p:escalation_denied` | MPA for role assignment (`Assumption:` covers `makeAdmin`) |
...
Two notes on the control-group row. `walle-owners@` is on it because the manifest's
`protected_principals` carries it (§"`agent-manifest.yaml` for Wall-E"); the platform HLD's list
does not name it. `walle-super-approvers@`, the band-B `SUPER` approval audience, is on the list and
watched by SA-09, pending the platform owner's confirmation with the P29 signature. The robot is
also on the committed floor list and is a protected principal under its own N7 rule
(§"Protected principals").

<a id="the-tier-super-two-person-list-added-2026-09-13"></a>
### The tier-`SUPER` two-person list
...
(band A). `walle-actions-super` runs the same library: steps 1 (halt), 4
...
4. **Hard-denied list, then protected principals.** The hard-denied list (§"The hard-denied
   list") runs first on the resolved target ids; then the protected-principal
   check. See below.
5. **Principal check.** Humans must be in the operators group, or in `walle-readers@` for
   ...
   trigger class the request claims. In band B this step is the requester
...
6. **Scope.** Target's `orgUnitPath` must be in the family's OU allowlist. Group targets
   must be in an allowed class. This check is **code only** — Workspace does not refuse an
   out-of-scope write at its end, because Super Admin cannot be OU-scoped — so it is tested by the denial suite as a hard-invariant candidate and backed by
...
An item loop inside the request would, at 25 items and a five-per-minute rate limit, exceed
Cloud Run's default 300-second timeout — leaving a half-applied plan, a consumed nonce, and no
terminal state. Deploy the service with an explicit
...
- It ran the admin enumeration under an **organisational-unit-scoped role**, which
  ...
  success. The read must be customer-scoped. **Moot for Wall-E**, whose read as a super
  admin is customer-wide by construction. The defect stays recorded because the same
...
It is the committed roster of [02](02-identity-and-auth.md) §"The roster rule" —
...
| `ladder_events` (contract) | Every raise, lowering, override and requalification | cell, from, to, actor surrogate, decision record, evidence hash |
| `grades` (contract) | Evidence copy of the shadow and blind grades Firestore `grades/` holds live | run, item, grader surrogate, verdict, sample id |
| `generic_requests` (Wall-E extension) | One row per band-B request and per band-C handoff | `request_id`, `a_band`, the **full canonical request** (fields named in the committed redaction list — passwords, backup codes, secrets — replaced by their SHA-256 before storage; the hash of the unredacted canonical form is `request_hash`), the pinned Discovery file's name and SHA-256, the method table row, the pre-state snapshot (redacted like `params_redacted`) and its hash, the handoff `route_id` and watch result |

**Who consumes each table.** Eve and Mo read these tables from their own projects (dataset-level
`dataViewer`, jobs billed in the reader's project); neither reads Wall-E's memory or asks Wall-E
about itself.

| Table | Consumed by Eve | Consumed by Mo |
|---|---|---|
| `actions` | Yes: reconciliation, drift, invariant denials | Yes: precision, cost, regression attribution |
| `runs` | Yes | Yes |
| `plans` | Yes: pre-approval | Yes |
| `approvals` | Yes | Yes |
| `verifications` | Yes | Yes |
| `config_versions` | Yes: config drift | Yes |
| `ladder_events`, `grades` | *tbd* | *tbd* |
| `generic_requests` | Yes: its reconciliation covers band-B rows (§"The correlation contract, applied to Wall-E") | *tbd* |

Beside `walle_audit`, the Workspace audit logs in `walle_workspace_logs` (the second sink) record
what the robot actually did, **written by Google rather than by Wall-E**; both Eve and Mo read them,
and that is what makes Eve independent.
...
`a_screen_state`, `a_ws_unique_qualifiers`; and, for the lanes, **`a_band`**
...
The contract's keys, their origins, the validator check and the gap severities are
[../agentic-platform/07-monitoring-detection-incident-response.md §5](../agentic-platform/07-monitoring-detection-incident-response.md#5-the-correlation-contract).
Every `actions` row, in every lane, carries them; this table is where Wall-E gets each one.
...
Eve's reconciliation, and it covers band-B rows as well as band A.

Retention: row R1 of
[../agentic-platform/08-data-logging-retention-sovereignty.md §5.2](../agentic-platform/08-data-logging-retention-sovereignty.md#52-the-schedule)
([decision 17](09-open-decisions.md)).

The pre-contract layout, in use until 2026-09-13 (history):
...
- The action service's service account gets **insert-only** rights on the dataset. It must
  not be able to delete its own evidence. That is **both** service accounts,
  `walle-actions@` and `walle-actions-super@`, each insert-only; neither can update or delete a
  row the other wrote.
- **The request is evidence, the result is not.** "Never store payloads" still
...
- The dataset lives in `WALLE_PROJECT`. Its cross-project readers hold **dataset-level**
  `roles/bigquery.dataViewer` and run their jobs in their own projects
  ([../project-topology.md](../project-topology.md#3-cross-project-grants) rows 4, 6, 21, 34).
...
|---|---|---|---|
| `control/mode`, `control/freeze` | Halt state, with `halt_epoch` | Dispatcher and both action services, strongly consistent, never cached | Operators, Eve, `platform-drift@`, breakers, through `/v1/control/halt` on either service (clearing is human-only) |
| `overrides/{family}/{trigger}` | Live demotions, with `override_epoch`. Absent reads as **L0**, never L5 | Action service | Operators, Eve, breakers |
| `ladder/current` | Deployed config, its version and `ceilings_sha` | Action service, and the ladder endpoint | The config deploy tool, after CI validation |
| `plans/{id}` | **Frozen plan and its lifecycle**: `pending_human`, `pending_eve`, `held_until`, `released`, `vetoed`, `expired`, `done`. Create-only for the action service, so a signed plan cannot be rewritten between signature and execution | Action service. Eve and Mo read plans through `GET /v1/plans/{id}` and `walle_audit.plans`, never Firestore directly: a direct read from `EVE_PROJECT` or `MO_PROJECT` would need a project-level `datastore` role in `WALLE_PROJECT`, which the topology forbids. The project-level `roles/datastore.viewer` Eve's runbook used to grant is not made since 2026-09-13 ([../eve/02-identity-and-auth.md](../eve/02-identity-and-auth.md)); its resource-scoped replacement for Eve's polling read is decision 44 in [../project-topology.md](../project-topology.md) | Action service |
...
| `runs/{session_id}` | For a chat request, the `run_id` minted per managed session and the session's `tainted` bit (§"The agent") | Action service | Action service |
| `proposals/{id}` | Proposal queue with **verdict and reason code**, and the operator's grade | Operators; Eve and Mo through `GET /v1/runs/{id}` and the write-ahead copy in `walle_audit` — no cross-project Firestore grant exists | Action service creates the proposal; operators write the verdict and the grade |
| `grades/{run_id}#{item}` | Shadow gradings — the input to the precision metric every promotion cites | Operators; Eve and Mo through `GET /v1/runs/{id}` and the `grades` table of `walle_audit` | Operators |
| `leases/{principal}` | One active run per target, plan freeze to terminal state | Action service | Action service |
| `cooldown/{principal}` | No autonomous write to a principal written to in the last 24 h | Action service | Action service |
...
| `drills/{date}` | Kill-switch drill results and measured times — what CI reads to refuse a stale promotion | CI, action service | Operators |
| `canary/{family}/{trigger}` | Run counter and target sample for a newly promoted cell | Action service | Action service |

Plan states, proposal verdicts, shadow grades, drill dates, leases and the canary sampler are
controls named in the enablement plan; without these collections they would have nowhere to
live, and the metrics that gate every promotion would have nothing to read.
...
On `audit.schema` every reason is namespaced: the platform vocabulary is
...
must not halt the programme. That exception does **not** extend to the
...
- **Two more tools, fixed, not generated.** `request_generic` takes exactly
...
- The Gemini Enterprise console's Model Armor setting does **not** cover custom ADK
  ...
  live in agent code. Prefer the two that Wall-E's own code
...
Required by platform HLD §12.2 and §18 item 10. Lives at `contract/agent-manifest.yaml` in
...
ceilings:                              # permanent maxima ; ladder.yaml cells sit at or below. The table of record is
                                       # 05 §4-§5; each value here is the lower of 05 and 10 §3.1.2's cap
...
hard_denied:                           # refused in every lane, before the ladder. The list of record is the committed
                                       # hard-denied library shared by both services and pinned by commit in each image;
                                       # its items and reasons are 03 §"The hard-denied list". A list here that drops an
                                       # entry of that library is refused by the validator (below).
  - "library: every row of 03 §The hard-denied list, at the pinned commit"
...
invokers:                              # plain REST ; never an agent principal ; the in-app allowlists are generated from here
  halt: [eve-controller@EVE_PROJECT, eve-verifier@EVE_PROJECT, platform-drift@CORE_PROJECT]   # on walle-actions and on walle-actions-super ; eve-verifier@ per topology row 27
