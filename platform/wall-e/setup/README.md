...
| M6 | 9 | Marking the client Trusted in API controls. The Gmail scopes are "restricted" and the organisation's own API controls can cut an untrusted client off weeks later, with an error that looks nothing like the cause. | **no** |
