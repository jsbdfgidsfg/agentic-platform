...
- Written against the platform objective of 2026-09-13 (Wall-E holds Super Admin, register row
  P33; [../agentic-platform/01-hld.md](../agentic-platform/01-hld.md) "What this reverses and
  what it costs"); placement per [../project-topology.md](../project-topology.md).
...
Since the objective of 2026-09-13 the path leads to a **super-admin** account. The action service is now the only gate; the OAuth scope set is the only ceiling
...
These nine rows are the statement of what must not happen, ordered worst first, and every row
maps to a mechanism in [03-lld.md](03-lld.md). They are not yet ratified: [decision 4](09-open-decisions.md)
is re-opened as **P29**, and what gets signed is the two lists in "The two lists" below
(platform HLD §13.1 item 2). Since 2026-09-13 (P33) N1, N2 and N7 rest on the policy chain, not
on a Workspace role.

| # | Must never happen | Why it is the ceiling | What prevents it | Enforced by |
|---|---|---|---|---|
| **N1** | **Wall-E changes tenant security posture** — 2SV, SSO, password policy, API controls, DWD, admin roles | A robot that can change the rules constraining it is not constrained. Recovery window is unbounded. | Not in the catalogue, and a **hard invariant in the policy chain of every lane** (`posture_change_denied` / `escalation_denied`, breaker trip, severity 1) for everything on the hard-denied list. Other posture work is either band B at tier `SUPER` (two human super admins, never autonomous) or band C, where a human does it in the console. Workspace multi-party approval makes role assignment, DWD, 2SV, session control, login challenges, account recovery, SSO and Context-Aware Access Google-enforced two-person acts over the robot's credential (P66). No Workspace role enforces this: the custom role that did was reversed on 2026-09-13, because the account is a super admin (P33) | Policy chain hard invariants + catalogue registry + denial suite; Workspace multi-party approval for the settings it covers; SA-02/SA-03 detection |
| **N2** | **Permanent data loss** — a deleted user, group or mailbox | Restore is possible for 20 days only, and needs a spare licence. Rollback does not exist. | Not in the catalogue; no role withholds deletion since 2026-09-13 (P33). Deleting any admin or the tenant account is a **hard invariant** in every lane (a reason from the closed denial vocabulary of platform HLD §13.1 item 2, breaker trip, severity 1). Deleting a non-admin user or a group is reachable only through band B at tier `SUPER`: two-person, change ticket, `hold_minutes`, treated as irreversible, never in a playbook | Policy chain hard invariants + catalogue registry + the `SUPER` ceiling of [05](05-autonomy-ladder.md#4-ceilings-that-are-code-not-config) §4 |
...
| **N6** | **Outbound communication chosen by the model, unattended** | An admin account is trusted by every employee; a phishing mail from it is not recoverable | Templated notification (`notify.operators`, family F2) and free text (`gmail.send`, `chat.message.send`, family **F2b**) are different families with different ceilings. One family would make free-text mail autonomous at L4 on a schedule — the exact thing this row forbids | Family split in the catalogue; recipient domain check as an explicit policy step |
| **N7** | **Touching a protected principal** — super admins, delegated admins, itself, its own OU | Self-modification and lateral movement | Protected-principal check, fails closed. The robot is a super admin itself, so the admin cache (`isAdmin`) includes it. It is also on the committed floor list, so it stays protected whatever the cache returns. Other admins' security settings and backup codes, `users.makeAdmin`, and any Super Admin assignment are on the hard-denied list in every lane. A second activity rule pages on any admin event whose actor is the robot and whose target is another admin | `_check_target`, background-refreshed admin cache, the committed floor list; SA-02 detection |
| **N8** | **An action nobody can see** | Undetected drift is worse than a visible failure | Write-ahead audit; writes refused when the audit sink is down; reconciliation against Workspace audit logs. That reconciliation needs an **organisation-level aggregated log sink into BigQuery**; routed only to Pub/Sub, those logs would leave the completeness metric nothing to query. It also needs org-level permission to create the sink. Two sinks land in two projects: Wall-E's copy is `walle-audit-bq` → `walle_workspace_logs` in `WALLE_PROJECT`; Eve's own organisation-level sink `eve-workspace-audit` → `eve_workspace_logs` in `EVE_PROJECT` (already designed), so Eve's evidence is outside Wall-E's teardown reach | Audit module, aggregated sinks, Eve's completeness metric |
...
This table says what is never in the **catalogue** (band A). Since the objective of 2026-09-13
(P33), "never in the catalogue" no longer means "never executed by the robot": a human can reach
part of this table through band B at tier `SUPER`, and part through band C. The last column
places each category against "The two lists" below. Where a category is not named on either
list, it stays out of the catalogue. Whether a human can reach it through band B then depends on the committed
method-to-tier table of [03](03-lld.md); where no API writes it, it is band C by fact.

| Category | Operations | Lane (platform HLD §13.1 item 2) |
|---|---|---|
| Privilege and posture | Admin role create, assign or revoke · 2SV, password, session, login-challenge or recovery settings · API controls and OAuth app trust · domain-wide delegation · domain and DNS settings · SSO/SAML · DLP · context-aware access · data regions | **Hard-denied:** `users.makeAdmin`, any Super Admin assignment or assignment of a role carrying admin-role management, DWD, the super-admin self-recovery setting, other admins' security settings and backup codes, "Share data with Google Cloud services", the SecOps export setting, the activity rules that alert on the robot. **Band B `SUPER`:** admin-role create or assign below Super Admin, domain add. **Band C** (no API writes them): 2SV enforcement and methods, sign-in challenges, password policy, session control, SSO, API controls and app trust, data regions — a human super admin does them in the console, watched for by `/v1/handoff`. DLP rules and detectors can be written through the Cloud Identity Policy API (v1), so they fall to the method table. Where Workspace multi-party approval covers a setting (role assignment, DWD, 2SV, session control, login challenges, account recovery, SSO, Context-Aware Access), Google itself requires a second admin (P66). A robot that can change the rules constraining it is not constrained; moving a user between OUs changes their posture without touching a security setting, which is why OU move is its own operation (F4b) with its own destination allowlist |
| Irreversible data | User delete or undelete · group delete · OU delete · mailbox purge · Vault matters, holds, exports and retention · tenant data export | **Hard-denied:** `users.delete` of any admin, deletion of the tenant account. **Band B `SUPER`:** user delete of non-admins, group delete — each one fully specified request, treated as irreversible, requester and approver two different human super admins, a change ticket and a hold window, permanently L3. OU delete: see Structure. The rest: not on either list. Restore of a deleted user is possible for 20 days only and needs a spare licence, so deletion is an operation the catalogue does not have, not a risk it manages |
...
| Self-modification | Anything targeting the robot account, its OU, the `walle-*` control groups, the ladder config, the catalogue, the overrides or the halt flags | **Hard-denied**, including `eve@`, Eve's role, `eve-owners@`, `ge-admins@`, `platform-approvers@`, the `mo-*` groups, the two OAuth clients and the organisation sinks |
| Structure | OU create, delete, rename or move. Moving a *user* into an allowlisted OU is F4b ([05](05-autonomy-ladder.md) §5); changing the tree is not. | **Band B `SUPER`:** OU create, rename, move and delete |
...
| Outside Workspace | IAM and settings of any of the four projects (`GEMINI_PROJECT`, `WALLE_PROJECT`, `EVE_PROJECT`, `MO_PROJECT`), the folder `FOLDER_ID`, Gemini Enterprise configuration | Unchanged: no lane reaches it. `cloud-platform` is consented on neither client (CI-checked). `walle@` holding any organisation-level IAM role is a drift row, and `SetIamPolicy` by `walle@` at organisation, folder or `EVE_PROJECT` is severity 1. Outside the reach of the agent and of every Wall-E service account by construction; outside the reach of the robot credential only by the hard-denied list, the scope set and detection, because a super admin can grant itself Organization Administrator in the console. No interactive login ever closes the console path. The fleet kill K7 and the platform's floors, deny policy and PAB live outside `WALLE_PROJECT` for this reason |
...
Beyond the catalogue, some things are excluded from the architecture itself. Each cost the
design something; they are exclusions, not gaps. With a super-admin robot no Workspace role
enforces any of them, and every row is watched by Eve's reconciliation and the SIEM's
super-admin set.

| Excluded from the architecture | Why, and what it costs | Lane |
|---|---|---|
| **Domain-wide delegation, in any form** | A tenant-wide impersonation capability scoped only by OAuth scopes: a compromised service account could act as anyone, including a super admin. Not for a service account, not through a private Marketplace app (which installs domain-wide and uses DWD underneath), not as a one-API exception. Cost: the Alert Center API requires DWD and is out of scope, replaced by Workspace audit-log sharing into Cloud Logging | A super admin can grant DWD in the console, so a DWD change is **hard-denied in every lane** (band C refuses to return the steps) and a severity-1 detection (SA-03) |
| **User impersonation of any kind** (N9) | Wall-E acts as exactly one identity. Gemini Enterprise Authorizations, which act as the requesting user, are rejected as the acting identity: every operator's own privileges would become the ceiling and every action would be attributed to them. They remain useful only to bind a verified email to a high-risk approval, and Wall-E never receives a user token carrying Workspace write scopes. `drive` is not requested for the same reasons: without delegation the robot sees only its own Drive | No lane: excluded by construction |
| **Any credential inside the model's process or context** | The agent holds nothing; no tool returns a credential and no prompt reveals one. Cost: one network hop of roughly 50 to 150 ms on every operation | No lane: excluded by construction |
| **Any approval the model can produce** | The agent is refused on the approval endpoints by caller identity, denial reason `approver_is_agent`, a hard invariant. Human consent arrives on a surface that authenticates the human; Eve signs with an asymmetric key the action service cannot mint. If the agent could reach the approval endpoint, every other control would be decoration | No lane: refused on every approval endpoint, band B's two-person approval included |
| **Irreversible operations in the catalogue** | See "Irreversible data" and "Credentials" above. Calendar invites are also treated as irreversible, because an invite that was seen cannot be unseen | Absent from the catalogue, so never autonomous; per-lane placement in the rows above |
| **Query-shaped writes** | Writes take explicit, enumerated targets. The playbook's selection query is pinned and hashed into the playbook version, because pinning only the selection operation left a free-form query a steered model could aim at an entire OU, and caps would turn a mass action into a drip rather than a refusal (N4) | Band A mechanism ([03](03-lld.md)) |
| **A single merged service for reasoning and action** | The merge that would destroy the design ([ARCHITECTURE.md](ARCHITECTURE.md) §3) | Not built |

### The two lists

Decision 4 re-ratified as **P29**, owner-signed before the super-admin grant, replaces the
never-list with two lists, both defined in [03-lld.md](03-lld.md) §"The hard-denied list" and
§"The tier-`SUPER` two-person list". The **hard-denied list** is refused in every lane — band A,
band B and the band-C handoff — as a hard invariant in the policy chain of both services
(denial, breaker trip, severity-1 page), tested by the denial suite, with CI ownership outside
the agent repository; on this page it is what N1, N2 and N7 and the "Hard-denied" entries above
rest on. The **tier-`SUPER` list** is everything else the old never-list excluded, reachable
only through `/v1/execute-generic` on `walle-actions-super` with two different human super
admins, a change ticket and `hold_minutes`, permanently L3 on chat
([05](05-autonomy-ladder.md#4-ceilings-that-are-code-not-config) §4); the gate checks that
both are signed and the denial suite's inverted exit checklist is green in
...
**The super-admin detection set.** With a super-admin robot, detection is the primary control
(platform HLD §13.1 item 5). The Workspace-side rules SA-01..SA-09 and the organisation-side rules
SG-01..SG-07 are hosted in the SIEM, never in `WALLE_PROJECT`, owned by IT security, severity 1,
and page the second human outside the Wall-E line in parallel with the on-duty desk; each rule's
definition, rationale, source, runbook and test fixture is
[../agentic-platform/07-monitoring-detection-incident-response.md](../agentic-platform/07-monitoring-detection-incident-response.md#62-the-super-admin-set-workspace-side--siem-hosted-severity-1-owned-by-it-security)
§6.2 and §6.3, and the absence alarms behind the `log_pipeline_silent` halt are §7 there. The
interactive-login row above is also SA-04 in the SIEM.

## Compliance

- **EU AI Act and TISAX.** Wall-E's declared intended purpose is the catalogue (band A), with
  the hard-denied list as its Art. 6(3) boundary enforced in code; classification and legal
  sign-off are [../agentic-platform/10-eu-ai-act.md](../agentic-platform/10-eu-ai-act.md#3-classification-per-system)
  §3.1 (P28, P125). Super Admin is a signed TISAX deviation on ISA 4.2.1, row R-01 of the risk
  register (P33), which an assessor may still refuse at maturity 3
  ([../agentic-platform/11-tisax.md](../agentic-platform/11-tisax.md#6-the-super-admin-deviation-record-p136)
  §6); this page's Monitoring and threat-model rows are evidence both frames cite, not a
...
- **Data protection, and `Assumption:` employee representative bodies with a say here, where your jurisdiction has them.** Wall-E reads directory data, usage reports and audit logs
  ...
  Worker information and consultation come **before Stage 1** (decided 2026-09-13), under
...
- **Data residency.** All four projects sit under `FOLDER_ID` in `europe-west1` with
  ...
  with user-managed replication. Agent Runtime **Code Execution** is EU-resident (platform HLD
  §11.4) but stays off fleet-wide: its launch stage is unverified, its isolation technology
  unnamed and its egress path undocumented (P121), and Wall-E must not run arbitrary code anyway.
  A limit to record rather than discover: **CMEK is unavailable** when the runtime uses a
...
- **Retention.** Every audit table carries a table expiry: 400 days for evidence, a floor that
  is the larger of the EU AI Act six months (Art. 26(6)) and the organisation standard, under a
  ceiling the DPO sets; the schedule of record is
  [../agentic-platform/08-data-logging-retention-sovereignty.md](../agentic-platform/08-data-logging-retention-sovereignty.md#52-the-schedule) §5.2.
