...
- Objective restated 2026-09-13; see the platform HLD
  ([agentic-platform/01-hld.md](agentic-platform/01-hld.md) §3 and §18 item 25). Qualified
  2026-09-13: this page stays the single authority for **the three agents' placement and the
  cross-project grant shapes**; the platform's folder tree, core projects and tiers are the
  platform HLD §3, and the factory generalises §3's rows into module inputs keyed on
  `<agent>_PROJECT` ([agentic-platform/02-landing-zone-and-tiers.md](agentic-platform/02-landing-zone-and-tiers.md)
  §3.3). The four projects are the first instances of a per-agent pattern, not a count.
...
**Named platform exceptions, added 2026-09-13** (platform HLD D7, §18 item 25): the platform's
own folder-level principals — the drift job's `roles/iam.securityReviewer` on
`fld-agentic-platform` (row 36), the K7 job's PAM entitlement, the folder deny policy and the
Principal Access Boundary — are inherited into every project below the folder, `EVE_PROJECT`
included. They are the named exceptions to the one rule and to decision 48's three-carve-out
cap. The two principals among them are §3 rows 36 (the drift job, folder-level) and 37
(`factory-apply@`, only during an approved PAM grant), each with its level and maker; the deny
policy, the PAB and the K7 entitlement are policies the platform owner sets at the folder and
the organisation ([agentic-platform/04-identity-and-privileged-access.md](agentic-platform/04-identity-and-privileged-access.md)
§3–§5); all are recorded as dated lines on decision 48. One further project-level grant is a
named exception to the one rule: the CI identity's `roles/discoveryengine.editor` on
`GEMINI_PROJECT` (row 38, P49).

---

## 1. Why four projects

Qualified 2026-09-13: the argument below is for **one project per agent — a pattern, not a
...
- **Multiplied, 2026-09-13.** Budgets, IAM policies, API lists and owner groups are per
  ...
  (platform HLD §0.4). §3 now has 44 rows.
...
core projects `CORE_PROJECT`, `LOGGING_PROJECT`, `CICD_PROJECT` and `VALIDATOR_PROJECT` are
described in the platform HLD §3.1 and named here only where a §3 row crosses into them.
...
Qualified 2026-09-13: the validator custodian's project is `VALIDATOR_PROJECT` under
...
**Added 2026-09-13** (platform HLD §18 item 25): rows 27–44 are the cross-project reaches the
platform HLD and its detailed pages add, each with its level and maker; existing rows the
platform changes carry a dated sentence in their role cell rather than a rewrite. Where the
...
through PAM (P142). Owner of every row added here: the platform owner (the Wall-E owner signs
...
- **The allowlists are not IAM, and they now carry foreign emails.** `EXEC_CALLER_ALLOWLIST`
  ...
  2026-09-13: `walle-actions-super` has its own control list carrying `eve-controller@` and
  (extended the same day) `eve-verifier@` for `/v1/control/halt` only (row 27) and no read list for any foreign principal; both lists are
  generated from the manifest's `control_invokers[]` and `read_invokers[]`, never typed.
- **The platform's folder-level principals are the named exceptions (added 2026-09-13).**
  Rows 36 and 37 and the folder deny policy, the PAB and the K7 entitlement reach into every
  project, `EVE_PROJECT` included, by inheritance. They are not a quiet widening of the one
  rule: each is enumerated, dated on decision 48 and in Eve's drift job's expected set, and
  row 37 exists only while a PAM grant is active.
- **Cross-organisation grants are exactly two (added 2026-09-13).** Rows 32 and 33 are made
  ...
  precondition.
- **A super-admin credential ignores this table (added 2026-09-13).** Every row bounds a
...
| `EVE_PROJECT`, added 2026-09-13 | `eve-export@` → dataset-level `READER` on `eve.*` and `storage.objectViewer` on Eve's locked bucket, and `roles/storage.objectCreator` on `exports/` of that bucket under the condition `resource.name.startsWith(".../objects/exports/")` (create-only, the form of row 16); `eve-gate` → halt on both Wall-E lanes is rows 3 and 27, not a home grant; the authorised views over `eve_quality` and over the advisor's view dataset authorised on their sources inside `EVE_PROJECT`; `eve-console`'s IAP audience is a group whose membership change is a severity-1 rule (HLD §13.2). |
| `EVE_ADVISOR_PROJECT`, added 2026-09-13 | `eve-advisor@` → `roles/bigquery.jobUser` and the model API in its own project; nothing else at home. |
| `EVE_WITNESS_PROJECT` (organisation `org-witness`), added 2026-09-13 | The absence alarm (a Cloud Monitoring policy on the heartbeat table the export writes), the severity 1/2 notification channels, Access Approval and Access Transparency, the K5 rota records — all administered by the witness's own super admins. |
...
**Superseded 2026-09-13 by the folder tree of the platform HLD §3.1** (one folder remains the
parent — `fld-agentic-platform` — with tier and role folders beneath it; the constraint values
below are the platform baseline's, [agentic-platform/02-landing-zone-and-tiers.md](agentic-platform/02-landing-zone-and-tiers.md)
§4, and the per-project checklist is superseded by P41). The text below is kept as the record of
2026-09-13's first cut; the rows carry the dated alignments item 25 requires. Organisation-policy
administration is organisation-level only (roles `orgpolicy.policyAdmin` have no lower grant
level), so the kill plane's org-policy lever is a PAM entitlement at the organisation
([agentic-platform/04-identity-and-privileged-access.md](agentic-platform/04-identity-and-privileged-access.md) §5).
...
|---|---|
| `constraints/iam.allowedPolicyMemberDomains` (domain-restricted sharing) | With the organisation's customer id allowed, "all service accounts … in any project in the organization" and "all service agents associated with resources in your organization" are eligible members — so every cross-project row in §3 that names an `…@<project>.iam.gserviceaccount.com` principal is admissible without an exception. Exceptions may still be needed for Google-managed accounts outside the organisation: `gmail-api-push@system.gserviceaccount.com` (Wall-E, Phase 16) and, per Google's own list, Logging sink writer identities of the `@gcp-sa-logging` form — check each of the three organisation sinks' writer identities. The `gmail-api-push@` exception is a domain-restricted-sharing exception, not a sink. Source: [Restricting identities by domain](https://docs.cloud.google.com/resource-manager/docs/organization-policy/restricting-domains). **2026-09-13:** set at `fld-agentic-platform` to the tenant customer id, plus the sandbox tenant in nonprod, plus the recorded `gmail-api-push@` exception; **no witness exception** — rows 32–33 are pushes into witness-owned stores, and the exception lives on the witness side for `eve-export@` only (HLD §3.3). The legacy constraint is kept until Google retires it (P41). |
| `constraints/gcp.resourceLocations` | `europe-west1`, the `EU` BigQuery multi-region and `global` (floor settings) for the three agent projects; additionally `eu` (the app's multi-region) for `GEMINI_PROJECT`. Agent Registry enforces it at write time. **Aligned 2026-09-13 with platform HLD §3.3 and B1 of [02](agentic-platform/02-landing-zone-and-tiers.md) §4.1:** `in:eu-locations` at `fld-agentic-platform`; `fld-gemini-enterprise` adds `eu`. The `global` floor-setting allowance is **not** in the list: Google's supported-services page lists Model Armor template creation and Agent Registry regional resources as location-enforced, not floor settings. It is verified or dropped at the first factory run in nonprod — if the folder floor write is refused, `global` is added for the floor-setting resource only, as a dated exception in HLD §8.2, and this row is updated either way. Owner: platform owner; gate: the factory's first run. |
...
| `constraints/iam.managed.disableAccessPolicyBinding` | Lifted on `WALLE_PROJECT` only, before Phase 13b binds `roles/iap.egressor`. Enforced on the other three. **Superseded 2026-09-13 (B7, P41):** enforced at `fld-agentic-platform`; **lifted as a folder value** at `fld-agents-r`, `fld-agents-w`, `fld-agents-p` and `fld-controllers` (so on `EVE_PROJECT` and `EVE_ADVISOR_PROJECT` too), because every agent project binds `roles/iap.egressor` on its gateway; lifted on `GEMINI_PROJECT` as a per-project factory input for the tenant gateway's binding; stays enforced on `fld-platform-core` and `fld-improvers`. |
| `iam.managed.disableServiceAccountKeyCreation`, `iam.disableServiceAccountKeyUpload` | Enforced on all four. No agent identity ever has a key file. **Corrected 2026-09-13 (B3):** the key-upload constraint is spelled `iam.managed.disableServiceAccountKeyUpload`, the managed form, one spelling fleet-wide, enforced at `fld-agentic-platform`; exceptions only through PAM, dated. |
| VPC Service Controls | No perimeter around `WALLE_PROJECT` (Agent Gateway access policies do not support it). Eve's egress control names VPC Service Controls plus a host allowlist ([E-19](eve/09-open-decisions.md)); a perimeter around `EVE_PROJECT` alone is now possible without touching Wall-E's, but it must carry egress rules for Eve's reads of `walle_audit` and calls to `walle-actions`. `Assumption:` that is E-19's problem to solve, not this page's; recorded here so the two projects' network postures are not assumed identical. **2026-09-13:** the perimeter model is platform decision P3 — spike 1 (an engine calling an `internal-and-cloud-load-balancing` action service through an internal load balancer and PSC endpoint) after which `run.allowedIngress = internal-and-cloud-load-balancing` is applied at five folders (P90), a precondition of the super-admin grant; spike 2 (VPC Service Controls) is the Tier W backstop; E-19 is closed by P3's record ([agentic-platform/06-gateways-model-armor-perimeter.md](agentic-platform/06-gateways-model-armor-perimeter.md) §4). |
| `constraints/gcp.restrictServiceUsage` (added 2026-09-13) | Per folder, as the allow-lists of [02](agentic-platform/02-landing-zone-and-tiers.md) §4.2; **on `EVE_PROJECT` a project-level denylist of `aiplatform.googleapis.com`**, not a folder value, because `EVE_ADVISOR_PROJECT` under the same folder needs the API; `agentregistry.googleapis.com` absent from every tier folder (P71). Also the first fleet-kill lever (HLD §11.4). |
...
wins there (**superseded 2026-09-13, P84 as redesigned:** Google defines template conformance
at organisation and folder level and inline enforcement at project level only, and a project
floor overrides its parents; so every agent project, `WALLE_PROJECT` included, gets a
factory-generated `Custom` project floor carrying inline enforcement — always
`INSPECT_AND_BLOCK` at P-SA — and any other project floor write, `Inherit` or `Disable` is
severity-1 drift; `floorSettingsAdmin` at project level only through PAM;
[agentic-platform/06-gateways-model-armor-perimeter.md](agentic-platform/06-gateways-model-armor-perimeter.md) §3.2); Mo's project inherits the folder floor for `mo-narrator@`'s `generateContent`
...
| `EVE_ADVISOR_PROJECT` (added 2026-09-13) | Eve's reporting-path project | Eve's runbook (rows 30–31); the factory |
| `EVE_WITNESS_PROJECT` (added 2026-09-13) | The witness project in `org-witness` | Eve's runbook (the export job's targets, rows 32–33); the witness runbook |
| `CORE_PROJECT`, `LOGGING_PROJECT`, `CICD_PROJECT`, `VALIDATOR_PROJECT`, `KMS_PROJECT` (added 2026-09-13) | The five core projects under `fld-platform-core` (platform HLD §3.1) | rows 29, 35–44; the factory |
...
record the first cut and carry dated lines where a phase changes; §7.5 and §7.6 list who makes
the rows this page added.
...
|---|---|---|---|
| 6 | The project (`--folder="$FOLDER_ID"`), APIs, `walle-actions@`, `walle-agent@` (fallback), `walle-dispatcher@`, `walle-tasks@`, `walle-operators-caller@`, staging bucket, budget. 2026-09-13: a factory call under `fld-agents-p-sa`; adds `walle-actions-super` and its service account | — | Create `eve-controller@`. Grant `datastore.viewer` to any Eve identity (`EVE_PROJECT_ROLES` becomes empty or a database-scoped condition, decision 44). |
...
| 10 | `walle-actions` (2026-09-13: and `walle-actions-super`) | `run.invoker` for `eve-controller@`, `eve-verifier@`, `eve-console@` (`EVE_PROJECT`) and `mo-analyst@` (`MO_PROJECT`); 2026-09-13: `eve-controller@` and `eve-verifier@` on `walle-actions-super` for halt only (row 27); dataset `READER` on `walle_audit` for `eve-export@` (row 34); all as `control_invokers[]` / `read_invokers[]` / `audit_readers[]` in the privileged phase; `CONTROL_CALLER_ALLOWLIST` and the read-endpoint list with the foreign emails; `EVE_KMS_KEY` pointing at Eve's project on the fallback path | Bind `SA_EVE` in its old `${PROJECT}` spelling. |
| 11 | `walle-dispatcher`, the two organisation sinks and their writer grants (row 23) | — | 2026-09-13: no organisation sink — row 23 retired, replaced by rows 39–41 made by the factory in `LOGGING_PROJECT` (P104); gate before Stage 1 |
...
| 13b | Agent Registry entry, egress gateway, `agentregistry.admin` to `CI_DEPLOYER` (2026-09-13: the entry is written into the shared registry in `CORE_PROJECT` by `factory-apply@`, the only standing registry writer; no registry in `WALLE_PROJECT`, P71, P142) | **None** — the `agentregistry.viewer` grants to `SA_EVE` and `MO_PRINCIPAL` are removed (decision 43); `MO_PRINCIPAL` stays a config key only if some other subcommand needs it | Grant a project-level role to any foreign principal. |
...
| Added 2026-09-13 (HLD §13.2, §13.3; before the super-admin grant for the observe-and-report items) | `eve-export@`; `eve_quality` with its authorised views; `eve_advice` and the advisor's view dataset; `eve.incidents`, `eve.pages`; the sink widened to six streams | Row 28 (`mo-metrics@` → `eve_quality`), row 29 (validator custodian → `eve_quality`, P30), rows 30–31 (`eve-advisor@`); rows 32–33 are **requested from** the witness administrators, not made here | Phase 9 check 2's "no `userByEmail` entry for `mo-metrics@` on `eve` — ever" narrows to the non-quality datasets; `eve-approval` at HSM protection level |
...
### 7.5 The witness administrators (added 2026-09-13)
...
### 7.6 The platform owner and the factory (added 2026-09-13)

| Made by | Rows |
|---|---|
| Factory `agent-project` module (routine phase; the credential-path half in the privileged phase through PAM, P142) | 1, 3, 4, 6, 8, 34 as manifest inputs; 39; 42; 43 |
| Factory `verifier-project` module | 14–16 (carve-outs), 22 |
...
Updated 2026-09-13 (platform HLD §18 item 25): the diagram adds Eve's reporting-path project,
the witness organisation, the core projects the new rows cross into, `walle-actions-super`
and rows 27–41 (rows 45–46, proposed later the same day, drawn dashed); rows 22–23's Wall-E organisation sinks are drawn dashed as retired (P104).
...
flowchart LR
    ...
    subgraph MO["MO_PROJECT — continuous improvement"]
        ...
        DROP["Bucket 'mo-proposals' (renamed 2026-09-13)"]
    end

    EXTCI["CI ingestion bot (git host, tbd)"]
    VAL["Validator custodian (VALIDATOR_PROJECT)"]
    GRD["BigQuery 'eve_grades' with 'grades_eve' (VALIDATOR_PROJECT, proposed 2026-09-13)"]
    ...
    EV -->|"27: run.invoker, halt only (added 2026-09-13)"| ACTS
...
- [agentic-platform/01-hld.md](agentic-platform/01-hld.md) — the platform HLD: §3 the folder tree and factory, §13 the three agents, §18 item 25 the edits this page carries (added 2026-09-13)
