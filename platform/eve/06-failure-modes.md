...
- Objective: the platform HLD ([../agentic-platform/01-hld.md](../agentic-platform/01-hld.md)
  §13.2); this page carries §18 items 12, 14 and 15 (owner the Eve owner, gate the super-admin
  grant, P143).
...
[08](../wall-e/08-team-eve-mo.md) item 7 requires Eve to fail closed in every direction — "Eve
must never be a thing whose absence lets more happen" — and this design meets it with
mechanisms **outside Eve**, two deterministic sweepers and one passive stamp inside
`walle-actions`, none of which raises anything on Eve's return, because a component cannot be
relied on to notice its own absence. Item 5's "halting, on its own judgement, with a documented
bias toward halting when uncertain" is void under [C12](../wall-e/14-hld-challenge.md) for
Eve's authority path, which has thresholds rather than judgement (the report-only `eve-advisor`
may reason and holds no authority, P34), so two `thresholds.yaml` rules replace it: at the
approval point every unresolved comparison resolves to **refuse**, and **halting** is reserved
for the closed invariant-trigger list. The amendment is E-7 in
[09-open-decisions.md](09-open-decisions.md) and an edit back onto the contract in
[08-contract-changes.md](08-contract-changes.md); the bias is argued in
...
| **Google's audit event has not landed** | Never reported as `unverifiable` for lag alone: current state is the primary evidence and proves the write inside the 60-minute SLA, the admin event is the attribution evidence on its own lag budget, and a verdict waiting on the event is `verified_state_only` until the 6-hour deep pass, escalating to `reconciliation_gap` only if nothing lands by T+6 h — [04-flows.md, the primary-evidence split](04-flows.md#the-primary-evidence-split). |
| **Google writes no audit log for the family at all** | Calendar has no Cloud Logging audit stream, so calendar-family items are `verified_state_only` with `no_audit_stream` **permanently**, a declared limit carried in every attestation touching the family — [04-flows.md, the primary-evidence split](04-flows.md#the-primary-evidence-split). |
| **Eve cannot verify a licence change** | F7 is verified from licence events only and recorded `verified_partial` with `licence_event_only`, a declared, permanent limit carried in every F7 attestation — the dropped scope and its consequences are in [02-identity-and-auth.md](02-identity-and-auth.md#the-scopes-and-the-one-that-was-dropped). |
...
| **The evidence goes silent** — "Share data with Google Cloud services" switched off, a sink re-filtered, Eve's token returning `invalid_grant` | Heartbeat H-1 or the Eve credential check fires `log_pipeline_silent`: the halt, the severity-1 page and the human clear are in [../agentic-platform/07-monitoring-detection-incident-response.md](../agentic-platform/07-monitoring-detection-incident-response.md) §7 and [03-lld.md §14](03-lld.md#14-the-detection-catalogue-the-roster-check-and-the-evidence-heartbeat). The Reports API poll is independent of the sharing switch and keeps reading, and the alarm also lives in the witness, so deleting it inside the tenant does not silence it. |
| **Wall-E's super-admin credential turns on Eve** — suspends `eve@`, revokes Eve's token, removes Eve's role, grants itself Organization Administrator and redeploys `eve-gate`, deletes `EVE_PROJECT` | Every one of these is hard-denied in every Wall-E lane and a severity-1 tenant-integrity or SG-* rule in the SIEM and in Eve's catalogue. Inside the tenant's organisation this is **detection, not prevention**: the witness sees `EVE_PROJECT`'s deletion within fifteen minutes and the absence of evidence, and pages out of band; it does not see a forged approval. K6 (a human removes Super Admin from the robot) is the switch that survives a token already minted. The structural end state is P15 ([01-hld.md](01-hld.md) structural choice 1). |
| **The witness push does not arrive** | Heartbeat H-4 (window, alarm and runbook RB-03 in [../agentic-platform/07-monitoring-detection-incident-response.md](../agentic-platform/07-monitoring-detection-incident-response.md) §7). A manifest mismatch in the evidence-bucket export is severity 2 and holds any promotion citing that day. |
| **The super-admin roster changes** | The daily roster check raises `role_assignment_added` or `role_assignment_missing`, severity 1 in both directions, `halt_all` on the P-SA lanes; K5/K6 remain human acts on the two-person rota. |
| **The reporting path is wrong or compromised** | Bounded by construction: `eve-advisor@` holds no signer, no invoker, no secret, and nothing it writes is read by `eve-gate`, `eve-reconciler` or either action service. The worst it can do is page at severity 2 inside the budget, write a misleading narrative, or stay silent — and silence costs nothing the control path relies on. A narrative never replaces the deterministic finding it cites; a model-pin change resets its paging thresholds (P123). |
...
the operator veto, not on Eve. It also rests on the SIEM's independent evaluation of
...
1. **Revoke `roles/run.invoker`** on `walle-actions` from `eve-controller@` and
   `eve-verifier@` — and on `walle-actions-super` from its Eve holder (the
...
An alert that trains its readers to ignore it is a defect in Eve, so Eve's own noise is measured and gated like any other metric: every page is written to
...
Eve is not read by one person: who each page reaches, through which channel, and what
`eve.pages` records are the reporting contract of
[03-lld.md §15](03-lld.md#15-the-reporting-contract) (platform HLD §13.2). The budget rule above
is the platform's RP-6, under which a verifier over budget for two weeks has its severity-2
rules reviewed, never silenced.
...
`thresholds.yaml` numbers E-18 defers to S2 calibration on measured data. All six conditions
are named; 5 and 6 mirror SIEM rules:
...
| 5 | Posture change by any actor — SIEM rule SA-03 | `eve-reconciler`, mirrored from the SIEM rule | Severity 1. The settings covered and the unverified event names (until observed, the absence alarms are the detector) are in SA-03 of [../agentic-platform/07-monitoring-detection-incident-response.md](../agentic-platform/07-monitoring-detection-incident-response.md) §6.2. |
| 6 | Any `login.googleapis.com` event for `walle@` or `eve@`, an interactive robot login — SIEM rule SA-04 | `eve-reconciler`, mirrored from the SIEM rule | Severity 1. |

Outside the budget, because they are severity 1 and never budgeted:
...
`oncall.yaml`, in Eve's own configuration repository, declares coverage — a
...
These are carried in every attestation bundle for an affected cell, so that a promotion
decision reads them rather than inherits them, and stated here rather than narrowed away. Each
one is a place where an honest reviewer should push.
...
3. **F7 licence verification is event-only.** It cannot be made state-aware without a
   ...
   `verified_partial` with reason `licence_event_only`. The consequences are in the
...
### The declared limits of watching a super admin

Carried in every attestation beside the five above, as a separate list (platform HLD §13.2):

- **Independence inside the organisation is detective.** Structural choice 1
  ([01-hld.md](01-hld.md)): a super admin who reaches Organization Administrator can redeploy
  `eve-gate`; the witness sees deletion and absence, not forgery. The end state that closes it
  is P15.
