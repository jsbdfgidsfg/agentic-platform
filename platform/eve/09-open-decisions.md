...
- **Objective restated 2026-09-13; see the platform HLD**
  ...
  decision 52 blocking the super-admin grant, P143). E-2, E-6, E-7, E-13, E-16 and E-19 carry
  a dated line each, and so do the two settled rows "No model anywhere in Eve v1 or v2" and
  "`eve-advisor` is not built". Nothing below is rewritten: each line supersedes, never
  erases. Platform decisions continue as **P1…** in
...
**Numbering.** Decisions in this set are numbered `E-1` to `E-20` (`E-21` added 2026-09-13). The `E-` prefix exists so
...
| **Before the super-admin grant** (added 2026-09-13) | **E-2** (reopened as blocking for the grant) and topology decision 52; **E-16** (before the consent, which now precedes the grant) |
| Before S4 exit | **E-10** |
| With this design record | **E-7**, **E-8**, **E-12** |
| With Mo's design | **E-20** |
| Before Wall-E's Stage 1 (added 2026-09-13) | **E-21** |
...
| **E-21** (added 2026-09-13, review-findings pass) | **Where `grades_eve` lives.** | Platform HLD §13.3 has the approval surface write `grades_eve` and Mo's source rule (A10) and metrics E1–E3 read it, but no page fixed its dataset or granted a reader, so Mo's independent Eve numbers had no source. It must survive a compromised Eve (so not `EVE_PROJECT`), keep Eve's verdicts out of Wall-E's project (so not `WALLE_PROJECT`), and keep Mo from being Eve's grader (so not `MO_PROJECT`). | **Proposed:** dataset `eve_grades` (table `grades_eve`) in `VALIDATOR_PROJECT`, owned by the validator custodian; the approval surface's grading identity writes it insert-only at dataset level; `mo-metrics@` reads it at dataset level; the custodian reads it at home; no Eve identity reads or writes it ([../project-topology.md](../project-topology.md) §3 rows 45–46). Residual: the grading identity lives in the approval surface's project (per agent, P46), so a compromise of that project can insert false grades; the grades carry the IAP-asserted grader and the custodian's re-derivation compares them with the blind sample. | Wall-E's Stage 1 (before any Eve number is evidence-eligible) | platform owner with the security reviewer; this page, topology rows 45–46 |
