...
- **Objective restated 2026-09-13; see the platform HLD**
  ([../agentic-platform/01-hld.md](../agentic-platform/01-hld.md) §13.2 and §17): this page
  carries §18 item 18 (owner the Eve owner, gate Wall-E's Stage 1, with E-2 and topology
  decision 52 blocking the super-admin grant, P143); platform decisions are **P1…** in
  [../agentic-platform/12-open-decisions.md](../agentic-platform/12-open-decisions.md), whose
  §8 indexes every E-row against the platform rows that touch it.

This page lists the twenty-one decisions Eve's design does **not** settle. Each one is stated
...
**Numbering.** Decisions in this set are numbered `E-1` to `E-21`. The `E-` prefix exists so
...
with the Eve obligations `E1`–`E88` drawn from Wall-E's set. A bare number in this set always
means a Wall-E decision; an `E-` number always means one of the rows below.

**Where each is recorded.** The reasoning lives on the page named in the last column of the
table, and the answer as a dated, append-only file in [`../../decisions/`](../../decisions/),
as [../agentic-platform/12-open-decisions.md](../agentic-platform/12-open-decisions.md#1-how-this-register-works)
§1 describes; that directory holds P33's and P34's records and none for any `E-` row yet.

None of these is a decision a program may take. Eve lowers and halts on thresholds;
...
| **Before the super-admin grant** | **E-2** (reopened as blocking for the grant) and topology decision 52; **E-16** (before the consent, which now precedes the grant) |
| Before S4 exit | **E-10** |
| With this design record | **E-7**, **E-8**, **E-12** |
| With Mo's design | **E-20** |
| Before Wall-E's Stage 1 | **E-21** |
...
| **E-2** | **Who owns Eve's project?** | The project boundary is only as real as the org chart. With one administrator it is notional. | Target an `eve-owners@` group containing IT security and not Wall-E's deployers, bound at `EVE_PROJECT` level; with all four projects under `FOLDER_ID`, no Wall-E deployer group may hold a folder-level role, or the boundary is inherited away (topology decision 52 generalises this to four owner groups). Build the boundary now, record in writing that it is notional until decision 11's second human exists, and pre-refuse the temptation to fold Eve back into Wall-E's project. **2026-09-13, reopened as blocking for the super-admin grant** (platform HLD §0.3, §0.4, §13.2; P1, P14, P98, P137): the owner of `eve-owners@` is a **second human outside the Wall-E administration line** (IT security), named before the grant, who also administers the witness organisation, reviews `eve/config`, receives every severity 1/2 page in parallel and is sole recipient of reports about the administrator; topology decision 52 is reopened with it as blocking for the grant. Eve's build does not wait for that person; the grant does. | Before S4; **the super-admin grant** (2026-09-13) | [02-identity-and-auth.md](02-identity-and-auth.md), [06-failure-modes.md](06-failure-modes.md) |
...
| **E-6** | **Eve's observe mode, split.** | E63 says Eve is observe-mode through Stage 3; E53 says Eve must fail closed in every direction. Both cannot hold literally: the only component watching Google's log would be forbidden to act on what it sees. | Split: **signing** is observe-only through S3; **halt and demote on the invariant class** are live from Eve onboarding, with rate-based triggers observe-only until calibrated. Halting is the direction a human can undo in seconds. Record as a reading, with the cost named — a buggy Eve can halt the programme during the stage the programme is trying to prove itself. **2026-09-13:** consistent with the objective's "independent and autonomous" Eve; the split is re-cut by layer (platform HLD §13.2): the observe-and-report layer — halting on the tenant-integrity class, `reconciliation_gap` over every stream and `log_pipeline_silent` — is live **before the super-admin grant**, the gate layer keeps this split. The platform verifier's compensations for Tier W (P79) generalise this reading for other agents and do not weaken Eve's. | **S3 entry**; the observe-and-report half before the super-admin grant (2026-09-13) | [05-stages.md](05-stages.md), [06-failure-modes.md](06-failure-modes.md) |
| **E-7** | **Eve's halting bias, restated.** | 08 item 5's "own judgement" is void under C12; "bias toward halting when uncertain" would make a degraded Eve the outage. | At the approval point the conservative act is **refuse** (the plan waits, costing minutes); **halt** is reserved for the closed invariant-trigger list and is never discretionary. Record as an amendment to 08 item 5. **2026-09-13:** stands for the control path — halting stays non-discretionary; **reporting is reopened** for the report-only reporting path `eve-advisor`, which may reason and holds no authority (platform HLD §13.2; P34, file `decisions/2026-09-13-eve-reporting-path-may-reason.md`, *proposed*; its AI Act class is P19, legal). The halting-bias question is moot for a report-only path. | With the design record | [06-failure-modes.md](06-failure-modes.md), [08-contract-changes.md](08-contract-changes.md) |
...
| **E-12** | **E18's wording versus E19.** | E18 says Eve's only project-level role in Wall-E's project is `datastore.viewer`; E19 grants `agentregistry.viewer`. Both are read-only and both are in the runbook; the wordings cannot both be literally satisfied. | **Settled by removal on 2026-09-13:** E18 reads "**no project-level role in `WALLE_PROJECT` at all**, and no write role" — `EVE_PROJECT_ROLES = ()`, neither role granted, the Firestore discovery read absent until topology decisions 43 and 44 say otherwise ([../project-topology.md](../project-topology.md#8-open-decisions-this-topology-adds) §8). | With the design record; decision 44 before S3 entry | [02-identity-and-auth.md](02-identity-and-auth.md), [08-contract-changes.md](08-contract-changes.md) |
| **E-13** | **The second grader.** | Without a grader who is not the ladder owner, the blind sample is blind in form only, and the compromised-Eve bound is weaker than stated. Blindness for *refusals* is structural (Eve's verdicts never enter Wall-E's project); for *accepts* it rests on an allowlisted column set and a CI test. | Name one under [decision 11](../wall-e/09-open-decisions.md) by S3, not S4. No `WRITE_HIGH` cell reaches `eve_authority: binding` without them. **2026-09-13** (platform HLD §13.3; P25, P79, P98, P137): unchanged, plus two rules — Mo's duty to improve Eve never makes **Mo Eve's grader**; and Eve's own verdicts are graded in a separate `grades_eve`, written and sampled by the platform approval surface, never by `eve-console`. Separation of duties is a counted minimum (P137). | Before S3 exit | [04-flows.md](04-flows.md), [06-failure-modes.md](06-failure-modes.md) |
...
| **E-16** | **Eve's credential chapter is contingent on [decision 26](../wall-e/09-open-decisions.md).** | If a keyless service account can hold a custom admin role with no DWD, Eve's robot, consent, hardware key, refresh token and six-month clock all disappear. | Re-examine Eve's whole credential chapter before onboarding rather than building it as specified. **Changed 2026-09-13 (P33):** decision 26 is **closed for Wall-E by fact** (P33 — a service account cannot hold Super Admin) and survives for Eve only, here. The question now also carries Eve's **widened read privilege and scope set**, fixed before the one-sitting consent (platform HLD §13.2): domain and customer settings read, every Reports application the poll reads, no content scope, resolved with `privileges.list` ([02-identity-and-auth.md](02-identity-and-auth.md)). The consent moves before the super-admin grant, and the key location it touches is P118's. | Before Eve onboarding — **before the super-admin grant** (2026-09-13) | [02-identity-and-auth.md](02-identity-and-auth.md), [07-build-runbook.md](07-build-runbook.md) |
...
| **E-19** | **Agent Gateway egress for Eve** (E88's "through its own egress gateway"). | Agent Gateway availability in europe-west1 is unverified, and Gemini Enterprise is supported in egress mode only. | `Assumption:` until confirmed, Eve's egress control is VPC Service Controls plus a host allowlist — `walle-actions`, `admin.googleapis.com`, `cloudkms.googleapis.com`, `secretmanager.googleapis.com`, `bigquery.googleapis.com`, `firestore.googleapis.com`, `storage.googleapis.com`. `aiplatform.googleapis.com` is deliberately never registered. Agent Gateway availability in `europe-west1` is unverified and stays so; it blocks nothing in Eve but changes this row if answered. Reopen when answered. **2026-09-13, closed by platform decision P3** (the perimeter model by two spikes, with P81 and P89; platform HLD §8.1): the gateway-or-perimeter question is the platform's; `run.allowedIngress` and the controller folder's policies follow P3; the host allowlist stays Eve's interim until P3's spikes pass, and `aiplatform.googleapis.com` is denied in `EVE_PROJECT` by a project-level `restrictServiceUsage` denylist (CP5). | Before S4; closed 2026-09-13 | [02-identity-and-auth.md](02-identity-and-auth.md), [07-build-runbook.md](07-build-runbook.md) |
...
| **E-21** | **Where `grades_eve` lives.** | The approval surface writes `grades_eve` (platform HLD §13.3) and Mo's source rule (A10) and metrics E1–E3 read it, so it needs a dataset and a granted reader that survive a compromised Eve, keep Eve's verdicts out of Wall-E's project and keep Mo from being Eve's grader. | **Proposed:** dataset `eve_grades` (table `grades_eve`) in `VALIDATOR_PROJECT`, made by the validator custodian; no Eve identity reads or writes it; placement and grants are [../project-topology.md](../project-topology.md#3-cross-project-grants) §3 rows 45–46. Residual: the grading identity lives in the approval surface's project (per agent, P46), so a compromise of that project can insert false grades; the grades carry the IAP-asserted grader and the custodian's re-derivation compares them with the blind sample. | Wall-E's Stage 1 (before any Eve number is evidence-eligible) | platform owner with the security reviewer; this page, topology rows 45–46 |
...
These were argued and closed in the design. They are listed so nobody
...
| No model anywhere in Eve v1 or v2 | Required by [C12](../wall-e/14-hld-challenge.md) and [decision 34](../wall-e/09-open-decisions.md), and enforced by dependency absence and permission absence rather than by policy. **2026-09-13, reopened on new evidence for the reporting path only:** the objective ("report to the human anything wrong") is the new evidence this row asked for. The row stands for the **control path** — no model produces an approval, a signature, a halt, a demotion or a veto, now also enforced by a project-level `restrictServiceUsage` denylist on `EVE_PROJECT` and recorded as the AI Act Art. 3(1) invariant. It no longer holds for all of Eve: platform register row **P34** (*proposed*, owner the Eve owner with the security reviewer, gate the `eve-advisor` build) permits a report-only reasoning path (platform HLD §13.2). |
...
| `eve-advisor` is not built | The slot is named and left empty so it cannot be smuggled in. If it is ever built, adding it is a decision record, not an implementation detail. **2026-09-13, the decision record is due:** P34 is that record — [../../decisions/2026-09-13-eve-reporting-path-may-reason.md](../../decisions/2026-09-13-eve-reporting-path-may-reason.md), status **proposed**, unsigned; this row is reopened pending its acceptance, not yet superseded. `eve-advisor` is the reporting path in `EVE_ADVISOR_PROJECT`, report-only by construction — no signer, no invoker, no secret, nothing it writes read by any gate; the earlier "wire to raise a refusal" is withdrawn. Still not built; its paging rights and any registration wait on P19 (legal). |
...
| **C12** — Eve is drawn as an LLM controller and everything it must do is deterministic | Partially stood | **Confirms.** Every Eve decision is code, enforced five mechanical ways; 08 item 5's "own judgement" is void and replaced by **E-7**'s two rules. **Qualified 2026-09-13:** every decision on Eve's *authority path* is code; the report-only reporting path may reason and decides nothing (P34). |
...
|---|---|
| Agent Gateway availability in `europe-west1` | **E-19**. Until it is confirmed, Eve's egress control is VPC Service Controls plus a host allowlist. 2026-09-13: E-19 is closed by P3; the question moves to the platform's perimeter spikes. |
...
| Whether `roles/datastore.viewer` can be narrowed to Wall-E's `(default)` database by an IAM Condition | Topology decision 44. If it can, Eve's Firestore discovery read — ungranted since 2026-09-13 (**E-12**) — is granted in that resource-level form; if not, a list endpoint on `walle-actions` is the alternative, as a contract-change row (CC-33). |
