...
| `roles/datastore.viewer` and `roles/agentregistry.viewer` in Wall-E's project | No Eve process reads Firestore or the registry yet. |
...
taken through it. `eve.verdict_receipts` is the opposite case — it is created at S4 entry
