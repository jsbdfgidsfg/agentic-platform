...
- 2026-09-13: objective restated for a super-admin Wall-E (P33) — see the platform HLD
  ([../agentic-platform/01-hld.md](../agentic-platform/01-hld.md) §13.2–§13.3, §18 items 13 and
  17; owners and gates P143). `eve@` stays a read-only role, **never Super Admin**.
...
flowchart LR
  ...
  EC -->|"run.invoker on walle-actions-super, halt path only"| WAS
  EV -->|"run.invoker on walle-actions-super, halt path only"| WAS
...
[../project-topology.md](../project-topology.md) §3 (rows 14, 15, 16). The six edges drawn
after them are the cross-boundary grants of platform HLD §18 item 25: the halt path onto
`walle-actions-super`, the export and witness push, the reporting path's view and write, and
Mo's and the custodian's quality read.
...
The project boundary buys five things that IAM hygiene inside one project cannot:
...
4. **The forbidden edges become drawable.** A reviewer can check the claim by reading one
   IAM policy, not by reasoning about seventeen role bindings: Wall-E's deployers appear
   nowhere in Eve's IAM policy.
5. **Eve's queries stay in Eve's project.** Query jobs run in `EVE_PROJECT`, so Eve's
   BigQuery cost and job creation never touch Wall-E's project.

**Against a super admin** (platform HLD §13.2). All five are structural against Wall-E's
...
rather than narrowed away, so that nobody later reads the project-boundary diagram (four
projects under one folder) as proof of a separation of duties that no rota supports. Folding
Eve back into Wall-E's project "for now" is pre-refused.
...
own actions. Eve's *build* still does not wait for that person — Eve v0 and the
observe-and-report layer can start with one administrator — but the grant does, and "the four
owner groups are one person" expires on the grant date.
...
**Widened 2026-09-13, before the one-sitting consent (E-16).** Two scopes are added, so the
consented set is **ten**, all read-only:
...
- **No Cloud Identity Policy API scope.** The Policy API reads security and API-controls
  ...
  [06-failure-modes.md](06-failure-modes.md#the-declared-limits-of-watching-a-super-admin).
- **No device or Chrome management scope.** `Assumption:` context-aware-access policy changes
  ...
  read-only API for access-level state readable by a non-super-admin is not identified and
  is *tbd*.
...
**`apps.licensing` is dropped**, per [C13](../wall-e/14-hld-challenge.md). Removing it from
Wall-E's runbook and `walle_setup.py` is an edit listed in
...
Since 2026-09-13 the consent happens earlier: in one sitting with the observe-and-report layer,
**before the super-admin grant**, with the widened read set fixed first (E-16;
[05-stages.md](05-stages.md#the-stage-table)). The first reason loses its force because the
daily Reports API poll uses the token continuously; the second is why the scope set is decided
before that sitting.
...
Since 2026-09-13 [decision 26](../wall-e/09-open-decisions.md) is **closed for
...
Every grant an Eve identity holds on a resource in `WALLE_PROJECT` is made **by Wall-E's
runbook** (`walle_setup.py`, config keys `EVE_PROJECT`, `SA_EVE`, `SA_EVE_VERIFIER`,
`SA_EVE_CONSOLE`) against Eve's full `…@${EVE_PROJECT}.iam.gserviceaccount.com` emails; Eve's
runbook requests and verifies them and never makes them. They are rows of
[../project-topology.md §3](../project-topology.md#3-cross-project-grants): row 3
(`run.invoker` on `walle-actions` for the controller, verifier and console), row 4
(dataset-level `READER` on `walle_audit`, with `jobUser` only in `EVE_PROJECT`; E-9), row 27
(`run.invoker` on `walle-actions-super`, halt path only: the verifier for the reconciler
limb's super-admin-lane halts, the controller for halts raised at the approval point),
row 34 (`READER` on `walle_audit` for `eve-export@`), the ungranted rows 11
(`agentregistry.viewer`) and 12 (Firestore: no read until topology decision 44 lands), and
anti-grant row 26 — no Eve identity holds any project-level role in `WALLE_PROJECT`, a rule
whose history is E-12 in [09-open-decisions.md](09-open-decisions.md#the-twenty-decisions).

### The control-caller allowlist

`run.invoker` is granted per service, not per path, so `walle-actions` narrows paths with an
in-app allowlist keyed on the verified identity token:
`CONTROL_CALLER_ALLOWLIST="${SA_EVE},${SA_EVE_VERIFIER},${OPERATORS}"` carries `eve-controller@`,
`eve-verifier@` (so that `eve-reconciler` can halt and demote without holding the signing role)
and the operators group, and Eve's entries are created at S3 entry. `eve-console@` sits only on
the separate read-endpoint allowlist, for `GET /v1/plans` and `GET /v1/ladder`, which never
admits approve, veto, halt, demote or execute. The lists, their build assertions and the
CI-only stub caller's denial tests are specified in
[../wall-e/03-lld.md](../wall-e/03-lld.md#gcp-resource-inventory).

## Grants Eve's runbook makes to principals outside `EVE_PROJECT`

Grants on Eve's resources to foreign principals are made by Eve's runbook
([07-build-runbook.md](07-build-runbook.md)) or Eve's owner and are rows of
[../project-topology.md §3](../project-topology.md#3-cross-project-grants): rows 28–29
(dataset-level `READER` on `eve_quality` for `mo-metrics@` and the validator custodian, from
Wall-E's Stage 1), rows 30–31 (`eve-advisor@`'s `READER` on the authorised-view dataset and
`dataEditor` on `eve_advice`), row 18 (`mo-metrics@` on the mirror's dataset from S4) and rows
14–16 (the three Wall-E carve-outs of topology decision 48). Two grants leave the tenant's
organisation — rows 32–33, `eve-export@`'s `dataEditor` on `eve_mirror` and `objectCreator` on
the witness bucket — and are made by the witness administrators on witness resources, so no
witness principal holds anything in the tenant. The grades of Eve's own verdicts live in
`eve_grades` in `VALIDATOR_PROJECT` (rows 45–46, E-21), which no Eve identity reads or writes.
...
- **Neither `eve-controller@` nor `eve-verifier@` holds any `aiplatform.*` permission.**
  `aiplatform.reasoningEngines.query` is removed from `eve-controller@` per
  [C10](../wall-e/14-hld-challenge.md), so no Eve identity is among the two query principals on
  Wall-E's engine ([../wall-e/12-agent-identity.md §7](../wall-e/12-agent-identity.md#7-locking-aiplatformreasoningenginesquery);
  topology row 13). CI asserts the resulting IAM policy.
...
**The project-level absence, and the reporting path.** The section holds for the two runtime
identities, with a project-level absence beside it: a `gcp.restrictServiceUsage` denylist on `aiplatform.googleapis.com`
...
`Assumption:` until the platform's perimeter spikes pass, Eve's egress control is VPC Service
Controls plus a host allowlist that never registers `aiplatform.googleapis.com`; the allowlist
and its closure by platform decision P3 are E-19 in
[09-open-decisions.md](09-open-decisions.md#the-twenty-decisions), and the perimeter model is
[../agentic-platform/06-gateways-model-armor-perimeter.md §4](../agentic-platform/06-gateways-model-armor-perimeter.md#4-the-perimeter).
...
- **Eve's own daily drift job in `EVE_PROJECT`**, and the Eve owner running
  ...
  Eve's. Under platform HLD §4.7 and §18 item 25 the drift job's expected
...
- **Wall-E's drift job** asserts only what is visible in `WALLE_PROJECT`: `walle-actions@`
  ...
  project-level role in `WALLE_PROJECT` (the E-12 exception list is empty), and the allowlists carry the expected
  foreign emails.
...
Eve holds no domain-wide delegation at any stage: `eve@<domain>` holds its own consented
refresh token and is never impersonated by anything, under the platform rule — no DWD for any
agent — stated once in
[../agentic-platform/04-identity-and-privileged-access.md §1](../agentic-platform/04-identity-and-privileged-access.md#1-principles-inherited-stated-once)
item 7. Eve is exactly where DWD would look attractive, since a verifier that reads the whole
tenant is the textbook case for a delegated service account, but a delegated Eve could
impersonate the robot it verifies, and a verifier that can act as the thing it verifies is not
a verifier.
...
| `aiplatform.googleapis.com` denied on `EVE_PROJECT` by a project-level `restrictServiceUsage` denylist | Eve's drift job; SG-04 in the SIEM | Daily; on change |
| `eve-approval` protection level is `HSM` | Phase 11 verify; the folder constraint refuses anything else | At creation; drift daily |
| `eve@`'s consented scope set equals the ten-scope set exactly, no content scope, `apps.licensing` absent; `eve@` is not a super admin | Phase 8 token verification; Eve's roster check (`users.list` `isAdmin`) | Onboarding; daily |
| No control-path identity holds a read on `eve_advice`; `eve-advisor@` holds nothing but its two dataset grants | CI assertion on the dataset access lists; Eve's drift job | Every build; daily |
...
- [../agentic-platform/01-hld.md](../agentic-platform/01-hld.md) §13.2–§13.3 — the two paths, the witness, `eve_quality`
