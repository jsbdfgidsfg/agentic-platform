...
- **Objective restated 2026-09-13; see the platform HLD**
  ([../agentic-platform/01-hld.md](../agentic-platform/01-hld.md) §13.2 and §18): CC-34 to
  CC-37 carry the edits its Eve items (11–17) force back on Wall-E (owner the Eve owner, the
  Wall-E agent owner for their landing, P143); Wall-E's own super-admin edits are §18 items 1–10.
...
| CC-34 | Wall-E's runbook; the in-app control list of `walle-actions-super` | `roles/run.invoker` on the Cloud Run service `walle-actions-super` for Eve's halt identities, `eve-controller@` and `eve-verifier@`, service-level; that service's control list admits them to the halt endpoint and to nothing else; `halt_all` for `log_pipeline_silent` and severity-1 detection rules applies to both lanes | Platform HLD §18 item 25 (the grant, named for `eve-controller@`) and page 07 §7 (the halt, issued by `eve-reconciler`, identity `eve-verifier@`), so both identities hold it, halt path only ([../project-topology.md](../project-topology.md#3-cross-project-grants) row 27). The allowlist rule of CC-30 (operators always present) applies to this list too. | The super-admin grant |
| CC-35 | Wall-E's runbook, `walle_audit` dataset access | Dataset-level `READER` on `walle_audit` for `eve-export@${EVE_PROJECT}` | The daily newline-JSON export with SHA-256 manifests into Eve's locked bucket and the witness (P107) | Wall-E's Stage 1; the witness push before the super-admin grant |
| CC-36 | Wall-E's band-B audit rows | The audit rows `walle-actions-super` writes for `/v1/execute-generic` and `/v1/handoff` are readable by `eve-verifier@` and `eve-export@` in the same dataset-level form as `walle_audit` (dataset *tbd* on Wall-E's side; `audit.schema`) | Reconciliation over every stream matches robot-attributed events against `walle_audit` **and** the band-B rows; a robot event with neither is `reconciliation_gap` (platform HLD §13.2) | The super-admin grant |
...
**The seeded-fault half.** The twelve faults are one per invariant class; their list, the
two negative controls and the required verdicts are defined in
[05-stages.md](05-stages.md#the-s3-exit-gate-twelve-seeded-faults-and-two-negative-controls).
At 95 % over twelve, one whole class may be undetected and the gate still passes — and every
one of those classes is a hard invariant whose error budget is zero. Eleven of twelve is not a
grade, it is a hole. C12 raised it to 100 % and this design keeps it there, with the two
negative controls an always-refusing Eve fails, while an always-approving Eve fails faults 1
to 8. That pair is what the gate actually measures.
...
1. **The two absence sweepers and the passive `eve_last_seen` stamp** (CC-11). `eve_silence`:
   ...
   | Mechanism | Condition and effect | Active from |
   |---|---|---|
   | `eve_silence` | Any plan at `pending_eve` for more than four business hours: `no_autonomous`, origin `breaker`, with an incident note | S3 entry |
   | `eve_evidence_stale` | Any executed L5 item with no row in `${EVE_RECEIPTS_DS}.verdict_receipts` after 60 minutes: promotions freeze; after four hours: that cell drops to L4 | S4 entry (with item 5's read) |
   | `eve_last_seen` | Stamped passively from any successfully authenticated `eve-controller@` or `eve-verifier@` call; Cloud Monitoring absence policy at 15 minutes | S3 entry |

   How they run. Deterministic code in `walle-actions` with no Eve involvement: a 1-minute
   internal tick plus request-path evaluation, with the timers extending the existing L3 TTL
   sweeper. They act as `walle-actions@`, writing `config_versions` rows with origin
   `breaker`. The code is present from the first commit and inert until the stage in the
   table.

   Properties that are not negotiable, because they are easy to "improve" into uselessness:
   - **They key on the absence of work, never on reported health.** A liveness signal Eve
     publishes cannot prove Eve is alive: a wedged Eve that still heartbeats keeps autonomy
     alive. This is why the rejected heartbeat endpoint is replaced by the passive stamp
     ([01-hld.md](01-hld.md) choice 2).
   - **Nothing raises on Eve's return**, and the presence of an Eve row is never treated as
     permission. `eve_evidence_stale` is absence-only: it checks that a receipt arrived and
     never reads what a verdict says, because the authorised view exposes
     `(run_id, item, verdict_ts)` and nothing else ([06-failure-modes.md](06-failure-modes.md)
     names the residual: a compromised Eve can write false receipts and suppress it).
   - **The absence alert is an absence policy, not a threshold.** A threshold-only policy is
     silent exactly when the metric stops being written (verified 2026-09-12,
     [Alerting policies in depth](https://docs.cloud.google.com/monitoring/alerts/concepts-indepth)).

   `eve_last_seen` is also the source of Eve's S4 availability figure (the fraction of
   scheduled business-hour passes that completed, never self-reported by Eve;
   [09-open-decisions.md](09-open-decisions.md) E-10, [05-stages.md](05-stages.md)). The
   thresholds belong to `walle-actions`; Eve's `thresholds.yaml`
   ([03-lld.md](03-lld.md) §7) holds the reference copy the two are asserted equal against.
   Wall-E's CI proves the properties: the three mechanisms have unit tests that assert the
   "never raises" property directly ([07-build-runbook.md](07-build-runbook.md) Phase 6).
...
Listing these matters as much as listing the edits: a reader who sees thirty-seven changes
...
| **`walle_workspace_logs` and its no-actor-exclusion property** | Eve stops reconciling against it and reconciles against its own sink instead. The standing check that Wall-E's copy still carries no actor exclusion — a drift check on Wall-E, in the direction that has no other watcher — needs a grant no set makes today; topology decision 47 gives it a form: a data-level comparison against Eve's copy, first proposed as dataset-level `READER` on `walle_workspace_logs` in `WALLE_PROJECT` for `eve-verifier@` (topology row 5) and, since 2026-09-13 (P104, P107), reading the authorised view `platform_logs_views.walle_workspace_logs` in `LOGGING_PROJECT` instead ([../project-topology.md](../project-topology.md#3-cross-project-grants) row 40). Until that lands the check is not run. |
...
CC-5 is small in the table and load-bearing in practice. The rule — every key version's PEM
exported before first use to Eve's locked bucket and to Wall-E's repository, why offline
verification is strictly stronger than a live fetch, and manual rotation with disable-never-destroy
— is defined in [02-identity-and-auth.md](02-identity-and-auth.md#the-pem-archive-in-two-places-before-first-use).
What it changes on Wall-E's side:

- `walle-actions` verifies an Eve approval **offline, against the PEM committed at
  `contracts/eve-public-keys/<version>.pem`** under the same CODEOWNERS as `ladder.yaml`, so
  changing which key verifies an approval takes a merged pull request, not a binding. Cloud
  KMS `getPublicKey` cross-project, using `roles/cloudkms.publicKeyViewer`, is the
  **fallback** only.
- The envelope names the full KMS key-version resource name (CC-2), because a KMS signature
  carries no version identifier of its own, and the verifier selects the pinned PEM by that
  name.
- At S0 the path is exercised with a **CI-only test key**, so the verification code is real
  and tested six to eight weeks before a real key exists.
...
| **Before the super-admin grant** | CC-34, CC-36, and CC-35's witness half | The observe-and-report layer must halt both lanes and reconcile band B before Wall-E holds Super Admin (platform HLD §0.4, §13.2). |
| **Wall-E's Stage 1** | CC-35, CC-37 | The daily export and the Eve quality inputs Mo reads (P143 gate for §18 item 17). |

---

## 7. Decisions

The `E-` decisions this page depends on are defined, with their reasoning, recommendation, gate
and dated lines, in [09-open-decisions.md](09-open-decisions.md#the-twenty-decisions), which is
also where each is recorded. Which contract-change rows each one drives:

- **E-1** (Eve's own GCP project, answered yes 2026-09-13) — the root cause of CC-14,
  CC-16, CC-17, CC-20, CC-24, CC-25, CC-29, CC-30 and CC-31.
- **E-3** (`items_hash`) — CC-1. **E-4** (`eve_authority`, `eve_authority_advisory`) — CC-4,
  CC-6. **E-5** (the absence sweepers) — CC-11. **E-7** (halting bias) — CC-10.
  **E-8** (demotion granularity) — CC-8. **E-9** (Eve's BigQuery grant) — CC-12, CC-22.
  **E-11** (outcome code for an Eve-rejected item) — CC-3. **E-12** (no project-level role in
  `WALLE_PROJECT`, settled by removal) — CC-26. **E-17** (the `verdict_receipts` read) —
  CC-11 and §3 item 5. **E-20** (`walle-events` schema) — CC-13.
