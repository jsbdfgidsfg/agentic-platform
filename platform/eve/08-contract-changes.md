...
| CC-12 | [`../wall-e/08-team-eve-mo.md`](../wall-e/08-team-eve-mo.md), shared data plane | Specify the BigQuery grant that exists nowhere: dataset-level `roles/bigquery.dataViewer` on `walle_audit` for Eve's identities, and `roles/bigquery.jobUser` **in Eve's own project**. Never project-level `dataViewer` | Every "Eve and Mo read BigQuery" line in the contract is unbuilt — `add_dataset_access` is called exactly twice in the runbook and neither call is for Eve or Mo. Dataset-level keeps the grant to the six tables; project-level would be a lateral path into Wall-E's project; `jobUser` in Eve's project keeps job creation and query cost off Wall-E's bill. New, [E-9](09-open-decisions.md) | S3 entry |
...
| CC-26 | [`../wall-e/setup/walle_setup.py`](../wall-e/setup/walle_setup.py) | `EVE_PROJECT_ROLES` — today `("roles/datastore.viewer",)` — becomes the full list of roles Eve's identities hold **in Wall-E's project**: `roles/datastore.viewer` and `roles/agentregistry.viewer`, applied to `eve-controller@` **and** `eve-verifier@`. The dataset grants of CC-12 are not project roles and stay separate | The set currently asserts in one place that Eve's only project-level role in Wall-E's project is `datastore.viewer`, and grants `agentregistry.viewer` in another. Both are read-only, both are in the runbook, and the two wordings cannot both be literally satisfied. The reworded rule is "no project-level role beyond the two named read-only roles, and no write role". New, [E-12](09-open-decisions.md) | With the design record; the code at S3 entry |
...
| CC-28 | [`../wall-e/setup/walle_setup.py`](../wall-e/setup/walle_setup.py) | Remove `SA_EVE` from `lock_engine_iam()`'s member list for the custom `walleEngineQuery` role | Same disagreement as CC-15, in code. Two query principals remain: the Discovery Engine service agent and `walle-dispatcher@`. Already binding, C10 | Before S3 entry |
| CC-29 | [`../wall-e/setup/walle_setup.py`](../wall-e/setup/walle_setup.py) | `ensure_kms()` creates ring `eve` in `EVE_PROJECT`; the existing assertion that `walle-actions@` holds only `publicKeyViewer` becomes a **cross-project** assertion; `ensure_eve_secrets()` creates in `EVE_PROJECT`; the secret separation sweep spans both projects; add an assertion that **no Wall-E principal appears in Eve's project IAM policy at all** | The build already fails on a project- or keyring-level signer grant to `walle-actions@` because "it could mint an Eve approval". That check has to follow the key, and gains a stronger sibling once the project boundary exists. New, [E-1](09-open-decisions.md) | Before Eve onboarding |
| CC-30 | [`../wall-e/setup/walle_setup.py`](../wall-e/setup/walle_setup.py) | The `CONTROL_CALLER_ALLOWLIST` assertion accepts **three** members and still FAILs when `${OPERATORS}` is absent | The assertion exists precisely because an allowlist holding Eve alone means no human can halt or demote. Adding `eve-verifier@` must not weaken it into a length check. New, from [E-1](09-open-decisions.md) | With the allowlist change, S3 entry |
| CC-31 | [`../wall-e/setup/walle.env.example`](../wall-e/setup/walle.env.example) | Add `EVE_PROJECT`. `EVE_ROBOT` and `EVE_TOKEN_VERSION` stay as they are. `MO_PRINCIPAL` is untouched here and belongs to Mo's design | The example env file is what anyone copies. New, [E-1](09-open-decisions.md) | With CC-24 |
...
1. **The two absence sweepers and the passive `eve_last_seen` stamp** (CC-11). `eve_silence`:
   ...
   `eve.verdict_receipts` after 60 minutes freezes promotions, and after four hours drops that
...
5. **The `eve.verdict_receipts` read and the plan-lifecycle timer** (CC-11), the read narrowed
   to an existence-only authorised view.
...
Listing these matters as much as listing the edits: a reader who sees thirty-one changes
...
| **`walle_workspace_logs` and its no-actor-exclusion property** | Eve stops reconciling against it and reconciles against its own sink instead, but asserts on every pass that Wall-E's copy still carries no actor exclusion — a drift check on Wall-E, in the direction that has no other watcher. |
...
|---|---|---|
| **First commit / S0, in code** | CC-2 (column), CC-11 (inert), CC-23 | The seams, the audit columns and the stub caller are what let Stage 0 pass without provisioning anything of Eve's. |
| **Before the approve endpoint is built** | CC-1, CC-2, CC-4 (schema), CC-5, CC-6 (schema) | Each one changes the bytes that are signed or the strings that are denied. Retrofitting any of them invalidates every approval produced before the retrofit. |
| **Now, as wording** | CC-7, CC-8, CC-10, CC-13, CC-15, CC-16, CC-17, CC-26 | Decisions already taken, or readings this design fixes. They cost nothing to land and they are what the next reader inherits. |
...
| **E-7** | **Eve's halting bias, restated** | 08 item 5's "own judgement" is void under C12; "bias toward halting when uncertain" would make a degraded Eve the outage | At the approval point the conservative act is **refuse** (the plan waits, costing minutes); **halt** is reserved for the closed invariant-trigger list and is never discretionary. Record as an amendment to 08 item 5 | With the design record | [09-open-decisions.md](09-open-decisions.md) |
